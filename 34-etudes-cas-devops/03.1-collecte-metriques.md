...
La collecte de métriques est le cœur d'un agent de monitoring système. Elle consiste à récupérer périodiquement des informations sur l'état du système (CPU, mémoire, disque, réseau) pour les exposer ou les transmettre à un système de supervision.

Dans cette section, nous allons explorer comment collecter ces métriques en C sur un système Linux en utilisant les interfaces natives du système d'exploitation.

---

## Pourquoi collecter des métriques en C ?

- **Performance** : Un agent écrit en C consomme très peu de ressources
- **Pas de dépendances** : Pas besoin de runtime (Python, JVM)
- **Accès direct au système** : Lecture optimisée des fichiers `/proc` et `/sys`
- **Portabilité** : Code facilement déployable sur différentes distributions Linux

---

## Les sources de métriques sur Linux

Linux expose les informations système via des **pseudo-systèmes de fichiers** :

### 1. `/proc` - Informations processus et système
Le système de fichiers `/proc` est une interface virtuelle vers le noyau Linux.

**Fichiers clés :**
- `/proc/stat` : Statistiques CPU globales
- `/proc/meminfo` : Informations mémoire détaillées
- `/proc/loadavg` : Charge système (load average)
- `/proc/uptime` : Temps depuis le démarrage
- `/proc/net/dev` : Statistiques réseau par interface
- `/proc/diskstats` : Statistiques disque

### 2. `/sys` - Informations matériel et kernel
Le système de fichiers sysfs expose les informations du matériel.

**Exemples :**
- `/sys/class/thermal/thermal_zone0/temp` : Température CPU
- `/sys/block/sda/stat` : Statistiques disque

### 3. Appels système
Certaines métriques nécessitent des appels système directs :
- `sysinfo()` : Informations système globales
...
Le fichier `/proc/stat` contient les compteurs de temps CPU en **jiffies** (unité de temps kernel, généralement 1/100e de seconde).

**Format :**
```
cpu  user nice system idle iowait irq softirq steal guest guest_nice  
cpu0 ... (même format pour chaque cœur)  
```

**Signification des champs :**
- `user` : Temps en mode utilisateur
- `nice` : Temps en processus à priorité modifiée
- `system` : Temps en mode kernel
- `idle` : Temps inactif
- `iowait` : Temps d'attente I/O
- `irq` : Temps de traitement d'interruptions matérielles
- `softirq` : Temps de traitement d'interruptions logicielles
- `steal` : Temps volé par l'hyperviseur (virtualisation)
...
int read_cpu_stats(cpu_stats_t *stats) {
    ...
    // Lire la première ligne (cpu global)
...
Pour obtenir le pourcentage d'utilisation, il faut **deux mesures espacées dans le temps** :
...
**Points clés :**
- Les valeurs dans `/proc/stat` sont des **compteurs cumulatifs** depuis le démarrage
- Il faut calculer la **différence** entre deux mesures
- Le pourcentage = (temps_actif / temps_total) × 100
...
Le fichier `/proc/meminfo` contient toutes les informations sur la mémoire système.
...
### Exemple de code : Lecture de la mémoire
...
void print_memory_usage(void) {
    ...
    // Affichage
    printf("Mémoire Totale:     %lu MB\n", stats.total_kb / 1024);
    printf("Mémoire Utilisée:   %lu MB (%.2f%%)\n",
           used_kb / 1024, mem_usage_percent);
    printf("Mémoire Disponible: %lu MB\n", stats.available_kb / 1024);
    printf("Swap Total:         %lu MB\n", stats.swap_total_kb / 1024);
    printf("Swap Utilisé:       %lu MB (%.2f%%)\n",
           swap_used_kb / 1024, swap_usage_percent);
}
```

**Points importants :**
- `MemAvailable` est plus pertinent que `MemFree` (inclut la mémoire récupérable)
- La mémoire utilisée réelle = Total - Free - Buffers - Cached
...
Le fichier `/proc/diskstats` contient les statistiques I/O pour chaque périphérique.
...
**Colonnes principales (indexées à partir de 1) :**
- Col 3 : Nom du périphérique
- Col 4 : Lectures complétées
- Col 6 : Secteurs lus
- Col 8 : Écritures complétées
- Col 10 : Secteurs écrits
...
int read_disk_stats(const char *device_name, disk_stats_t *stats) {
    ...
    if (!found) {
        fprintf(stderr, "Périphérique %s non trouvé\n", device_name);
        return -1;
    }
...
void monitor_disk(const char *device) {
    ...
    // 1 secteur = 512 octets généralement
...
Le fichier `/proc/net/dev` contient les statistiques réseau par interface.
...
### Exemple de code : Lecture des stats réseau
...
void monitor_network(const char *interface) {
    ...
    printf("Interface %s:\n", interface);
    printf("  Réception:    %.2f MB/s (%llu paquets/s)\n",
...
## Utilisation de l'appel système `sysinfo()`

Pour certaines métriques globales, l'appel système `sysinfo()` est plus simple.
...
- Informations cohérentes au même instant

**Limitations :**
- Moins de détails que `/proc/meminfo`
- Pas d'accès aux métriques par périphérique

---

## Structure d'un collecteur de métriques

### Organisation modulaire

Un bon agent de monitoring sépare les responsabilités :
...
Toujours vérifier les retours des fonctions :

```c
if (read_cpu_stats(&stats) != 0) {
    // Log l'erreur, retourner une valeur par défaut, ou arrêter
    fprintf(stderr, "Impossible de lire les stats CPU\n");
    return -1;
}
```

### 2. Éviter les fuites de descripteurs
Toujours fermer les fichiers, même en cas d'erreur :
...
Ou mieux, utiliser un pattern de nettoyage systématique :
...
### 3. Buffer sizing
Utiliser des buffers suffisamment grands pour éviter les dépassements :
...
Vérifier que les valeurs lues sont cohérentes :
...
### 5. Échantillonnage temporel
Pour les métriques basées sur des différences (CPU, réseau, disque) :
- Utiliser un intervalle constant (ex: 1 seconde)
- Stocker la mesure précédente
- Calculer le delta
...
Pour des lectures fréquentes, garder les descripteurs ouverts :
...
### Collecte par cœur CPU

Pour monitorer chaque cœur individuellement :
...
### Collecte des métriques réseau pour toutes les interfaces
...
La collecte de métriques système en C sur Linux repose sur :

1. **Lecture des pseudo-fichiers** (`/proc`, `/sys`)
   - Parsing de formats textuels simples
   - Gestion des erreurs d'ouverture/lecture

2. **Calculs de différences** pour les métriques dynamiques
   - Deux échantillons espacés dans le temps
   - Calcul de taux (par seconde)

3. **Appels système** pour certaines métriques
   - `sysinfo()` pour vue globale rapide
   - Complémentaire aux fichiers `/proc`
...
Dans la prochaine section, nous verrons comment **exposer ces métriques au format Prometheus** pour intégration dans un écosystème de monitoring moderne.
...
⏭️ [Export Prometheus](/34-etudes-cas-devops/03.2-export-prometheus.md)
