🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 1.3.1 Focus sur C23 : Nouveautés et état de l'adoption

## Introduction

Le langage C, bien qu'ayant plus de 50 ans, continue d'évoluer. En 2024, une nouvelle norme majeure a été publiée : **C23** (officiellement ISO/IEC 9899:2024). C'est la première mise à jour majeure depuis C11 en 2011, soit plus de 12 ans d'évolution !

Dans cette section, nous allons découvrir ce que C23 apporte de nouveau, comprendre son état d'adoption actuel en 2025, et surtout, déterminer si vous devriez l'utiliser dès maintenant ou vous concentrer sur des versions plus établies.

> 💡 **Pour les débutants** : Ne vous inquiétez pas si C23 vous semble avancé. La plupart des projets en production utilisent encore C99 ou C11. Cette section vous donne une vue d'ensemble pour que vous sachiez ce qui existe, mais vous n'avez pas besoin de tout maîtriser immédiatement.

---

## 📅 Chronologie des standards C

Avant de plonger dans C23, voici un rappel des versions du langage C :

| Année | Standard | Nom courant | Statut en 2025 |
|-------|----------|-------------|----------------|
| 1989 | ANSI C | C89 / C90 | Legacy, mais encore utilisé |
| 1999 | ISO C99 | C99 | Très largement utilisé |
| 2011 | ISO C11 | C11 | **Standard de facto en production** ⭐ |
| 2018 | ISO C17 | C17 | Corrections de bugs de C11 |
| 2024 | ISO C23 | C23 | **Nouvelle norme, adoption progressive** 🆕 |

> 🎯 **À retenir** : C11 (avec les corrections C17) est actuellement la version la plus utilisée en production. C23 est très récent et son adoption est encore limitée en 2025.

---

## 🆕 Les principales nouveautés de C23

C23 apporte plus de 50 changements et améliorations au langage. Voici les plus significatifs, classés par importance pour un débutant.

### 1. Attributs standardisés [[...]]

**Ce que c'est** : Une syntaxe standardisée pour ajouter des métadonnées au code.

**Avant C23** (avec extensions GCC) :
```c
__attribute__((deprecated)) void old_function(void);
__attribute__((noreturn)) void exit_program(void);
```

**Avec C23** (syntaxe standard) :
```c
[[deprecated]] void old_function(void);
[[noreturn]] void exit_program(void);
```

**Pourquoi c'est utile** :
- Syntaxe plus propre et plus lisible
- Compatible avec C++ (qui utilise déjà cette syntaxe)
- Standardisée, donc portable entre compilateurs

**Attributs standardisés en C23** :
- `[[deprecated]]` : Marquer du code obsolète
- `[[nodiscard]]` : Le retour de fonction ne doit pas être ignoré
- `[[noreturn]]` : La fonction ne retourne jamais
- `[[maybe_unused]]` : Peut ne pas être utilisé (évite les warnings)
- `[[reproducible]]` : Fonction reproductible (même input → même output)
- `[[unsequenced]]` : Fonction pure (aucun effet de bord)

**Exemple pratique** :
```c
#include <stdio.h>
#include <stdlib.h>

// Indique qu'on doit vérifier la valeur de retour
[[nodiscard]] int initialize_system(void) {
    // Code d'initialisation
    return 0; // 0 = succès
}

// Cette fonction ne retourne jamais
[[noreturn]] void fatal_error(const char *message) {
    fprintf(stderr, "FATAL: %s\n", message);
    exit(1);
}

int main(void) {
    // Warning si on ignore le retour !
    initialize_system();

    return 0;
}
```

### 2. Le mot-clé `nullptr`

**Le problème avec `NULL`** :
En C classique, `NULL` est souvent défini comme `((void *)0)` ou simplement `0`, ce qui peut créer des ambiguïtés.

**Avant C23** :
```c
int *ptr = NULL;  // NULL est juste 0 ou (void*)0
```

**Avec C23** :
```c
int *ptr = nullptr;  // nullptr est un véritable type pointeur null
```

**Pourquoi c'est mieux** :
- Plus sûr en termes de types
- Compatible avec C++ (qui a `nullptr` depuis C++11)
- Évite les conversions implicites problématiques

**Exemple comparatif** :
```c
// Avant C23 : NULL peut être 0 ou (void*)0 selon l'implémentation
#define NULL ((void*)0)  // Définition courante

int *p1 = NULL;          // OK  
int val = NULL;           // Compile sans warning sur certains compilateurs !  

// Avec C23 : nullptr a son propre type (nullptr_t)
int *p2 = nullptr;        // OK
// int val2 = nullptr;    // Erreur de compilation ! Plus sûr.
```

### 3. `typeof` et `typeof_unqual`

**Ce que c'est** : Obtenir le type d'une expression.

**Avant C23** (extension GNU) :
```c
__typeof__(variable) new_variable;
```

**Avec C23** (standard) :
```c
typeof(variable) new_variable;  
typeof_unqual(variable) unqualified_variable;  // Sans const/volatile  
```

**Exemple pratique** :
```c
int x = 42;  
typeof(x) y = x;  // y est de type int  

const int a = 10;  
typeof(a) b = a;           // b est "const int"  
typeof_unqual(a) c = a;    // c est juste "int" (sans const)  
```

**Pourquoi c'est utile** :
- Création de macros génériques plus robustes
- Évite de répéter les types
- Utile pour la métaprogrammation

### 4. Constantes binaires

**Avant C23** :
```c
int flags = 0x0F;  // Hexadécimal uniquement pour les bits
```

**Avec C23** :
```c
int flags = 0b00001111;  // Notation binaire directe !
```

**Utilisation pratique** :
```c
// Masques de bits plus lisibles
#define FLAG_READ   0b00000001
#define FLAG_WRITE  0b00000010
#define FLAG_EXEC   0b00000100
#define FLAG_DELETE 0b00001000

int permissions = FLAG_READ | FLAG_WRITE;  // 0b00000011
```

**Bonus : Séparateurs de chiffres** (aussi en C23) :
```c
int big_number = 1'000'000;        // Plus lisible que 1000000  
int binary = 0b1111'0000'1010'1100;  // Groupes de 4 bits  
```

### 5. `#embed` pour inclure des fichiers binaires

**Le problème avant** :
Pour inclure un fichier binaire (image, fonte, firmware...) dans un programme C, il fallait le convertir en tableau C.

**Avant C23** :
```c
// Fichier généré par un outil externe
const unsigned char data[] = {
    0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A,
    // ... des milliers de lignes ...
};
```

**Avec C23** :
```c
const unsigned char logo[] = {
    #embed "logo.png"  // Inclus directement le fichier binaire !
};
```

**Pourquoi c'est révolutionnaire** :
- Plus besoin d'outils externes pour convertir les données
- Temps de compilation réduit
- Code source plus propre

**Cas d'usage** :
- Systèmes embarqués (inclure firmware, images)
- Jeux (inclure assets, textures)
- Applications (inclure icônes, fontes)

### 6. Améliorations des `constexpr`

**Ce que c'est** : Évaluation à la compilation (introduit partiellement en C11, étendu en C23).

**Exemple C23** :
```c
constexpr int BUFFER_SIZE = 256;  
constexpr int MAX_ITEMS = BUFFER_SIZE / 4;  // Calculé à la compilation  

int buffer[BUFFER_SIZE];  // OK, BUFFER_SIZE est une vraie constante
```

**Différence avec `const`** :
```c
const int size = 100;          // Constante à l'exécution  
int array1[size];              // ❌ Erreur en C standard (VLA requis)  

constexpr int size_ce = 100;   // Constante à la compilation  
int array2[size_ce];           // ✅ OK !  
```

### 7. Booleans améliorés

**Avant C23** :
```c
#include <stdbool.h>  // Nécessaire pour bool, true, false
bool is_valid = true;
```

**Avec C23** :
```c
// bool, true, false sont maintenant des mots-clés !
bool is_valid = true;  // Plus besoin de #include <stdbool.h>
```

**Bonus C23 : `_BitInt(N)`**

Types entiers de taille arbitraire :
```c
_BitInt(7) small_int = 100;    // Entier sur exactement 7 bits
_BitInt(128) huge_int = 12345; // Entier sur 128 bits
```

### 8. Labels avant déclarations

**Avant C23** :
```c
void function() {
    goto end;

    int x = 10;  // Il faut déclarer avant le label

end:
    printf("Done\n");
}
```

**Avec C23** :
```c
void function() {
    goto end;

end:  // Label peut être directement suivi d'une accolade
    int x = 10;  // Déclaration après le label !
    printf("Done\n");
}
```

### 9. `auto` redéfini (comme en C++)

**En C classique** :
`auto` était un mot-clé inutile (stockage automatique par défaut).

**En C23** :
`auto` peut être utilisé pour l'inférence de type (comme en C++) :
```c
auto x = 42;           // x est int  
auto ptr = &x;         // ptr est int*  
auto message = "Hi";   // message est char*  
```

> ⚠️ **Attention** : Cette fonctionnalité est encore expérimentale et peut évoluer.

### 10. Autres améliorations notables

**Chaînes littérales UTF-8 garanties** :
```c
const char8_t *str = u8"Hello, 世界";  // UTF-8 garanti
```

**Suffixes pour size_t** :
```c
size_t size = 100uz;  // 'uz' ou 'zu' pour size_t littéral
```

**`static_assert` sans message** :
```c
static_assert(sizeof(int) == 4);  // Message optionnel en C23
```

**Attributs sur les types** :
```c
struct [[deprecated]] OldStruct {
    int data;
};
```

---

## 📊 État de l'adoption de C23 en 2025

### Support des compilateurs principaux

| Compilateur | Version | Support C23 | Statut |
|-------------|---------|-------------|--------|
| **GCC** | 14.x - 15.x | ~80-90% | 🟡 Partiel |
| **Clang** | 18.x - 19.x | ~75-85% | 🟡 Partiel |
| **MSVC** | 2022+ | ~40-50% | 🟠 Limité |
| **ICC** (Intel) | 2024+ | ~60% | 🟠 Limité |

> 🔔 **État actuel** : En 2025, C23 est supporté **partiellement** par les compilateurs majeurs. Certaines fonctionnalités marchent, d'autres pas encore.

### Comment vérifier le support ?

**Avec GCC** :
```bash
gcc --version  
gcc -std=c23 -dM -E - < /dev/null | grep __STDC_VERSION__  
```

**Avec Clang** :
```bash
clang --version  
clang -std=c23 -dM -E - < /dev/null | grep __STDC_VERSION__  
```

**Résultat attendu** :
```
__STDC_VERSION__ 202311L
```
(`202311L` = valeur définie par le standard C23, formellement publié par l'ISO en octobre 2024)

### Fonctionnalités C23 bien supportées (2025)

✅ **Déjà utilisables en production** :
- Attributs `[[deprecated]]`, `[[nodiscard]]`, etc.
- Constantes binaires `0b...` et séparateurs `'`
- `typeof` et `typeof_unqual`
- `static_assert` sans message
- Améliorations des booleans

🟡 **Support partiel / expérimental** :
- `nullptr` (GCC 14+, Clang 18+)
- `constexpr` étendu
- `#embed` (support variable selon version)
- `auto` pour inférence de type

❌ **Pas encore largement supporté** :
- `_BitInt(N)` (support limité)
- Certaines fonctionnalités de bibliothèque standard
- Fonctionnalités avancées de métaprogrammation

---

## 🤔 Devriez-vous utiliser C23 en 2025 ?

### ✅ Utilisez C23 si...

- Vous démarrez un **nouveau projet personnel**
- Vous voulez **expérimenter** les nouvelles fonctionnalités
- Votre environnement cible utilise **GCC 14+ ou Clang 18+**
- Vous êtes prêt à gérer des **incompatibilités potentielles**
- Vous voulez être **à la pointe** du langage

### ❌ N'utilisez PAS C23 si...

- Vous travaillez sur du **code de production critique**
- Votre projet doit être **portable** sur de nombreux systèmes
- Vous utilisez des **toolchains industrielles** (souvent en retard)
- Votre équipe n'est pas familière avec C23
- Vous ciblez des **systèmes embarqués** avec compilateurs spécifiques

### 🎯 Recommandation pour les débutants

**Apprenez d'abord C11** (ou C99) pour les raisons suivantes :

1. **Documentation abondante** : Tous les tutoriels, livres, cours utilisent C99/C11
2. **Compatibilité maximale** : Votre code fonctionnera partout
3. **Stabilité** : Pas de bugs de compilateurs ou de fonctionnalités manquantes
4. **Marché du travail** : La plupart des entreprises utilisent C11 ou antérieur
5. **Apprentissage solide** : Maîtrisez les bases avant les nouveautés

**Ensuite**, explorez C23 pour :
- Comprendre l'évolution du langage
- Utiliser les fonctionnalités pratiques (attributs, `typeof`)
- Être prêt pour l'avenir

---

## 🛠️ Compiler avec C23 sur Ubuntu

### Installation d'un compilateur récent

**GCC 14+ ou Clang 18+** supportent C23 partiellement.

**Ubuntu 24.04+ (GCC 13/14 par défaut)** :
```bash
# Vérifier la version GCC
gcc --version

# Si nécessaire, installer GCC 14
sudo apt update  
sudo apt install gcc-14 g++-14  

# Définir GCC 14 comme défaut
sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-14 100
```

**Compiler avec C23** :
```bash
# Avec GCC
gcc -std=c23 -Wall -Wextra program.c -o program

# Avec Clang
clang -std=c23 -Wall -Wextra program.c -o program
```

### Exemple de programme C23

```c
// test_c23.c
#include <stdio.h>

// Attributs C23
[[nodiscard]] int compute(int x) {
    return x * 2;
}

int main(void) {
    // Constante binaire
    int flags = 0b1010;

    // typeof
    typeof(flags) result = compute(flags);

    // nullptr
    int *ptr = nullptr;

    printf("Result: %d\n", result);
    printf("Pointer is null: %s\n", ptr == nullptr ? "yes" : "no");

    return 0;
}
```

**Compilation** :
```bash
gcc -std=c23 -Wall -Wextra test_c23.c -o test_c23
./test_c23
```

**Sortie** :
```
Result: 20  
Pointer is null: yes  
```

---

## 📈 L'évolution prévisible (2025-2030)

### Horizon 2026-2027
- **Support complet de C23** dans GCC et Clang
- **Adoption progressive** dans les projets open-source majeurs
- **Mise à jour des bibliothèques** pour utiliser les nouvelles fonctionnalités

### Horizon 2028-2030
- **C23 devient le standard** de facto pour les nouveaux projets
- **Migration progressive** des projets existants
- **Documentation et ressources** largement disponibles

### Ce qui ne changera pas
- **C99 et C11 resteront utilisés** pendant des décennies (code legacy)
- **La rétrocompatibilité** sera maintenue
- **Les concepts fondamentaux** restent les mêmes (pointeurs, mémoire, etc.)

---

## 🎓 Ce que vous devez retenir

### Les points essentiels

1. **C23 est la nouvelle norme** (ISO publiée en 2024), mais son adoption est progressive en 2025
2. **C11 reste la référence** pour la production et l'apprentissage
3. **Les nouveautés principales** : attributs `[[...]]`, `nullptr`, `typeof`, binaires `0b`, `#embed`
4. **Support partiel** par GCC 14+ et Clang 18+ en 2025
5. **Pour débuter** : commencez par C11, puis explorez C23

### Stratégie recommandée

**Phase 1 (maintenant)** : Apprenez C11
- Maîtrisez les bases solides
- Utilisez la documentation abondante
- Code compatible partout

**Phase 2 (après maîtrise C11)** : Découvrez C23
- Expérimentez les nouvelles fonctionnalités
- Comprenez l'évolution du langage
- Préparez-vous pour l'avenir

**Phase 3 (2026+)** : Adoptez C23 progressivement
- Sur les nouveaux projets
- Quand le support compilateur sera complet
- Quand l'écosystème sera prêt

### Message pour les débutants

**Ne vous stressez pas avec C23 !**

- Ce tutoriel utilisera principalement **C11** (avec C17), la version stable et universellement supportée
- Nous mentionnerons les fonctionnalités C23 quand elles sont pertinentes
- Vous pourrez toujours revenir à cette section plus tard

**Focus sur les fondamentaux** : Pointeurs, allocation mémoire, structures, fichiers, processus... Ces concepts sont **identiques en C89, C99, C11 et C23** !

---

## 📖 Pour aller plus loin

### Documentation officielle

- [ISO C23 Standard (payant)](https://www.iso.org/standard/82075.html)
- [Draft C23 gratuit (N3220)](https://www.open-std.org/jtc1/sc22/wg14/www/docs/n3220.pdf)
- [GCC C23 Status](https://gcc.gnu.org/projects/c23-status.html)
- [Clang C23 Status](https://clang.llvm.org/c_status.html)

### Articles et ressources

- *ModernesCpp.com* - Blog sur C moderne
- *Embedded Artistry* - C23 pour l'embarqué
- *LWN.net* - Articles Linux/C

### Communautés

- r/C_Programming (Reddit)
- Stack Overflow - Tag [c] [c23]
- comp.lang.c (Usenet/Google Groups)

---

## 🔜 Prochaines sections

Maintenant que vous connaissez C23 et son état d'adoption, explorons :

- **1.3.2** : Vérification du support compilateur sur votre système
- **1.3.3** : Pourquoi C11 reste la base recommandée en production
- **1.4** : C vs C++ vs Rust - Quand choisir C ?

**En route vers la maîtrise du C moderne !** 🚀

⏭️ [Vérification du support compilateur](/01-ecosysteme-c-linux/03.2-verification-support-compilateur.md)
