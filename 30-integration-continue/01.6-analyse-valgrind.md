...
**Valgrind** est une suite d'outils d'analyse dynamique considérée comme le "gold standard" pour détecter les problèmes de mémoire et de performance dans les programmes C. Contrairement aux sanitizers qui nécessitent une recompilation, Valgrind **analyse directement votre binaire** sans modifier le code source.

Dans un pipeline CI/CD, l'automatisation de Valgrind permet de détecter avec une **précision maximale** les fuites mémoire, corruptions et problèmes de concurrence avant qu'ils n'atteignent la production.
...
Valgrind est un **framework d'instrumentation** qui exécute votre programme dans un environnement virtualisé pour surveiller chaque accès mémoire, allocation et opération système. C'est comme avoir un microscope ultra-puissant pour observer votre programme en action.

### Les outils Valgrind

Valgrind est composé de plusieurs outils spécialisés :

#### 1. Memcheck (Le plus utilisé) 🔴

**Détecte :**
- Fuites mémoire (memory leaks)
- Utilisation de mémoire non initialisée
- Accès hors limites (buffer overflows)
- Use-after-free
- Double free
...
**Analyse :**
- Utilisation de la mémoire heap au fil du temps
- Profiling de consommation mémoire

**Utilisation :** Optimisation mémoire
...
|---------|-----------|----------|
| **Vitesse** | ✅ 2-3x overhead | ❌ 20-50x overhead |
| **Précision** | ✅ Très bonne | ✅ Excellente |
| **Recompilation** | ❌ Nécessaire | ✅ Pas nécessaire |
| **Portabilité** | ✅ Multiplateforme | ⚠️ Linux/macOS surtout |
| **CI quotidienne** | ✅ Recommandé | ⚠️ Trop lent |
| **Analyse approfondie** | ⚠️ Limitée | ✅ Très complète |
...
```yaml
- name: Vérifier la version de Valgrind
...
- `--tool=memcheck` : Utilise l'outil Memcheck (par défaut)
- `--leak-check=full` : Rapport détaillé des fuites
...
## Exemple : Détecter une fuite mémoire
...
**Note :** L'option `-g` est **essentielle** pour avoir des rapports lisibles avec numéros de ligne.
...
## Interpréter un rapport Memcheck
...
**Signification :** Mémoire allouée sans aucune référence restante. **Vraie fuite à corriger.**
...
**Signification :** Mémoire accessible uniquement via de la mémoire "definitely lost". Corriger la fuite principale corrigera celle-ci.
...
**Signification :** Pointeurs internes (milieu d'un bloc). Peut être intentionnel ou une fuite. À investiguer.
...
**Signification :** Mémoire accessible au moment de la sortie. Pas vraiment une fuite (sera libérée par l'OS), mais une bonne pratique de faire `free()`.
...
- **in use at exit** : Mémoire non libérée à la fin
- **total heap usage** : 6 allocations, 1 seul free
- **bytes allocated** : Total alloué (inclut allocations internes)

---

## Workflow de base avec Valgrind

Voici un workflow minimal pour exécuter Valgrind en CI :
...
**Option importante :** `--error-exitcode=1` fait échouer le workflow si Valgrind détecte des erreurs.

---

## Valgrind avec CTest

CTest intègre nativement Valgrind. Voici comment l'utiliser :
...
jobs:
  valgrind-tests:
    ...
    - name: Afficher les résultats Valgrind
      if: always()
      run: |
...
## Options Valgrind avancées

### Génération de rapport XML

Pour un parsing automatique des résultats :
...
int main(void) {
    ...
    *ptr = 42;
    ...
    // Bug : utilisation après libération
    *ptr = 99;
    printf("New value: %d\n", *ptr);
...
2. Où l'erreur se produit : ligne 11
3. Où la mémoire a été libérée : ligne 9
4. Où elle a été allouée : ligne 5
...
int main(void) {
    ...
    strcpy(buffer, "This is a very long string");  // Dépasse 10 !
    free(buffer);
    return 0;
}
```
...
## Workflow complet avec options avancées

```yaml
name: Analyse Valgrind Complète
...
jobs:
  valgrind-full-analysis:
    ...
    - name: Lancer Memcheck (fuites mémoire)
    ...
    - name: Afficher le résumé Memcheck
      if: always()
      run: |
        echo "=== Résumé Valgrind Memcheck ==="
        if [ -f valgrind-reports/memcheck.log ]; then
          grep "LEAK SUMMARY" -A 5 valgrind-reports/memcheck.log || echo "Pas de fuites détectées"
        fi

    - name: Vérifier les erreurs
      run: |
        if grep -q "ERROR SUMMARY: [1-9]" valgrind-reports/memcheck.log; then
          echo "❌ Erreurs détectées par Valgrind"
          exit 1
        else
...
- `schedule: cron:` : Exécution automatique nocturne (Valgrind est lent)
- `timeout-minutes: 60` : Limite le temps d'exécution
- `continue-on-error: true` : Continue même si Valgrind trouve des erreurs
- Upload des rapports pour analyse ultérieure

---

## Helgrind : Détecter les race conditions
...
**Utile pour :** Optimiser les structures de données pour une meilleure localité du cache.
...
Vous pouvez aussi utiliser **KCachegrind** (en local, pas en CI) pour visualiser les résultats graphiquement.

---

## Massif : Profiling de mémoire heap

Pour analyser l'évolution de la consommation mémoire :

```yaml
- name: Profiling mémoire avec Massif
  run: |
...
**Utile pour :** Détecter les pics de consommation mémoire et optimiser.
...
on:
  schedule:
    - cron: '0 3 * * 0'  # Dimanche à 3h (analyse hebdomadaire)
...
jobs:
  valgrind-analysis:
    ...
    - name: Exécuter ${{ matrix.tool.name }}
      run: |
...
## Optimisation : Valgrind seulement si nécessaire
...
jobs:
  ...
  # Valgrind seulement sur main ou si demandé
  valgrind-analysis:
    runs-on: ubuntu-latest
...
**Option `--gen-suppressions=all`** : Génère automatiquement les suppressions pour vous.

---

## Interpréter "ERROR SUMMARY"

À la fin de chaque rapport Valgrind :
...
**Signification :**
- **5 errors** : Nombre total d'erreurs détectées
- **3 contexts** : Nombre de sources différentes d'erreurs
- **suppressed: 0** : Aucune erreur supprimée par les fichiers .supp
...
Sans `-g`, Valgrind ne peut pas afficher les numéros de ligne.
...
Fait échouer le script CI si des erreurs sont détectées.
...
Ignore "still reachable" qui n'est généralement pas problématique.

### 5. Tester avec des données réalistes

Valgrind détecte les erreurs **pendant l'exécution**. Assurez-vous que vos tests couvrent tous les chemins de code.

### 6. Combiner avec les sanitizers

**Stratégie recommandée :**

| Fréquence | Outil | Utilisation |
|-----------|-------|-------------|
...
**Solutions :**
1. Exécuter en nocturne ou hebdomadaire
2. Limiter aux tests critiques
3. Utiliser `--leak-check=no` si seuls les accès invalides comptent

### Trop de faux positifs

**Solution :** Créer un fichier de suppressions (voir section précédente)
...
Voici un workflow complet et optimisé :
...
jobs:
  valgrind-memcheck:
    ...
    - name: Exécuter Valgrind Memcheck
      id: valgrind
      continue-on-error: true
      run: |
        valgrind --tool=memcheck \
    ...
    - name: Vérifier les erreurs critiques
      run: |
        ...
        if grep -q "ERROR SUMMARY: [1-9]" valgrind-memcheck.log; then
          echo "❌ Erreurs mémoire détectées !"
          exit 1
        fi

        echo "✅ Aucune fuite ou erreur détectée"
      ...
      with:
        script: |
          github.rest.issues.create({
            ...
            title: '⚠️ Valgrind a détecté des problèmes mémoire',
            body: 'Le workflow Valgrind hebdomadaire a échoué. Consultez les artifacts pour les détails.',
...
**Valgrind : L'analyse la plus précise**

**Outils principaux :**
- **Memcheck** : Fuites et erreurs mémoire (le plus important)
- **Helgrind** : Race conditions (multi-thread)
- **Cachegrind** : Profiling cache
- **Callgrind** : Profiling appels de fonction
- **Massif** : Profiling consommation mémoire
...
- **Hebdomadaire** : Valgrind (précis mais lent)

---

## Prochaines étapes

Maintenant que vous avez automatisé Valgrind, vous pouvez :

- Mesurer la **couverture de code** avec gcov/lcov
- Configurer l'**analyse statique** avec clang-tidy/cppcheck
- Créer des **rapports de tests** pour la PR
...
- ✅ Valgrind est le "gold standard" pour la détection de bugs mémoire
- ✅ Memcheck est l'outil principal (fuites, use-after-free, buffer overflow)
- ✅ Overhead de 20-50x : utiliser en analyse nocturne/hebdomadaire
- ✅ Compiler avec `-g -O0` pour des rapports lisibles
- ✅ `--error-exitcode=1` pour faire échouer la CI si erreurs
- ✅ CTest intègre Valgrind avec `ctest -T memcheck`
- ✅ Combiner avec sanitizers : ASan quotidien, Valgrind hebdomadaire
- ✅ Objectif : `ERROR SUMMARY: 0 errors` et `definitely lost: 0 bytes`

⏭️ [Code coverage et Codecov](/30-integration-continue/01.7-code-coverage-codecov.md)
