...
# 34.2.3 Agrégation et statistiques

## Introduction

Après avoir appris à lire efficacement de gros fichiers de logs (section 34.2.1) et à extraire des informations avec des expressions régulières (section 34.2.2), nous pouvons maintenant agréger ces données et produire des statistiques significatives.

Un parser de logs professionnel ne se contente pas de lire : il analyse, compte, trie et présente des statistiques utiles pour comprendre ce qui se passe dans un système.

**Exemples de questions auxquelles répondre :**
- Combien de requêtes par code HTTP (200, 404, 500, etc.) ?
- Quelles sont les 10 IPs les plus actives ?
- Quel est le temps de réponse moyen/médian/95e percentile ?
- Quelles URLs sont les plus appelées ?
- Quelle est la distribution des erreurs dans le temps ?

---

## 1. Structures de données pour l'agrégation
...
Pour compter des chaînes variables (IPs, URLs), on utilise une table de hachage :
...
Pour obtenir les "top N", il faut trier les résultats :
...
typedef struct {
    ...
    // Temps de réponse (pour calculs statistiques)
...
int parser_ligne_apache(const char *ligne, LigneLog *entry) {
    ...
    // Pattern Apache avec temps de réponse
    ...
    for (const char *p = data; p < fin; p++) {
        if (*p == '\n') {
            ...
            if (nb_lignes % 100000 == 0) {
                printf("\rLignes traitées: %lld", nb_lignes);
                fflush(stdout);
            }
        }
    }

    printf("\rLignes traitées: %lld (erreurs: %lld)\n", nb_lignes, nb_erreurs);
...
### 3.4 Statistiques complètes sur les temps de réponse
...
// Convertir HashMap en liste triée
...
Pour analyser la distribution (par exemple, des temps de réponse par tranches) :
...
void afficher_distribution(const DistributionTemps *dist, int total) {
    ...
    const char *labels[] = {
        ...
        "500-600ms", "600-700ms", "700-800ms", "800-900ms", "≥900ms"
    };
...
### 5.1 Fonction de génération de rapport
...
void generer_rapport(StatistiquesLog *stats) {
    ...
    printf("\n📊 VUE D'ENSEMBLE\n");
    printf("─────────────────────────────────────────────────────────\n");
    printf("  Requêtes totales:      %lld\n", stats->total_requetes);
    printf("  Données transférées:   %.2f GB\n",
           stats->total_octets / (1024.0 * 1024.0 * 1024.0));
    printf("  Octets/requête (moy):  %.0f octets\n",
    ...
    // 3. Top IPs
    afficher_top_n(stats->ips, "🔝 TOP 10 IPs", 10);

    // 4. Top URLs
    afficher_top_n(stats->urls, "🔝 TOP 10 URLs", 10);
    ...
    // 6. Statistiques temps de réponse
...
void exporter_csv(HashMap *map, const char *fichier, const char *colonne1, const char *colonne2) {
    ...
    printf("Données exportées dans %s\n", fichier);
}
```

---

## 7. Agrégation par période temporelle

### 7.1 Compteurs par heure

```c
typedef struct {
    int compteurs[24];  // Une entrée par heure (0-23)
} StatistiquesHoraires;

int extraire_heure(const char *timestamp) {
    // Format: "15/Jan/2025:14:30:45"
    // Extraire l'heure (caractères après le ':')
...
### 7.2 Agrégation par jour/semaine/mois
...
void extraire_date(const char *timestamp, char *date_sortie) {
    // Format entrée: "15/Jan/2025:14:30:45"
...
## 8. Analyse en temps réel (streaming)

### 8.1 Fenêtre glissante

Pour analyser les N dernières requêtes :
...
### 8.2 Détection d'anomalies simples
...
void verifier_anomalie(DetecteurAnomalies *detecteur, const LigneLog *entry) {
    ...
    if (entry->code_http >= 500) {
        ...
        if (detecteur->erreurs_consecutives >= detecteur->seuil_erreurs_consecutives) {
            printf("⚠️  ALERTE: %d erreurs 5xx consécutives!\n",
                   detecteur->erreurs_consecutives);
        }
    } else {
    ...
    if (temps_ms > detecteur->seuil_temps_reponse) {
        printf("⚠️  ALERTE: Temps de réponse élevé: %.2f ms pour %s\n",
               temps_ms, entry->url);
    }
}
```
...
Pour de très gros fichiers (>10 GB), traiter 1 ligne sur N :
...
### 9.2 Traitement parallèle

Pour exploiter plusieurs cœurs CPU :
...
int main(int argc, char *argv[]) {
    ...
    printf("Analyse terminée en %.2f secondes\n", temps);
...
### 11.1 Structures de données essentielles

| Structure | Usage | Performance |
|-----------|-------|-------------|
| HashMap | Compteurs par clé unique | O(1) insertion/lookup |
| Tableau dynamique | Stockage pour tri | O(n log n) tri |
| Buffer circulaire | Fenêtre glissante | O(1) insertion |
| Tableau de compteurs | Distribution par tranches | O(1) par catégorie |

### 11.2 Statistiques clés à calculer

✅ **Basiques :**
- Total de requêtes
- Total d'octets transférés
- Distribution des codes HTTP

✅ **Avancées :**
- Moyenne, médiane, percentiles (p95, p99)
- Écart-type
- Min/max

✅ **Classements :**
- Top N IPs/URLs/User-Agents
- Distribution temporelle (horaire, journalière)
...
2. **Réutiliser les regex** compilées
3. **HashMap efficace** pour les compteurs
4. **Échantillonnage** pour fichiers >10 GB
5. **Traitement parallèle** pour exploiter tous les cœurs

---

## 12. Conclusion

L'agrégation et les statistiques transforment des milliers de lignes de logs en informations exploitables. Les points clés :

1. **Structures de données adaptées** : HashMap pour compter, tableaux pour trier
2. **Statistiques pertinentes** : Au-delà de la moyenne, calculer médiane et percentiles
3. **Visualisation claire** : Rapports lisibles avec graphiques ASCII
4. **Exports standards** : JSON/CSV pour intégration avec d'autres outils
5. **Performance** : mmap, parallélisation pour gros volumes

Un bon parser de logs combine :
- Lecture efficace (section 34.2.1)
- Extraction précise avec regex (section 34.2.2)
- Agrégation intelligente et statistiques (section 34.2.3)

Avec ces trois compétences, vous pouvez créer des outils d'analyse de logs professionnels capables de traiter des téraoctets de données et de fournir des insights précieux sur le comportement de vos systèmes.

⏭️ [Agent de monitoring système](/34-etudes-cas-devops/03-agent-monitoring.md)
