...
# 34.1.3 Output formaté

## Introduction

Un outil CLI professionnel ne se contente pas de fonctionner correctement : il doit aussi présenter ses résultats de manière claire, lisible et esthétique. Un output bien formaté améliore l'expérience utilisateur et rend votre outil plus facile à comprendre et à utiliser.

Dans cette section, nous explorerons les différentes techniques pour créer des sorties formatées professionnelles en C.

---

## 1. Les bases de printf()

### 1.1 Spécificateurs de format essentiels
...
### 2.3 Détection du support des couleurs
...
int terminal_supporte_couleurs() {
    ...
    // Vérifier si NO_COLOR est défini
...
int main() {
    print_couleur("\033[32m", "Texte vert (si supporté)");
    return 0;
}
```
...
### 3.1 Niveaux de log standardisés
...
**Sortie :**
```
[2025-01-15 14:30:45] DEBUG Mode debug activé
[2025-01-15 14:30:45] INFO  Démarrage du programme
[2025-01-15 14:30:45] OK    Connexion établie avec succès
[2025-01-15 14:30:45] WARN  Fichier de configuration manquant, utilisation des valeurs par défaut
...
int main() {
    print_status(1, "Compilation réussie");
    print_status(0, "Tests échoués");
...
### 4.2 Tableau avec couleurs alternées
...
### 4.3 Tableau avec caractères Unicode
...
void afficher_tableau_unicode() {
    ...
    wprintf(L"╔════════════════════╦═══════╦════════════╗\n");
    wprintf(L"║ %-18s ║ %-5s ║ %-10s ║\n", "Produit", "Qté", "Prix");
    wprintf(L"╠════════════════════╬═══════╬════════════╣\n");
    wprintf(L"║ %-18s ║ %5d ║ %10.2f ║\n", "Clavier", 5, 49.99);
    wprintf(L"║ %-18s ║ %5d ║ %10.2f ║\n", "Souris", 10, 29.99);
    wprintf(L"║ %-18s ║ %5d ║ %10.2f ║\n", "Écran", 2, 299.99);
    wprintf(L"╠════════════════════╩═══════╬════════════╣\n");
    wprintf(L"║ Total                      ║ %10.2f ║\n", 899.93);
    wprintf(L"╚════════════════════════════╩════════════╝\n");
}
```
...
int main() {
    afficher_spinner("Chargement des données...", 30);
...
void afficher_dates() {
    ...
    // Format français
    strftime(buffer, sizeof(buffer), "%d/%m/%Y à %H:%M:%S", t);
    printf("FR:           %s\n", buffer);
...
## 7. Formats de sortie structurés
...
void afficher_menu() {
    ...
    printf("╠════════════════════════════════╣\n");
    printf("║ 1. Nouveau projet              ║\n");
    printf("║ 2. Ouvrir projet               ║\n");
    printf("║ 3. Paramètres                  ║\n");
    printf("║ 4. Aide                        ║\n");
    printf("║ 0. Quitter                     ║\n");
    printf("╚════════════════════════════════╝\n");
...
### 8.2 Liste sélectionnable avec flèches
...
void afficher_liste(const char *items[], int count, int selection) {
    ...
    printf("Utilisez ↑↓ pour sélectionner, Entrée pour valider\n\n");
...
int main() {
    const char *items[] = {
        "Option 1: Démarrer le service",
        "Option 2: Arrêter le service",
    ...
    while (1) {
        ...
        if (c == '\033') {  // Séquence d'échappement
            getchar();  // [
            char arrow = getchar();

            if (arrow == 'A' && selection > 0) {  // Flèche haut
                selection--;
            } else if (arrow == 'B' && selection < count - 1) {  // Flèche bas
                selection++;
            }
        } else if (c == '\n') {  // Entrée
            break;
...
### 9.1 Graphique à barres horizontal
...
void afficher_resultat(int code, const char *message, FormatSortie format) {
    if (format == FORMAT_HUMAN) {
        const char *symbole = code == 0 ? "\033[32m✓\033[0m" : "\033[31m✗\033[0m";
        printf("%s %s\n", symbole, message);
    } else {
        printf("%d %s\n", code, message);
    }
...
int main() {
    ...
    afficher_resultat(0, "Opération réussie", format);
    afficher_resultat(1, "Erreur détectée", format);

    return 0;
}
```

### 10.2 Niveaux de verbosité
...
void print_quiet(const char *msg) {
    // Toujours affiché (erreurs critiques)
    fprintf(stderr, "%s\n", msg);
}
...
int main(int argc, char *argv[]) {
    ...
    print_debug("Mode debug activé");
    print_verbose("Chargement de la configuration");
    print_normal("Démarrage du programme");
...
- ✅ **Lisibilité** : Utiliser des tableaux, alignements, couleurs appropriées
- ✅ **Consistance** : Format uniforme dans tout le programme
- ✅ **Adaptabilité** : Détecter si stdout est un terminal
- ✅ **Progression** : Barres de progression pour opérations longues
- ✅ **Niveaux de log** : DEBUG, INFO, WARNING, ERROR
- ✅ **Formats structurés** : JSON, CSV pour l'interopérabilité
- ✅ **Verbosité réglable** : Options -q, -v, -vv
- ✅ **Codes de couleur** : Respecter NO_COLOR si défini
- ✅ **Localisation** : Support UTF-8 pour Unicode
- ✅ **Documentation** : Messages d'aide clairs
...
| JSON | Génération manuelle ou bibliothèque |
| Logging | Niveaux + timestamps |
| Formats de données | snprintf() avec formatage |

---

## Conclusion

Un output bien formaté transforme un outil fonctionnel en outil professionnel. Les points clés à retenir :

1. **printf()** est votre ami : maîtrisez ses spécificateurs de format
2. **Les couleurs** améliorent la lisibilité (mais restent optionnelles)
3. **Les tableaux** structurent l'information visuellement
4. **Les barres de progression** informent l'utilisateur
5. **Les formats structurés** (JSON, CSV) permettent l'automatisation
6. **L'adaptabilité** rend votre outil utilisable dans tous contextes

Un bon output ne se contente pas d'afficher des données : il guide l'utilisateur, l'informe clairement et rend l'interaction agréable. C'est la différence entre un script d'amateur et un outil professionnel.

⏭️ [Parser de logs haute performance](/34-etudes-cas-devops/02-parser-logs.md)
