...
# 34.1.2 Interaction avec le système

## Introduction

Un outil CLI système efficace doit pouvoir interagir avec le système d'exploitation Linux : lire des fichiers, exécuter des commandes, obtenir des informations système, gérer les processus, etc. Cette section vous guide à travers les différentes façons d'interagir avec le système en C.

---

## 1. Appels système vs Bibliothèque standard

### 1.1 Différences fondamentales

Il existe deux approches principales pour interagir avec le système :

**Bibliothèque standard C (stdio.h)** :
- Fonctions portables (fonctionne sur Windows, Linux, etc.)
- I/O bufferisé (plus performant pour petites opérations)
- API de haut niveau : `fopen()`, `fprintf()`, `fgets()`

**Appels système POSIX (unistd.h, fcntl.h)** :
- Spécifique à UNIX/Linux
- I/O non bufferisé (accès direct au noyau)
- API de bas niveau : `open()`, `read()`, `write()`
- Plus de contrôle, mais plus complexe

### 1.2 Tableau comparatif

| Opération | Bibliothèque C | Appel système |
|-----------|----------------|---------------|
| Ouvrir un fichier | `fopen()` | `open()` |
| Lire | `fread()`, `fgets()` | `read()` |
| Écrire | `fwrite()`, `fprintf()` | `write()` |
| Fermer | `fclose()` | `close()` |
| Type de descripteur | `FILE*` | `int` (file descriptor) |

### 1.3 Quand utiliser quoi ?

**Utilisez la bibliothèque standard C quand :**
- Vous voulez de la portabilité
...
- Vous devez gérer les permissions, les verrous, etc.

---

## 2. Gestion des fichiers

### 2.1 Avec la bibliothèque standard
...
- `O_TRUNC` : Tronquer à 0 octet
- `O_APPEND` : Ajouter à la fin
- `O_EXCL` : Échec si fichier existe (avec O_CREAT)

### 2.3 Vérifier l'existence d'un fichier
...
### 3.1 stat() : Informations complètes
...
- `st_atime` : Dernière lecture du fichier
- `st_mtime` : Dernière modification du contenu
- `st_ctime` : Dernier changement des métadonnées (permissions, propriétaire)

---

## 4. Manipulation de répertoires

### 4.1 Lister le contenu d'un répertoire
...
### 4.2 Créer et supprimer des répertoires
...
// Note: rmdir() ne fonctionne que sur des répertoires VIDES
```

### 4.3 Parcours récursif de répertoires
...
**⚠️ Attention avec system() :**
- Lance un shell (`/bin/sh`)
- Risque de sécurité si la commande contient des entrées utilisateur
...
void executer_et_lire(const char *commande) {
    ...
    if (status != 0) {
        fprintf(stderr, "Commande échouée avec code: %d\n", status);
    }
}
...
### 5.3 fork() + exec() : Contrôle total (avancé)
...
- Pas de shell intermédiaire (plus sûr)
- Contrôle total sur stdin/stdout/stderr
...
### 6.3 Accéder à toutes les variables (environ)

```c
#include <stdio.h>

// Variable globale définie par le système
extern char **environ;
...
void afficher_info_utilisateur() {
    // UID et GID
    uid_t uid = getuid();      // UID réel
    uid_t euid = geteuid();    // UID effectif
    gid_t gid = getgid();      // GID réel
...
### 7.4 Limites et ressources du système
...
## 9. Lecture du système de fichiers /proc

Le répertoire `/proc` sous Linux contient des informations sur les processus et le système.
...
### 9.2 Lire les informations mémoire
...
## 10. Gestion des permissions et sécurité
...
int changer_permissions(const char *fichier, mode_t mode) {
    ...
    printf("Permissions changées: %s\n", fichier);
    return 0;
}

// Exemples
int main() {
    // Rendre un fichier exécutable (rwxr-xr-x)
    changer_permissions("script.sh", 0755);

    // Rendre un fichier privé (rw-------)
...
int changer_proprietaire(const char *fichier, const char *utilisateur, const char *groupe) {
    ...
    printf("Propriétaire changé: %s -> %s:%s\n", fichier, utilisateur, groupe);
    return 0;
}
...
## 11. Exemple complet : Outil d'information système
...
if (fd == -1) {  
    perror("Erreur ouverture");  // Affiche l'erreur système
    return -1;
}

// Utiliser errno pour des informations détaillées
#include <errno.h>
if (mkdir("test", 0755) == -1) {
    if (errno == EEXIST) {
        printf("Le répertoire existe déjà\n");
...
// ✅ SÉCURISÉ - Utiliser unlink directement
if (unlink(nom_fichier) == -1) {
    perror("unlink");
}

// ✅ SÉCURISÉ - Valider les entrées utilisateur
...
| Opération | Fonction | En-tête |
|-----------|----------|---------|
| Ouvrir fichier | `open()` | `<fcntl.h>` |
| Lire fichier | `read()` | `<unistd.h>` |
| Écrire fichier | `write()` | `<unistd.h>` |
| Info fichier | `stat()` | `<sys/stat.h>` |
| Lister répertoire | `opendir()`, `readdir()` | `<dirent.h>` |
| Exécuter commande | `system()`, `popen()` | `<stdlib.h>` |
| Variables env | `getenv()`, `setenv()` | `<stdlib.h>` |
| Info système | `uname()` | `<sys/utsname.h>` |
| Signaux | `signal()`, `sigaction()` | `<signal.h>` |
| Processus | `fork()`, `exec()` | `<unistd.h>` |

### Points clés à retenir

- ✅ **Toujours vérifier les valeurs de retour** : Les appels système peuvent échouer
- ✅ **Utiliser perror()** : Pour afficher des messages d'erreur clairs
- ✅ **Nettoyer les ressources** : Fermer les fichiers, libérer la mémoire
- ✅ **Valider les entrées** : Ne jamais faire confiance aux données utilisateur
- ✅ **Privilégier les appels système directs** : Plutôt que `system()` quand possible
- ✅ **Consulter les man pages** : `man 2 open`, `man 3 fopen`, etc.

---

## Conclusion

L'interaction avec le système Linux en C offre un contrôle puissant et précis. En maîtrisant ces concepts, vous pouvez créer des outils CLI professionnels capables de :

- Manipuler efficacement les fichiers et répertoires
- Exécuter et contrôler d'autres programmes
- Obtenir des informations système détaillées
- Gérer les signaux et les interruptions
- Créer des outils de monitoring et d'administration

La clé est de toujours privilégier la robustesse et la sécurité, en vérifiant systématiquement les erreurs et en validant les entrées utilisateur.

⏭️ [Output formaté](/34-etudes-cas-devops/01.3-output-formate.md)
