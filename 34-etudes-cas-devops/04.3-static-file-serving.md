...
Après avoir construit une architecture event-driven (34.4.1) et un parser HTTP (34.4.2), nous allons maintenant créer la fonctionnalité essentielle d'un serveur web : **servir des fichiers statiques**.
...
- Est envoyé **tel quel** au client (sans modification)
...
- `font.woff2` - Police de caractères

### Fichiers Statiques vs Dynamiques

| Type | Exemple | Génération |
|------|---------|------------|
| **Statique** | `about.html` | Fichier fixe sur disque |
| **Dynamique** | `/api/users` | Généré par code (PHP, Python, etc.) |
...
2. Serveur:
   ...
   ├─> Extraire l'URI: /images/logo.png
   ├─> Convertir en chemin disque: /var/www/html/images/logo.png
   ├─> Vérifier la sécurité (pas de ../.. etc.)
   ├─> Vérifier que le fichier existe
   ├─> Déterminer le type MIME: image/png
   ├─> Lire le fichier
   ├─> Générer réponse HTTP avec headers appropriés
   └─> Envoyer au client

3. Client reçoit l'image
```
...
- `DOCUMENT_ROOT` : Répertoire racine contenant les fichiers à servir
- `DEFAULT_FILE` : Fichier servi par défaut pour les répertoires
- `MAX_PATH_LEN` : Longueur maximale d'un chemin

---

## Conversion URI vers Chemin Système
...
Le serveur doit le convertir en **chemin système** :
```
...
if (uri_to_path("/docs/guide.html", "/var/www/html",  
                path, sizeof(path)) == 0) {
    printf("Chemin: %s\n", path);
    // Résultat: /var/www/html/docs/guide.html
}
```

### Gestion des répertoires

Si l'URI se termine par `/`, servir le fichier index par défaut :
...
// Exemples
// /docs/        → /var/www/html/docs/index.html
// /docs/guide/  → /var/www/html/docs/guide/index.html
...
**Attaque classique :** Un attaquant essaie d'accéder à des fichiers hors du document root.
...
Si non protégé, le serveur pourrait exposer :
```
/var/www/html/../../../etc/passwd
= /etc/passwd  ❌ DANGER!
```

### Solution : Canonicalisation et vérification
...
int is_safe_path(const char *uri, const char *document_root,
    ...
    // 1. Résoudre le document root en chemin absolu
    if (realpath(document_root, resolved_root) == NULL) {
        return -1;
    }

    // 2. Construire le chemin complet demandé
    snprintf(full_path, sizeof(full_path), "%s%s", document_root, uri);

    // 3. Résoudre le chemin (supprime .., ., liens symboliques)
    if (realpath(full_path, resolved_path) == NULL) {
        // Le fichier n'existe peut-être pas encore, essayer sans résolution
        // ou retourner une erreur
        return -1;
    }

    // 4. Vérifier que le chemin résolu est bien dans document_root
...
**Note importante :** `realpath()` échoue si le fichier n'existe pas. Pour certaines applications (upload), vous devrez gérer ce cas différemment.
...
file_status_t check_file(const char *path) {
    ...
    // stat() récupère les informations sur le fichier
    if (stat(path, &st) != 0) {
    ...
    // Ni fichier régulier ni répertoire (lien symbolique, device, etc.)
    return FILE_NO_ACCESS;
}
```
...
Si le chemin est un répertoire, essayer de servir `index.html` :
...
### Méthode 1 : Lecture complète en mémoire

**Simple mais limitée aux petits fichiers :**
...
**Inconvénients :**
- Consomme beaucoup de mémoire pour les gros fichiers
- Pas adapté aux fichiers de plusieurs Mo/Go

### Méthode 2 : Lecture par blocs

**Recommandée pour les gros fichiers :**
...
**Avantages :**
- Consommation mémoire constante (8 KB)
- Peut gérer des fichiers de n'importe quelle taille

**Inconvénients :**
- Plusieurs lectures/écritures (plus d'appels système)

### Méthode 3 : sendfile() - Zéro copie (Linux)
...
int send_file_zerocopy(int client_fd, const char *path,
    ...
    // Envoyer le fichier avec sendfile (zéro copie!)
    off_t offset = 0;
...
- **Zéro copie** : Les données vont directement du disque au socket
- Pas de passage par l'espace utilisateur
- Très performant (utilisé par nginx, Apache)

**Fonctionnement :**
```
Méthode traditionnelle:  
Disque → Kernel → Espace Utilisateur → Kernel → Socket  
        (DMA)      (copy)            (copy)    (DMA)

sendfile():  
Disque → Kernel → Socket  
        (DMA)    (DMA)
```
...
Rappel de la section 34.4.2, amélioration avec cache :
...
Pour améliorer les performances, les navigateurs et serveurs utilisent le cache HTTP.

### Headers de cache

```c
void add_cache_headers(char *response, size_t response_size,
                      int *offset, const struct stat *st) {
    char time_buf[128];
    struct tm tm;
    ...
    *offset += snprintf(response + *offset, response_size - *offset,
    ...
    // ETag (identifiant unique basé sur l'inode et mtime)
    *offset += snprintf(response + *offset, response_size - *offset,
                       "ETag: \"%lx-%lx\"\r\n",
...
### Répondre 304 Not Modified

Si le client a déjà le fichier en cache :
    ...
    if (if_modified != NULL) {
        ...
        if (strptime(if_modified, "%a, %d %b %Y %H:%M:%S GMT", &tm) != NULL) {
            ...
            if (st->st_mtime <= client_time) {
                return 0;  // Pas modifié
            }
        }
    }

    return 1;  // Modifié ou pas de cache
}
...
Voici un serveur complet intégrant tous les concepts :
...
// Table MIME (définie précédemment)
...
int is_safe_path(const char *uri, const char *doc_root,
    ...
    if (realpath(full_path, resolved_path) == NULL) {
        // Le fichier n'existe pas, mais vérifier quand même le chemin
        // On va accepter et laisser check_file() gérer l'erreur
...
int main(void) {
    ...
    printf("Serveur de fichiers statiques démarré\n");
    printf("Port: %d\n", PORT);
...
# Télécharger une image de test (ou créer une image simple)
...
### Log des requêtes (format Apache)
...
void print_stats(void) {
    printf("\n=== Statistiques ===\n");
    printf("Requêtes totales: %lu\n", stats.total_requests);
    printf("Bytes envoyés: %lu (%.2f MB)\n",
...
### 1. Cache de fichiers en mémoire

Pour les petits fichiers fréquemment demandés :
...
### 3. Range Requests (Téléchargement partiel)

Utile pour les vidéos :
...
- ✅ **Path Traversal** : Utiliser `realpath()` et vérifier le préfixe
- ✅ **Permissions** : Vérifier `access(path, R_OK)`
- ✅ **Taille des requêtes** : Limiter à MAX_REQUEST_SIZE
- ✅ **Timeout** : Ne pas bloquer indéfiniment sur un client
- ✅ **Symlinks** : `realpath()` les résout automatiquement
- ✅ **Hidden files** : Optionnel, refuser les fichiers commençant par `.`
- ✅ **Directory listing** : Ne pas exposer le contenu des répertoires
- ✅ **Error messages** : Ne pas révéler les chemins internes

### Refuser les fichiers cachés
...
```bash
# 1000 requêtes, 100 concurrentes
...
```bash
# 4 threads, 100 connexions, durée 30s
...
# Tenter d'accéder à un fichier caché
...
Servir des fichiers statiques en C nécessite :

1. **Conversion URI → Chemin** : Mapper les URLs vers le système de fichiers
2. **Sécurité** : Prévenir path traversal avec `realpath()`
3. **Vérifications** : Existence, permissions, type de fichier
4. **Lecture efficace** : `sendfile()` pour zéro copie
5. **Types MIME** : Détection automatique par extension
6. **Gestion des répertoires** : Servir `index.html` par défaut
7. **Erreurs** : 404, 403, 500 avec messages appropriés
8. **Cache HTTP** : Last-Modified, ETag, 304 Not Modified
9. **Logging** : Format Apache pour les requêtes
10. **Optimisations** : Cache mémoire, compression, range requests

**Votre serveur peut maintenant :**
- Servir n'importe quel type de fichier
- Gérer les répertoires et index files
- Protéger contre les attaques path traversal
- Optimiser avec sendfile() et cache
- Logger toutes les requêtes

**Prochaine étape :** Intégrer tout (event-driven + HTTP parsing + file serving) dans un serveur web complet et production-ready !
...
- **Virtual Hosts** : Servir plusieurs sites sur un même serveur
- **URL Rewriting** : Réécriture d'URLs (comme Apache mod_rewrite)
- **Directory Listing** : Générer automatiquement un index HTML
- **CGI/FastCGI** : Exécuter des scripts dynamiques
- **WebDAV** : Permettre l'upload de fichiers
- **HTTPS/TLS** : Chiffrement avec OpenSSL

### Serveurs Open Source à Étudier

- **lighttpd** : Serveur léger en C
- **nginx** : Architecture event-driven, code très optimisé
...
Vous avez maintenant toutes les connaissances pour créer un serveur web statique performant et sécurisé en C !

⏭️ [Debugging de code complexe](/35-debugging-code-complexe/README.md)
