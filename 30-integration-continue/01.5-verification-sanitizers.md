...
Les **sanitizers** (assainisseurs en français) sont des outils d'analyse dynamique intégrés aux compilateurs modernes (GCC et Clang) qui détectent automatiquement des bugs difficiles à trouver pendant l'exécution de votre programme. Ils constituent votre **première ligne de défense** contre les bugs mémoire, les comportements indéfinis et les problèmes de concurrence.

En intégrant les sanitizers dans votre pipeline CI/CD, vous détectez ces bugs **automatiquement** à chaque modification du code, avant qu'ils n'atteignent la production.

---

## Pourquoi les sanitizers sont essentiels ?

### Les bugs qu'ils détectent

Les sanitizers détectent des bugs **invisibles** qui ne font pas nécessairement crasher votre programme immédiatement, mais qui causent :

- **Corruptions de mémoire** : Accès hors limites, double free, use-after-free
- **Comportements indéfinis** : Division par zéro, débordements d'entiers, pointeurs NULL
- **Fuites mémoire** : Mémoire allouée mais jamais libérée
- **Race conditions** : Accès concurrents non synchronisés dans les programmes multi-thread

### Avantages en CI

1. **Détection immédiate** : Les bugs sont trouvés dès qu'ils sont introduits
2. **Rapports précis** : Stack trace exacte de l'erreur
3. **Faible coût** : Pas besoin d'outils externes comme Valgrind
4. **Performance acceptable** : Overhead de 2-3x (vs 20-50x pour Valgrind)
5. **Facilité d'utilisation** : Simple flag de compilation
...
**Détecte :**
- Buffer overflow (lecture/écriture hors limites)
- Use-after-free (accès à de la mémoire libérée)
- Use-after-return (accès à des variables locales après le retour de fonction)
- Double free
...
**Le plus utilisé et le plus important pour débuter.**
...
⚠️ **Incompatible avec ASan** : Ne peut pas être utilisé en même temps qu'AddressSanitizer.
...
**Flags recommandés supplémentaires :**
...
- `-O1` : Optimisation légère (équilibre entre performance et debugging)
...
- ✅ ASan + UBSan
- ✅ ASan + LSan (LSan est déjà dans ASan)
- ✅ UBSan + LSan
- ❌ ASan + TSan (incompatibles)
- ❌ LSan + TSan (incompatibles)

---

## Exemple : Détecter un buffer overflow
...
int main(void) {
    ...
    // Bug : accès hors limites !
    for (int i = 0; i <= 5; i++) {  // Devrait être i < 5
        printf("array[%d] = %d\n", i, array[i]);
    }
...
array[3] = 4
array[4] = 5
array[5] = 32767  # Valeur aléatoire, pas de crash !
```

Le programme ne crash pas, mais lit de la mémoire invalide.
...
**ASan détecte immédiatement le bug avec un rapport précis !**

---

## Interpréter un rapport ASan
...
2. **Opération** : `READ of size 4` = Lecture de 4 octets (un `int`)
3. **Localisation** : `main.c:7` = Ligne 7 du fichier main.c
4. **Stack trace** : Chemin d'appels jusqu'à l'erreur

### Types d'erreurs ASan courants

| Erreur | Signification |
|--------|---------------|
| `heap-buffer-overflow` | Accès hors limites sur le tas (malloc) |
| `stack-buffer-overflow` | Accès hors limites sur la pile (variables locales) |
| `heap-use-after-free` | Utilisation de mémoire libérée |
| `stack-use-after-return` | Utilisation de variable locale après retour de fonction |
| `double-free` | Libération de la même mémoire deux fois |
| `memory leak` | Mémoire allouée non libérée |

---

## Workflow de base avec sanitizers

Voici un workflow GitHub Actions minimal pour exécuter votre code avec sanitizers :

```yaml
name: Vérification avec Sanitizers
...
**Si un test détecte un bug, le workflow échouera avec le rapport ASan dans les logs.**

---

## Matrice de sanitizers

Pour tester avec différents sanitizers, utilisez une matrice :
...
**Ce workflow lance 4 jobs en parallèle, un pour chaque sanitizer.**

---

## Configuration CMake avec sanitizers

Pour intégrer les sanitizers proprement dans votre CMakeLists.txt :
...
if(ENABLE_UBSAN)
    message(STATUS "UndefinedBehaviorSanitizer activé")
    add_compile_options(-fsanitize=undefined)
    add_link_options(-fsanitize=undefined)
endif()

if(ENABLE_TSAN)
    message(STATUS "ThreadSanitizer activé")
...
## Workflow complet avec ASan et UBSan combinés
...
jobs:
  test-with-sanitizers:
    ...
    - name: Afficher résumé des erreurs
      if: failure()
      run: |
        echo "=== Des erreurs ont été détectées par les sanitizers ==="
        echo "Consultez les logs ci-dessus pour les détails"
```

**Variables d'environnement ASAN_OPTIONS :**

- `detect_leaks=1` : Active la détection de fuites mémoire
- `halt_on_error=0` : Continue l'exécution après une erreur (pour voir toutes les erreurs)
...
|--------|--------|-------------|
| `detect_leaks` | 0 ou 1 | Détecte les fuites mémoire |
| `halt_on_error` | 0 ou 1 | Arrête à la première erreur |
...
**Le rapport montre :**
1. Où la mémoire a été libérée (ligne 8)
2. Où elle a été allouée (ligne 5)
3. Où elle est utilisée après libération (ligne 10)
...
**Le rapport indique :**
- 4000 octets (10 allocations × 400 octets) non libérés
- Où les allocations ont eu lieu (ligne 4)
...
### Compilation et exécution avec TSan
...
**TSan détecte la race condition sur `shared_counter` !**
...
## Matrice complète : Sanitizers + Compilateurs

```yaml
name: CI Complète avec Sanitizers
  ...
  test:
    ...
    strategy:
      ...
      matrix:
        ...
        exclude:
          # TSan a de meilleurs résultats avec Clang
          - compiler: gcc
            sanitizer: thread
...
| Sanitizer | Overhead mémoire | Overhead CPU | Utilisation |
|-----------|------------------|--------------|-------------|
| ASan | 2-3x | 2x | Tests unitaires, CI |
| UBSan | Négligeable | 1.2x | Tests unitaires, CI |
| TSan | 5-10x | 5-15x | Tests concurrence |
| LSan | Faible | Négligeable | Fin d'exécution |
...
✅ **Si programme multi-thread :**
- ThreadSanitizer (TSan)

✅ **En développement local :**
- Tous les sanitizers pendant le développement

❌ **Jamais en production :**
- Les sanitizers ne doivent JAMAIS être activés en production

---

## Faux positifs et suppressions

Parfois, un sanitizer peut signaler un "bug" qui n'en est pas un (faux positif). Vous pouvez créer un fichier de suppression :
...
## Intégration avec compilation conditionnelle

Dans votre code, vous pouvez détecter si ASan est actif :
...
```c
  ...
  // Code spécifique pour ASan
  printf("Running with AddressSanitizer\n");
#endif
```

Utile pour désactiver certaines optimisations ou tests incompatibles.
...
### Générer un core dump lors d'une erreur
...
jobs:
  test-sanitizers:
    ...
    - name: Uploader les logs (si échec)
      if: failure()
...
| **Vitesse CI** | ✅ Rapide | ❌ Très lent |
| **Facilité** | ✅ Flag de compilation | Outil externe |
| **Détection** | Mémoire, UB, threads | Mémoire principalement |
...
Sans `-g`, les rapports ne montreront que des adresses mémoire.

### 2. Utilisez `-O1` en Debug

```bash
-fsanitize=address -g -O1  # Meilleur équilibre
```

`-O1` améliore les performances sans nuire au debugging.
...
Ces deux sanitizers sont complémentaires et compatibles.
...
Les sanitizers ajoutent un overhead important et sont réservés au développement/CI.

### 6. Corrigez TOUS les bugs détectés

Ne supprimez pas les warnings sans comprendre. Chaque erreur détectée est un vrai bug.
...
**Solution :**
```yaml
- name: Installer les bibliothèques sanitizer
  run: sudo apt-get install -y libasan6 libubsan1 libtsan0
```

### Erreur : "Cannot combine ASan and TSan"

**Solution :** Utilisez des jobs séparés dans la matrice.

### Performances trop lentes

**Solution :** Augmentez `-O1` à `-O2` :
```bash
...
**Solution :** Créez un fichier de suppressions (voir section précédente).
...
1. **ASan** (`-fsanitize=address`) : Bugs mémoire
2. **UBSan** (`-fsanitize=undefined`) : Comportements indéfinis
3. **TSan** (`-fsanitize=thread`) : Race conditions
4. **LSan** (`-fsanitize=leak`) : Fuites mémoire (inclus dans ASan)
...
- ✅ Les sanitizers détectent bugs mémoire, UB et race conditions
- ✅ ASan est le plus important : activez-le toujours en CI
- ✅ Combinez ASan + UBSan pour une couverture maximale
- ✅ TSan pour le multi-threading (incompatible avec ASan)
- ✅ Flag minimal : `-fsanitize=address -g`
- ✅ Overhead acceptable en CI : 2-3x (vs 20-50x pour Valgrind)
- ✅ JAMAIS en production, seulement en dev/CI
- ✅ Corrigez tous les bugs détectés, ce sont de vrais problèmes

⏭️ [Analyse Valgrind automatisée](/30-integration-continue/01.6-analyse-valgrind.md)
