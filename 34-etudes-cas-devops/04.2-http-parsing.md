...
Maintenant que nous avons une architecture event-driven capable de gérer des milliers de connexions (section 34.4.1), nous devons comprendre et parser le **protocole HTTP** pour construire un serveur web fonctionnel.

HTTP (HyperText Transfer Protocol) est le protocole qui permet aux navigateurs web et aux serveurs de communiquer. Notre objectif : créer un parser HTTP simple mais robuste en C, capable d'interpréter les requêtes des clients et de générer des réponses valides.
...
HTTP est un protocole **textuel** (contrairement à des protocoles binaires), ce qui le rend facile à lire et à debugger.
...
**Notre serveur :** Nous implémenterons **HTTP/1.1** (la version la plus utilisée pour les serveurs simples).
...
```
┌─────────────────────────────────────────────────────────┐
│ 1. REQUEST LINE (ligne de requête)                      │
...
|---------|-------|---------|
| GET | Récupérer une ressource | `GET /page.html` |
| POST | Envoyer des données | `POST /form` |
| HEAD | Comme GET mais sans body | `HEAD /page.html` |
| PUT | Mettre à jour une ressource | `PUT /file.txt` |
| DELETE | Supprimer une ressource | `DELETE /file.txt` |

**Pour notre serveur simple :** Nous implémenterons **GET** et **HEAD**.
...
| `Accept` | Types MIME acceptés | `Accept: text/html` |
...
GET /index.html HTTP/1.1\r\n  
Host: example.com\r\n  
\r\n  ← Cette ligne vide marque la fin des headers
```

### 4. Body (optionnel)

Le body contient des données pour les méthodes POST, PUT, etc.
...
**Pour notre serveur GET/HEAD :** Pas de body dans les requêtes.

---

## Structure d'une Réponse HTTP

Une réponse HTTP se compose également de **trois parties** :

```
┌─────────────────────────────────────────────────────────┐
│ 1. STATUS LINE (ligne de statut)                        │
│    HTTP/1.1 200 OK                                      │
├─────────────────────────────────────────────────────────┤
│ 2. HEADERS (en-têtes)                                   │
│    Content-Type: text/html                              │
...
HTTP/1.1 200 OK
│        │   │
│        │   └─ Phrase explicative (optionnelle)
│        └─ Code de statut
└─ Version HTTP
```
...
| `Date` | Date de la réponse | Format RFC 7231 |
...
Extraire les trois composants de la première ligne :

```
GET /index.html HTTP/1.1
│   │           │
│   │           └─ version (doit être HTTP/1.0 ou HTTP/1.1)
│   └─ uri (chemin de la ressource)
...
int parse_request_line(const char *line, http_request_line_t *req_line) {
    ...
    if (parsed != 3) {
        return -1;  // Ligne malformée
    }

    // Valider la méthode (optionnel mais recommandé)
    if (strcmp(req_line->method, "GET") != 0 &&
        strcmp(req_line->method, "HEAD") != 0 &&
        strcmp(req_line->method, "POST") != 0) {
        return -1;  // Méthode non supportée
    }
...
**Explication :**
- `sscanf()` parse la ligne en 3 tokens séparés par des espaces
- Les limites (`%15s`, `%2047s`) préviennent les buffer overflows
- Validation pour rejeter les requêtes invalides

### Alternative : Parsing manuel

Pour plus de contrôle, on peut parser manuellement :

```c
int parse_request_line_manual(const char *line, http_request_line_t *req_line) {
    const char *p = line;

    // 1. Extraire la méthode (jusqu'au premier espace)
    size_t method_len = 0;
    ...
    if (*p != ' ') {
        return -1;  // Pas d'espace après la méthode
    }
    p++;  // Sauter l'espace

    // 2. Extraire l'URI (jusqu'au deuxième espace)
    size_t uri_len = 0;
    ...
    if (*p != ' ') {
        return -1;  // Pas d'espace après l'URI
    }
    p++;  // Sauter l'espace

    // 3. Extraire la version (jusqu'à \r ou \n ou fin)
...
int parse_header(const char *line, http_header_t *header) {
    ...
    // Extraire la valeur (jusqu'à \r ou \n)
...
int parse_headers(const char *buffer, http_headers_t *headers) {
    ...
    while (*line_start != '\0' && headers->num_headers < MAX_HEADERS) {
        ...
        if (line_end == NULL) {
            break;  // Pas de fin de ligne trouvée
        }
        ...
        // Créer une copie temporaire de la ligne
...
typedef struct {
    ...
    char *body;           // Pointeur vers le body (si présent)
...
int parse_http_request(const char *raw_request, http_request_t *request) {
    ...
    // 1. Parser la request line (première ligne)
    const char *line_end = strstr(raw_request, "\r\n");
    if (line_end == NULL) {
        return -1;  // Requête incomplète
    }
    ...
    // 3. Trouver le body (après la ligne vide)
    const char *body_start = strstr(headers_start, "\r\n\r\n");
    if (body_start != NULL) {
        body_start += 4;  // Sauter \r\n\r\n

        // Vérifier si un body est présent
...
Le type MIME (Multipurpose Internet Mail Extensions) indique au client le type de contenu qu'il reçoit.
...
### Fonction de détection du type MIME
...
**Problème :** Un attaquant peut essayer d'accéder à des fichiers hors du répertoire web.
...
int is_safe_path(const char *uri, const char *document_root) {
    ...
    // Résoudre le chemin réel (supprime .., ., liens symboliques)
    if (realpath(full_path, real_path) == NULL) {
        return 0;  // Chemin invalide
    }

    // Vérifier que le chemin réel est bien dans document_root
...
### 2. Limitation de la taille des requêtes
...
ssize_t read_request(int client_fd, char *buffer, size_t buffer_size) {
    ...
    while (total_read < buffer_size - 1) {
        ...
        // Vérifier si on a reçu la fin des headers
        if (strstr(buffer, "\r\n\r\n") != NULL) {
            break;
        }

        // Protection contre les requêtes trop grandes
        if (total_read >= MAX_REQUEST_SIZE) {
            return -1;  // Requête trop grande
        }
    }
...
int validate_headers(const http_headers_t *headers) {
    // Vérifier que Host est présent (requis en HTTP/1.1)
    const char *host = get_header_value(headers, "Host");
    if (host == NULL) {
        return -1;
    }

    // Vérifier Content-Length si présent
...
void handle_client(int client_fd) {
    ...
    close(client_fd);  // Fermer après chaque requête
}
```
...
void handle_client_keepalive(int client_fd) {
    while (1) {
        ...
        // Vérifier si le client veut garder la connexion
        ...
        // En HTTP/1.0, keep-alive doit être explicite
        ...
        // Continuer à lire la prochaine requête...
    }
...
**Note :** Pour un serveur simple, il est plus facile de fermer la connexion après chaque requête (`Connection: close`).
...
```c
// ✅ Bon pour requêtes courtes : buffer sur la pile
void handle_client(int client_fd) {
    char buffer[8192];  // Rapide
    // ...
}

// ✅ Bon pour requêtes longues : allocation dynamique
void handle_client(int client_fd) {
...
Ne parser que ce qui est nécessaire :
...
Pour les noms de headers (répétés) :
...
Le parsing HTTP en C nécessite :

1. **Comprendre le protocole** : Format texte, request line, headers, body
2. **Parser la request line** : Extraire méthode, URI, version
3. **Parser les headers** : Extraire paires nom:valeur
4. **Valider les données** : Sécurité contre injections, path traversal
5. **Générer des réponses** : Status line, headers, body
6. **Gérer les types MIME** : Content-Type approprié
7. **Gérer les erreurs** : 400, 404, 500, 501

**Points clés :**
- HTTP est **textuel** et **simple à parser**
- Toujours **valider et limiter** les entrées
- Gérer les **cas d'erreur** proprement
- Utiliser `\r\n` (CRLF) pour les fins de ligne

**Prochaine étape :** Nous allons maintenant implémenter le **serving de fichiers statiques** pour servir HTML, CSS, images, etc. depuis le disque.
...
- **http-parser** (utilisé par Node.js) : https://github.com/nodejs/http-parser
- **picohttpparser** : Parser minimaliste et ultra-rapide
- **llhttp** : Successeur de http-parser

### Concepts Avancés

- **Chunked Transfer Encoding** : Envoyer des données par morceaux
...
Le parsing HTTP est la base de tout serveur web. Avec une compréhension solide de ces concepts, vous pouvez construire des serveurs robustes et performants !

⏭️ [Static file serving](/34-etudes-cas-devops/04.3-static-file-serving.md)
