...
Les **artifacts** (artefacts en français) sont des fichiers générés par votre pipeline CI/CD que vous souhaitez **sauvegarder et partager**. Il peut s'agir de binaires compilés, de rapports de tests, de logs, de documentation, ou de tout autre fichier produit pendant le workflow.

GitHub Actions permet de publier ces artifacts directement depuis votre workflow, de les télécharger via l'interface web, et de les réutiliser dans d'autres jobs ou workflows. C'est essentiel pour **distribuer vos builds**, **archiver les rapports** et **déboguer les problèmes** en CI.
...
1. **Distribution de binaires** : Partager les exécutables compilés avec les utilisateurs
2. **Rapports de tests** : Archiver les résultats de tests, couverture, analyse statique
3. **Documentation** : Publier la documentation HTML générée (Doxygen, etc.)
4. **Debugging** : Sauvegarder les logs et core dumps pour investigation
5. **Releases** : Créer des packages pour les versions officielles
6. **Artefacts intermédiaires** : Partager des fichiers entre jobs d'un même workflow

### Avantages

- ✅ **Accessibles facilement** : Via l'interface GitHub Actions
- ✅ **Versionnés automatiquement** : Associés au commit/PR
- ✅ **Partage d'équipe** : Tout le monde peut les télécharger
- ✅ **Temporaires ou permanents** : Durée de rétention configurable
- ✅ **Organisation** : Groupés par workflow run
...
- `name:` Nom de l'artifact (apparaît dans l'interface)
- `path:` Fichier(s) ou répertoire(s) à uploader

---

## Exemple simple : Publier un binaire

Voici un workflow qui compile un programme et publie l'exécutable :
...
**Résultat :** Le fichier `mon_app` sera téléchargeable depuis l'interface GitHub Actions.
...
2. Cliquez sur le workflow run concerné
3. Scrollez jusqu'à la section **Artifacts**
4. Cliquez sur le nom de l'artifact pour le télécharger (ZIP)

### Via l'action download-artifact (entre jobs)

Pour réutiliser un artifact dans un autre job du même workflow :
...
**Note :** La structure de répertoires est préservée dans l'artifact.
...
```yaml
- name: Uploader les bibliothèques
  uses: actions/upload-artifact@v4
  with:
    name: libraries
    path: |
...
Par défaut, les artifacts sont conservés **90 jours**. Vous pouvez modifier cette durée :

```yaml
- name: Uploader avec rétention courte
  uses: actions/upload-artifact@v4
  with:
    name: temp-artifact
    path: temp/
...
| Type d'artifact | Rétention recommandée |
|-----------------|----------------------|
...
**Résultat :** 3 artifacts, un par plateforme, disponibles au téléchargement.
...
Par défaut, tous les fichiers du chemin sont inclus. Pour exclure certains fichiers :
...
GitHub Actions compresse automatiquement les artifacts en ZIP. Vous n'avez rien à faire.

**Taille maximale par artifact :**
- Comptes gratuits : Illimitée (mais quotas de stockage)
...
### Upload seulement en cas de succès

```yaml
- name: Uploader si succès
  if: success()
  uses: actions/upload-artifact@v4
  with:
    name: release-binary
...
### Upload seulement en cas d'échec

```yaml
- name: Uploader les logs de debug si échec
  if: failure()
  uses: actions/upload-artifact@v4
  with:
    name: failure-logs
...
### Upload toujours (succès ou échec)
...
- `if: success()` : Job réussi
- `if: failure()` : Job échoué
- `if: always()` : Toujours (succès ou échec)
- `if: cancelled()` : Job annulé

---

## Artifacts par événement

### Artifacts différents selon l'événement

```yaml
- name: Uploader binaire de développement
...
```yaml
- name: Uploader avec métadonnées
  uses: actions/upload-artifact@v4
  with:
...
- `${{ github.run_number }}` : Numéro de run
...
## Combiner des artifacts

Pour rassembler des artifacts de plusieurs jobs, uploadez-les avec des **noms différents** puis téléchargez-les ensemble :
...
jobs:
  ...
  download-all:
    ...
    steps:
      - uses: actions/download-artifact@v4
        with:
          merge-multiple: true  # Fusionne tous les artifacts

      - run: ls -la  # linux.txt et windows.txt présents
```

**Note :** Avec `v4`, chaque artifact doit avoir un **nom unique**. Utilisez `merge-multiple: true` dans `download-artifact` pour fusionner les fichiers de plusieurs artifacts dans un même répertoire.
...
**Résultat :** L'artifact est disponible :
1. Dans le workflow run (temporaire)
2. Dans la release GitHub (permanent)

---

## Artifacts privés vs publics

### Par défaut : Privés

Les artifacts sont **privés** par défaut :
- Accessibles uniquement aux collaborateurs du dépôt
- Nécessite une authentification GitHub

### Partage public

Pour partager publiquement, vous devez :

1. **Attacher à une release** (recommandé)
   ```yaml
   - uses: softprops/action-gh-release@v2
     with:
       files: build/app
...
Dans GitHub : `Settings → Billing → Storage for Actions and Packages`
...
| Caractéristique | Artifacts | Cache |
|-----------------|-----------|-------|
| **Usage** | Résultats de build | Dépendances/builds intermédiaires |
| **Durée** | 1-90 jours | 7 jours max |
| **Téléchargeable** | ✅ Oui (via UI) | ❌ Non (CI seulement) |
| **Entre workflows** | ✅ Oui | ❌ Non (même workflow seulement) |
| **Taille limite** | Illimitée (quotas stockage) | 10 GB par dépôt |
| **Exemple** | Binaire compilé | node_modules, build/ |
...
- Objets de compilation intermédiaires
...
**Cause :** Le chemin spécifié ne contient aucun fichier.
...
2. Vérifier le working directory
3. Utiliser des chemins absolus si nécessaire

### Erreur : Artifact trop volumineux

**Cause :** L'artifact dépasse les quotas de stockage.

**Solutions :**
1. Exclure les fichiers inutiles
2. Compresser avant upload
3. Publier seulement ce qui est nécessaire

### Artifact non téléchargeable

**Cause :** Workflow run en cours ou artifact expiré.

**Solutions :**
1. Attendre la fin du workflow
2. Vérifier la durée de rétention
3. Régénérer si expiré

### Conflit de noms d'artifacts

**Cause :** Plusieurs jobs uploadent avec le même nom.
...
```
GitHub → Actions → Workflow run → Artifacts (section en bas)
```
...
jobs:
  ...
  release:
    ...
    steps:
    - name: Télécharger tous les artifacts
      uses: actions/download-artifact@v4
    ...
    - name: Créer la release
      ...
      with:
        ...
        body: |
          ...
          Téléchargez les binaires pour votre plateforme ci-dessous.
        draft: false
        prerelease: false
```
...
**Les artifacts sauvegardent les fichiers générés par votre CI.**
...
- Créer des **releases automatiques** avec GitHub Releases
- Publier sur des **registres de packages** (npm, PyPI, etc.)
- Configurer des **déploiements continus**
...
- ✅ Les artifacts sauvegardent les fichiers générés par la CI
- ✅ Accessible via l'interface GitHub Actions
- ✅ `actions/upload-artifact@v4` pour publier
- ✅ `actions/download-artifact@v4` pour réutiliser
- ✅ Durée de rétention configurable (1-90 jours)
- ✅ Noms descriptifs avec métadonnées (${{ matrix.os }}, ${{ github.sha }})
- ✅ Upload conditionnel selon le résultat (if: success(), if: failure())
- ✅ Différence artifacts (résultats) vs cache (accélération)

⏭️ [GitLab CI pour projets C](/30-integration-continue/02-gitlab-ci.md)
