🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 15.1.7 Intégration dans le workflow

## Introduction

Connaître les sanitizers est une chose, mais les **intégrer efficacement** dans votre processus de développement quotidien en est une autre. Cette section vous montre comment faire des sanitizers une partie **naturelle et automatique** de votre travail, du premier coup de code jusqu'à la mise en production.

### Qu'est-ce qu'un workflow de développement ?

Un **workflow** (flux de travail) est l'ensemble des étapes que vous suivez pour développer, tester et déployer du code :

```
Écrire du code → Compiler → Tester localement → Commit → CI/CD → Review → Merge → Production
```

**L'objectif :** Intégrer les sanitizers à **chaque étape** pour détecter les bugs le plus tôt possible.

### Pourquoi l'intégration est importante ?

**Sans intégration :**
- 😕 Vous oubliez d'utiliser les sanitizers
- 🐛 Les bugs passent inaperçus
- 💥 Découverte tardive (coûteux à corriger)

**Avec intégration :**
- ✅ Détection automatique à chaque étape
- 🚀 Bugs corrigés immédiatement
- 💪 Code plus robuste sans effort conscient

---

## Vue d'ensemble du workflow

### Les différentes étapes

```
┌─────────────────────────────────────────────────────────────┐
│                   WORKFLOW DE DÉVELOPPEMENT                 │
└─────────────────────────────────────────────────────────────┘

1. DÉVELOPPEMENT LOCAL
   ├── Écrire du code
   ├── Compiler avec sanitizers       ← ASan + UBSan
   ├── Exécuter et tester
   └── Itérer

2. TESTS LOCAUX (avant commit)
   ├── Tests unitaires               ← ASan + UBSan
   ├── Tests d'intégration           ← ASan + UBSan
   └── Tests de concurrence          ← TSan (si applicable)

3. PRÉ-COMMIT
   ├── Hooks Git                     ← Validation automatique
   ├── Formatage
   └── Vérifications rapides

4. CI/CD (après push)
   ├── Build matrix                  ← Plusieurs configurations
   ├── Tests automatisés             ← ASan + UBSan
   ├── Tests de concurrence          ← TSan
   └── Analyse de code

5. CODE REVIEW
   ├── Review humaine
   └── Validation sanitizers OK

6. DÉPLOIEMENT
   ├── Staging avec sanitizers       ← Dernière vérification
   └── Production sans sanitizers    ← Performance max
```

---

## Phase 1 : Développement local

### Configuration de l'environnement

**Objectif :** Avoir les sanitizers toujours prêts à l'emploi.

#### Fichier de configuration : `.vimrc` ou `.bashrc`

**Créer des alias pour faciliter la compilation :**

```bash
# ~/.bashrc ou ~/.zshrc

# Alias de compilation avec sanitizers
alias gcc-san='gcc -fsanitize=address,undefined -g -O1 -Wall -Wextra'  
alias gcc-tsan='gcc -fsanitize=thread -g -O1 -pthread -Wall -Wextra'  
alias gcc-debug='gcc -g -O0 -Wall -Wextra'  
alias gcc-release='gcc -O2 -DNDEBUG -Wall -Wextra'  

# Fonction pour compiler et exécuter
crun() {
    gcc-san -o /tmp/a.out "$1" && /tmp/a.out
}

# Variables d'environnement pour sanitizers
export ASAN_OPTIONS="halt_on_error=1:detect_leaks=1"  
export UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1"  
export TSAN_OPTIONS="halt_on_error=1:history_size=7"  
```

**Utilisation :**
```bash
# Compilation rapide avec sanitizers
gcc-san main.c -o prog

# Compilation et exécution
crun main.c
```

#### Configuration du projet

**Structure recommandée :**

```
mon_projet/
├── src/                    # Code source
├── tests/                  # Tests
├── build/                  # Builds temporaires
├── scripts/                # Scripts d'automatisation
│   ├── build_san.sh       # Build avec sanitizers
│   ├── build_tsan.sh      # Build avec TSan
│   └── run_tests.sh       # Exécution des tests
├── .clang-format          # Style de code
├── .gitignore
├── Makefile               # Système de build
└── README.md
```

### Makefile intégré

**Créer un Makefile avec plusieurs targets :**

```makefile
# Makefile

CC = gcc  
CFLAGS = -Wall -Wextra -std=c11  
SANITIZER_FLAGS = -g -O1 -fno-omit-frame-pointer  

# Sources
SRC = src/main.c src/utils.c  
OBJ = $(SRC:.c=.o)  

# Targets
.PHONY: all clean dev test test-tsan debug release

# Build par défaut : avec sanitizers
all: dev

# Build développement : ASan + UBSan
dev:
	$(CC) $(CFLAGS) $(SANITIZER_FLAGS) \
		-fsanitize=address,undefined \
		-fno-sanitize-recover=all \
		$(SRC) -o build/prog_dev

# Build tests : Exécute les tests avec sanitizers
test: dev
	@echo "=== Exécution des tests avec ASan + UBSan ==="
	@export ASAN_OPTIONS=detect_leaks=1:halt_on_error=1; \
	 ./build/prog_dev --run-tests
	@echo "✅ Tests réussis"

# Build tests concurrence : TSan
test-tsan:
	$(CC) $(CFLAGS) $(SANITIZER_FLAGS) \
		-fsanitize=thread \
		-pthread \
		$(SRC) -o build/prog_tsan
	@echo "=== Exécution des tests avec TSan ==="
	@export TSAN_OPTIONS=halt_on_error=1; \
	 ./build/prog_tsan --run-tests
	@echo "✅ Tests TSan réussis"

# Build debug : Sans optimisation
debug:
	$(CC) $(CFLAGS) -g -O0 $(SRC) -o build/prog_debug

# Build release : Production
release:
	$(CC) $(CFLAGS) -O2 -DNDEBUG $(SRC) -o build/prog_release
	@echo "✅ Build production créé"

# Nettoyage
clean:
	rm -rf build/*

# Vérification complète avant commit
pre-commit: dev test test-tsan
	@echo "✅ Toutes les vérifications sont passées"
```

**Utilisation quotidienne :**

```bash
# Développement
make dev
./build/prog_dev

# Tests rapides
make test

# Tests complets (avant commit)
make pre-commit

# Build production
make release
```

### Scripts d'automatisation

**Script : `scripts/build_san.sh`**

```bash
#!/bin/bash
# Build avec sanitizers et affichage coloré

set -e  # Arrêt en cas d'erreur

GREEN='\033[0;32m'  
RED='\033[0;31m'  
NC='\033[0m' # No Color  

echo -e "${GREEN}🔨 Compilation avec sanitizers...${NC}"

gcc -fsanitize=address,undefined \
    -fno-sanitize-recover=all \
    -g -O1 \
    -Wall -Wextra -Werror \
    -o build/prog_san \
    src/*.c

if [ $? -eq 0 ]; then
    echo -e "${GREEN}✅ Compilation réussie${NC}"
else
    echo -e "${RED}❌ Erreur de compilation${NC}"
    exit 1
fi
```

**Script : `scripts/run_tests.sh`**

```bash
#!/bin/bash
# Exécution des tests avec sanitizers

set -e

GREEN='\033[0;32m'  
RED='\033[0;31m'  
YELLOW='\033[1;33m'  
NC='\033[0m'  

# Configuration des sanitizers
export ASAN_OPTIONS="detect_leaks=1:halt_on_error=1:abort_on_error=1"  
export UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1"  

echo -e "${YELLOW}🧪 Exécution des tests...${NC}"

# Compilation
./scripts/build_san.sh

# Exécution
echo -e "${YELLOW}📋 Tests unitaires...${NC}"  
if ./build/prog_san --unit-tests; then  
    echo -e "${GREEN}✅ Tests unitaires OK${NC}"
else
    echo -e "${RED}❌ Tests unitaires échoués${NC}"
    exit 1
fi

echo -e "${YELLOW}📋 Tests d'intégration...${NC}"  
if ./build/prog_san --integration-tests; then  
    echo -e "${GREEN}✅ Tests d'intégration OK${NC}"
else
    echo -e "${RED}❌ Tests d'intégration échoués${NC}"
    exit 1
fi

echo -e "${GREEN}🎉 Tous les tests sont passés !${NC}"
```

**Rendre les scripts exécutables :**
```bash
chmod +x scripts/*.sh
```

**Utilisation :**
```bash
./scripts/run_tests.sh
```

### Workflow quotidien recommandé

**1. Début de session : Compilation avec sanitizers**

```bash
# Le matin, première compilation
make dev
```

**2. Développement itératif**

```bash
# Écrire du code
vim src/feature.c

# Compiler et tester
make dev && ./build/prog_dev

# Si erreur sanitizer → Corriger immédiatement
# Recommencer
```

**3. Tests réguliers**

```bash
# Toutes les 15-30 minutes, ou après chaque feature
make test
```

**4. Avant le déjeuner / fin de journée**

```bash
# Vérification complète
make pre-commit
```

---

## Phase 2 : Tests locaux (pré-commit)

### Hooks Git

**Les hooks Git** permettent d'automatiser des vérifications avant chaque commit.

#### Installation d'un hook pre-commit

**Créer le fichier `.git/hooks/pre-commit` :**

```bash
#!/bin/bash
# Hook pre-commit : Vérifie le code avant commit

set -e

echo "🔍 Vérification pre-commit..."

# 1. Compilation avec sanitizers
echo "📦 Compilation avec sanitizers..."  
if ! make dev > /dev/null 2>&1; then  
    echo "❌ Erreur de compilation"
    exit 1
fi

# 2. Exécution des tests
echo "🧪 Exécution des tests..."  
if ! make test > /dev/null 2>&1; then  
    echo "❌ Tests échoués"
    exit 1
fi

# 3. Vérification du formatage (optionnel)
echo "🎨 Vérification du formatage..."  
if command -v clang-format &> /dev/null; then  
    clang-format --dry-run --Werror src/*.c
fi

# 4. Analyse statique rapide (optionnel)
echo "🔬 Analyse statique..."  
if command -v cppcheck &> /dev/null; then  
    cppcheck --quiet --error-exitcode=1 src/
fi

echo "✅ Toutes les vérifications sont passées"  
echo "✅ Commit autorisé"  
```

**Rendre le hook exécutable :**
```bash
chmod +x .git/hooks/pre-commit
```

**Test du hook :**
```bash
# Essayer de commit
git add .  
git commit -m "Test commit"  

# Le hook s'exécute automatiquement
# Commit bloqué si tests échouent
```

#### Hook plus sophistiqué

**Version avec timing et couleurs :**

```bash
#!/bin/bash
# Hook pre-commit avancé

GREEN='\033[0;32m'  
RED='\033[0;31m'  
YELLOW='\033[1;33m'  
NC='\033[0m'  

START_TIME=$(date +%s)

echo -e "${YELLOW}🔍 Pre-commit checks...${NC}"

# Fonction pour mesurer le temps
check() {
    local name=$1
    shift
    echo -ne "${YELLOW}⏳ ${name}...${NC} "

    if "$@" > /tmp/precommit_log 2>&1; then
        echo -e "${GREEN}✓${NC}"
        return 0
    else
        echo -e "${RED}✗${NC}"
        cat /tmp/precommit_log
        return 1
    fi
}

# Vérifications
check "Compilation" make dev || exit 1  
check "Tests unitaires" make test || exit 1  

# Si code multi-threadé
if grep -q "pthread" src/*.c; then
    check "Tests TSan" make test-tsan || exit 1
fi

# Temps total
END_TIME=$(date +%s)  
DURATION=$((END_TIME - START_TIME))  

echo -e "${GREEN}✅ Toutes les vérifications OK (${DURATION}s)${NC}"
```

### Stratégie de tests par type

**Tests rapides (à chaque commit) :**
```bash
# Tests unitaires uniquement (~30 secondes)
make test
```

**Tests complets (commits importants) :**
```bash
# Tests unitaires + intégration + TSan (~2-5 minutes)
make pre-commit
```

**Tests exhaustifs (avant PR) :**
```bash
# Tous les tests, toutes les configurations
./scripts/full_test_suite.sh
```

---

## Phase 3 : Intégration Continue (CI/CD)

### GitHub Actions

**Créer `.github/workflows/ci.yml` :**

```yaml
name: CI avec Sanitizers

on:
  push:
    branches: [ main, develop ]
  pull_request:
    branches: [ main ]

jobs:
  # Job 1 : Tests avec ASan + UBSan
  test-asan-ubsan:
    runs-on: ubuntu-latest

    steps:
      - name: Checkout code
        uses: actions/checkout@v3

      - name: Install dependencies
        run: |
          sudo apt-get update
          sudo apt-get install -y build-essential

      - name: Compile avec ASan + UBSan
        run: |
          gcc -fsanitize=address,undefined \
              -fno-sanitize-recover=all \
              -g -O1 -Wall -Wextra -Werror \
              -o prog src/*.c

      - name: Run tests
        run: |
          export ASAN_OPTIONS="detect_leaks=1:halt_on_error=1"
          export UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1"
          ./prog --run-tests

      - name: Upload logs on failure
        if: failure()
        uses: actions/upload-artifact@v3
        with:
          name: sanitizer-logs
          path: |
            *.log
            core.*

  # Job 2 : Tests avec TSan (si multi-threadé)
  test-tsan:
    runs-on: ubuntu-latest

    steps:
      - name: Checkout code
        uses: actions/checkout@v3

      - name: Compile avec TSan
        run: |
          gcc -fsanitize=thread \
              -g -O1 -pthread \
              -Wall -Wextra -Werror \
              -o prog_tsan src/*.c

      - name: Run concurrency tests
        run: |
          export TSAN_OPTIONS="halt_on_error=1:history_size=7"
          ./prog_tsan --run-tests

  # Job 3 : Analyse statique
  static-analysis:
    runs-on: ubuntu-latest

    steps:
      - name: Checkout code
        uses: actions/checkout@v3

      - name: Install tools
        run: |
          sudo apt-get update
          sudo apt-get install -y cppcheck clang-tidy

      - name: Run cppcheck
        run: cppcheck --error-exitcode=1 --quiet src/

      - name: Run clang-tidy
        run: |
          clang-tidy src/*.c -- -Wall -Wextra

  # Job 4 : Build matrix (plusieurs configurations)
  build-matrix:
    runs-on: ubuntu-latest
    strategy:
      matrix:
        compiler: [gcc, clang]
        sanitizer: [address,undefined, thread, leak]
        exclude:
          # TSan incompatible avec ASan
          - sanitizer: thread

    steps:
      - name: Checkout code
        uses: actions/checkout@v3

      - name: Install compiler
        run: |
          if [ "${{ matrix.compiler }}" = "clang" ]; then
            sudo apt-get install -y clang
          fi

      - name: Build
        run: |
          ${{ matrix.compiler }} \
            -fsanitize=${{ matrix.sanitizer }} \
            -g -O1 -o prog src/*.c

      - name: Test
        run: ./prog --run-tests
```

**Configuration expliquée :**
- **test-asan-ubsan** : Tests principaux (sur chaque commit)
- **test-tsan** : Tests de concurrence (sur chaque commit si multi-threadé)
- **static-analysis** : Analyse statique complémentaire
- **build-matrix** : Tests avec plusieurs compilateurs et sanitizers

### GitLab CI

**Créer `.gitlab-ci.yml` :**

```yaml
stages:
  - build
  - test
  - analyze

variables:
  ASAN_OPTIONS: "detect_leaks=1:halt_on_error=1"
  UBSAN_OPTIONS: "halt_on_error=1:print_stacktrace=1"
  TSAN_OPTIONS: "halt_on_error=1:history_size=7"

# Template pour builds avec sanitizers
.sanitizer_build:
  image: gcc:latest
  before_script:
    - apt-get update && apt-get install -y make

# Build et test avec ASan + UBSan
test:asan:
  extends: .sanitizer_build
  stage: test
  script:
    - make dev
    - make test
  artifacts:
    when: on_failure
    paths:
      - build/
      - "*.log"

# Build et test avec TSan
test:tsan:
  extends: .sanitizer_build
  stage: test
  script:
    - make test-tsan
  only:
    - merge_requests
    - main

# Analyse statique
analyze:static:
  extends: .sanitizer_build
  stage: analyze
  script:
    - apt-get install -y cppcheck
    - cppcheck --error-exitcode=1 src/
  allow_failure: true

# Tests nightly (exhaustifs)
test:nightly:
  extends: .sanitizer_build
  stage: test
  script:
    - make pre-commit
    - make test-tsan
  only:
    - schedules
```

### Jenkins Pipeline

**Créer `Jenkinsfile` :**

```groovy
pipeline {
    agent any

    environment {
        ASAN_OPTIONS = 'detect_leaks=1:halt_on_error=1'
        UBSAN_OPTIONS = 'halt_on_error=1:print_stacktrace=1'
    }

    stages {
        stage('Build') {
            steps {
                sh 'make clean'
                sh 'make dev'
            }
        }

        stage('Test - ASan + UBSan') {
            steps {
                sh 'make test'
            }
        }

        stage('Test - TSan') {
            when {
                branch 'main'
            }
            steps {
                sh 'make test-tsan'
            }
        }

        stage('Static Analysis') {
            steps {
                sh 'cppcheck --xml --xml-version=2 src/ 2> cppcheck.xml'
            }
            post {
                always {
                    recordIssues(tools: [cppCheck(pattern: 'cppcheck.xml')])
                }
            }
        }
    }

    post {
        failure {
            archiveArtifacts artifacts: 'build/*, *.log', allowEmptyArchive: true
        }
        always {
            cleanWs()
        }
    }
}
```

### Stratégie CI/CD par branche

**Branche `feature/*` (développement) :**
```yaml
# Tests légers uniquement
- ASan + UBSan
- Temps : ~2 minutes
```

**Branche `develop` :**
```yaml
# Tests standards
- ASan + UBSan
- TSan (si multi-threadé)
- Analyse statique
- Temps : ~5 minutes
```

**Branche `main` / Pull Request :**
```yaml
# Tests complets
- ASan + UBSan (tous les tests)
- TSan (tous les tests)
- Analyse statique
- Build matrix (GCC + Clang)
- Temps : ~10-15 minutes
```

**Tests nightly (1× par jour) :**
```yaml
# Tests exhaustifs
- Tous les sanitizers
- Toutes les configurations
- Tests de charge
- Temps : ~30-60 minutes
```

---

## Phase 4 : Code Review

### Checklist pour reviewers

**Vérifications automatiques (avant review) :**
- ✅ CI/CD passé (tous les jobs verts)
- ✅ Aucune erreur sanitizer détectée
- ✅ Coverage de code maintenu ou amélioré
- ✅ Analyse statique sans nouveaux warnings

**Vérifications manuelles :**

```markdown
## Checklist Code Review

### Gestion mémoire
- [ ] Pas de malloc sans free correspondant ?
- [ ] Pas de double free ?
- [ ] Pas d'utilisation après free ?
- [ ] Vérification des retours de malloc ?

### Concurrence (si applicable)
- [ ] Tous les accès partagés sont protégés ?
- [ ] Pas de deadlock potentiel ?
- [ ] Ordre de lock cohérent ?

### Comportements indéfinis
- [ ] Pas de débordement d'entiers possible ?
- [ ] Pas de division par zéro possible ?
- [ ] Pas de shift invalide ?

### Tests
- [ ] Tests unitaires ajoutés pour nouvelle feature ?
- [ ] Tests passent avec sanitizers activés ?
- [ ] Tests de concurrence si code multi-threadé ?
```

### Commande pour reviewer

**Tester la PR localement avec sanitizers :**

```bash
# Checkout de la PR
git fetch origin pull/123/head:pr-123  
git checkout pr-123  

# Build et test avec sanitizers
make clean  
make pre-commit  

# Si multi-threadé
make test-tsan

# Tests de charge (optionnel)
./scripts/stress_test.sh
```

---

## Phase 5 : Staging et pré-production

### Environnement de staging avec sanitizers

**Idée :** Avoir un environnement de staging avec sanitizers activés.

**Avantages :**
- ✅ Détection de bugs qui n'apparaissent qu'avec données réelles
- ✅ Test en conditions proches de la production
- ✅ Dernière barrière avant production

**Configuration Docker pour staging :**

```dockerfile
# Dockerfile.staging
FROM ubuntu:22.04

RUN apt-get update && apt-get install -y \
    gcc \
    make \
    gdb

WORKDIR /app

COPY . .

# Build avec sanitizers
RUN make dev

# Variables d'environnement
ENV ASAN_OPTIONS="detect_leaks=1:abort_on_error=1:log_path=/logs/asan.log"  
ENV UBSAN_OPTIONS="halt_on_error=1:log_path=/logs/ubsan.log"  

# Exécution
CMD ["./build/prog_dev"]
```

**Docker Compose :**

```yaml
# docker-compose.staging.yml
version: '3.8'

services:
  app-staging:
    build:
      context: .
      dockerfile: Dockerfile.staging
    volumes:
      - ./logs:/logs
    environment:
      - ASAN_OPTIONS=detect_leaks=1:abort_on_error=1:log_path=/logs/asan.log
      - UBSAN_OPTIONS=halt_on_error=1:log_path=/logs/ubsan.log
    ports:
      - "8080:8080"
```

**Utilisation :**
```bash
# Déployer en staging avec sanitizers
docker-compose -f docker-compose.staging.yml up

# Surveiller les logs
tail -f logs/asan.log  
tail -f logs/ubsan.log  
```

### Tests de charge en staging

**Script de stress test :**

```bash
#!/bin/bash
# stress_test.sh

echo "🔥 Stress test avec sanitizers..."

# Lancer le serveur avec sanitizers
./build/prog_dev &
PID=$!

# Attendre le démarrage
sleep 2

# Lancer des requêtes intensives
for i in {1..10000}; do
    curl -s http://localhost:8080/api/test > /dev/null &
done

# Attendre la fin
wait

# Arrêter le serveur
kill $PID

# Vérifier les logs sanitizers
if grep -q "ERROR: " logs/*.log; then
    echo "❌ Erreurs détectées !"
    cat logs/*.log
    exit 1
else
    echo "✅ Stress test réussi"
fi
```

---

## Phase 6 : Production

### Build de production (sans sanitizers)

**Important :** La production doit **toujours** être compilée sans sanitizers.

**Makefile target :**
```makefile
release:
	$(CC) -O2 -DNDEBUG -Wall -Wextra -Werror \
		-o build/prog_release \
		src/*.c
	strip build/prog_release  # Supprimer les symboles debug
```

**Dockerfile production :**

```dockerfile
# Dockerfile.production
FROM ubuntu:22.04 AS builder

RUN apt-get update && apt-get install -y gcc make

WORKDIR /build  
COPY . .  

# Build optimisé (SANS sanitizers)
RUN make release

# Image finale (minimale)
FROM ubuntu:22.04

WORKDIR /app

# Copier uniquement le binaire
COPY --from=builder /build/build/prog_release /app/prog

# Utilisateur non-root
RUN useradd -m appuser  
USER appuser  

CMD ["./prog"]
```

### Monitoring en production

**Même sans sanitizers, surveillez les indicateurs :**

```bash
# Surveillance mémoire
watch -n 5 'ps aux | grep prog'

# Logs d'erreurs
tail -f /var/log/app/errors.log

# Core dumps (si crash)
ulimit -c unlimited
# Les core dumps sont analysables avec GDB
```

**Script d'analyse de core dump :**

```bash
#!/bin/bash
# analyze_core.sh

if [ -f core ]; then
    echo "📊 Analyse du core dump..."

    # Recompiler avec symboles debug (pour analyse)
    make debug

    # Analyser avec GDB
    gdb -batch -ex "bt full" build/prog_debug core > crash_report.txt

    echo "📄 Rapport généré : crash_report.txt"
fi
```

---

## Workflow complet : Récapitulatif

### Jour typique d'un développeur

**Matin (9h) :**
```bash
# Pull des dernières modifications
git pull origin main

# Build avec sanitizers
make dev

# Vérification rapide
make test
```

**Développement (9h30 - 12h) :**
```bash
# Boucle de développement
while true; do
    # Coder
    vim src/feature.c

    # Compiler et tester
    make dev && ./build/prog_dev --test-feature

    # Si erreur sanitizer → Corriger immédiatement
done

# Tests réguliers (toutes les 30 min)
make test
```

**Avant déjeuner (12h) :**
```bash
# Vérification complète
make pre-commit

# Si OK, commit
git add .  
git commit -m "feat: nouvelle fonctionnalité"  
```

**Après-midi (14h - 18h) :**
```bash
# Continuer le développement
# ...

# Avant de push (fin de journée)
make pre-commit

# Si multi-threadé, tester TSan
make test-tsan

# Push
git push origin feature/ma-feature
```

**CI/CD (automatique) :**
```
GitHub Actions lance automatiquement :
- Compilation avec ASan + UBSan
- Exécution des tests
- Analyse statique
- Tests TSan (si applicable)
```

**Code Review (lendemain) :**
```bash
# Reviewer teste localement
git checkout pr-123  
make pre-commit  

# Validation et merge
```

**Production (après merge) :**
```bash
# Build de production (sans sanitizers)
make release

# Déploiement
docker build -f Dockerfile.production -t app:latest .  
docker push app:latest  
```

---

## Automatisation avancée

### Script tout-en-un

**`scripts/dev_workflow.sh` :**

```bash
#!/bin/bash
# Workflow de développement automatisé

set -e

GREEN='\033[0;32m'  
RED='\033[0;31m'  
YELLOW='\033[1;33m'  
BLUE='\033[0;34m'  
NC='\033[0m'  

# Fonction d'aide
usage() {
    echo "Usage: $0 [commande]"
    echo ""
    echo "Commandes:"
    echo "  dev         - Build développement avec sanitizers"
    echo "  test        - Exécuter les tests"
    echo "  check       - Vérifications complètes (pre-commit)"
    echo "  tsan        - Tests de concurrence (TSan)"
    echo "  release     - Build production"
    echo "  ci          - Simuler le CI en local"
    echo "  clean       - Nettoyer les builds"
    exit 1
}

# Commande dev
dev() {
    echo -e "${BLUE}🔨 Build développement...${NC}"
    make dev
    echo -e "${GREEN}✅ Build dev créé : build/prog_dev${NC}"
}

# Commande test
test() {
    echo -e "${BLUE}🧪 Exécution des tests...${NC}"
    make test
}

# Commande check (pre-commit)
check() {
    echo -e "${YELLOW}🔍 Vérifications pre-commit...${NC}"

    dev
    test

    # Analyse statique
    if command -v cppcheck &> /dev/null; then
        echo -e "${BLUE}🔬 Analyse statique...${NC}"
        cppcheck --quiet --error-exitcode=1 src/ || {
            echo -e "${RED}❌ Erreurs d'analyse statique${NC}"
            exit 1
        }
    fi

    # Formatage
    if command -v clang-format &> /dev/null; then
        echo -e "${BLUE}🎨 Vérification du formatage...${NC}"
        clang-format --dry-run --Werror src/*.c || {
            echo -e "${RED}❌ Erreurs de formatage${NC}"
            echo "Correction : clang-format -i src/*.c"
            exit 1
        }
    fi

    echo -e "${GREEN}✅ Toutes les vérifications OK${NC}"
}

# Commande tsan
tsan() {
    echo -e "${BLUE}🔀 Tests de concurrence (TSan)...${NC}"
    make test-tsan
}

# Commande release
release() {
    echo -e "${BLUE}📦 Build production...${NC}"
    make release

    # Vérifier que pas de symboles sanitizers
    if nm build/prog_release | grep -q "__asan"; then
        echo -e "${RED}❌ ERREUR : Sanitizers détectés dans le build production !${NC}"
        exit 1
    fi

    echo -e "${GREEN}✅ Build production créé : build/prog_release${NC}"
}

# Commande ci (simule CI en local)
ci() {
    echo -e "${YELLOW}🤖 Simulation CI/CD en local...${NC}"

    echo -e "${BLUE}📋 Étape 1/5 : Build${NC}"
    dev

    echo -e "${BLUE}📋 Étape 2/5 : Tests ASan + UBSan${NC}"
    test

    echo -e "${BLUE}📋 Étape 3/5 : Tests TSan${NC}"
    if grep -q "pthread" src/*.c 2>/dev/null; then
        tsan
    else
        echo "Skipped (pas de code multi-threadé)"
    fi

    echo -e "${BLUE}📋 Étape 4/5 : Analyse statique${NC}"
    if command -v cppcheck &> /dev/null; then
        cppcheck --quiet --error-exitcode=1 src/
    fi

    echo -e "${BLUE}📋 Étape 5/5 : Build production${NC}"
    release

    echo -e "${GREEN}🎉 CI simulé avec succès !${NC}"
}

# Commande clean
clean() {
    echo -e "${BLUE}🧹 Nettoyage...${NC}"
    make clean
    echo -e "${GREEN}✅ Nettoyage terminé${NC}"
}

# Dispatch des commandes
case "$1" in
    dev)     dev ;;
    test)    test ;;
    check)   check ;;
    tsan)    tsan ;;
    release) release ;;
    ci)      ci ;;
    clean)   clean ;;
    *)       usage ;;
esac
```

**Utilisation :**
```bash
chmod +x scripts/dev_workflow.sh

# Développement quotidien
./scripts/dev_workflow.sh dev
./scripts/dev_workflow.sh test

# Avant commit
./scripts/dev_workflow.sh check

# Simuler CI localement
./scripts/dev_workflow.sh ci
```

### Configuration VS Code

**`.vscode/tasks.json` :**

```json
{
    "version": "2.0.0",
    "tasks": [
        {
            "label": "Build (Dev)",
            "type": "shell",
            "command": "make dev",
            "group": {
                "kind": "build",
                "isDefault": true
            },
            "problemMatcher": ["$gcc"]
        },
        {
            "label": "Test",
            "type": "shell",
            "command": "make test",
            "group": "test"
        },
        {
            "label": "Pre-commit Check",
            "type": "shell",
            "command": "./scripts/dev_workflow.sh check",
            "problemMatcher": ["$gcc"]
        },
        {
            "label": "Build (Release)",
            "type": "shell",
            "command": "make release",
            "group": "build"
        }
    ]
}
```

**`.vscode/launch.json` :**

```json
{
    "version": "0.2.0",
    "configurations": [
        {
            "name": "Debug avec sanitizers",
            "type": "cppdbg",
            "request": "launch",
            "program": "${workspaceFolder}/build/prog_dev",
            "args": [],
            "stopAtEntry": false,
            "cwd": "${workspaceFolder}",
            "environment": [
                {
                    "name": "ASAN_OPTIONS",
                    "value": "detect_leaks=1:halt_on_error=0"
                },
                {
                    "name": "UBSAN_OPTIONS",
                    "value": "halt_on_error=0:print_stacktrace=1"
                }
            ],
            "externalConsole": false,
            "MIMode": "gdb",
            "preLaunchTask": "Build (Dev)"
        }
    ]
}
```

---

## Métriques et suivi

### Dashboard de métriques

**Métriques à suivre :**

1. **Taux de détection sanitizers**
   - Nombre de bugs détectés par sanitizers / semaine
   - Évolution dans le temps

2. **Temps de build**
   - Temps moyen de compilation avec sanitizers
   - Temps CI/CD

3. **Taux de réussite CI/CD**
   - % de builds qui passent
   - Évolution après intégration sanitizers

4. **Couverture de code**
   - % de code testé avec sanitizers

**Script de collecte de métriques :**

```bash
#!/bin/bash
# collect_metrics.sh

DATE=$(date +%Y-%m-%d)  
METRICS_FILE="metrics/${DATE}.json"  

# Créer le dossier si nécessaire
mkdir -p metrics

# Nombre de tests
NUM_TESTS=$(./build/prog_dev --count-tests)

# Temps de build
START=$(date +%s)  
make dev > /dev/null 2>&1  
END=$(date +%s)  
BUILD_TIME=$((END - START))  

# Temps de tests
START=$(date +%s)  
make test > /dev/null 2>&1  
END=$(date +%s)  
TEST_TIME=$((END - START))  

# Générer JSON
cat > "$METRICS_FILE" << EOF
{
    "date": "$DATE",
    "build_time_seconds": $BUILD_TIME,
    "test_time_seconds": $TEST_TIME,
    "num_tests": $NUM_TESTS,
    "sanitizers": ["address", "undefined"]
}
EOF

echo "✅ Métriques enregistrées : $METRICS_FILE"
```

---

## Résumé

### Points clés à retenir

1. **Intégrer les sanitizers à chaque étape** du workflow
2. **Automatiser** avec Makefile, scripts, hooks Git
3. **CI/CD** : Tests avec sanitizers sur chaque commit
4. **Pre-commit hooks** : Validation avant commit
5. **Production** : Toujours compiler **sans** sanitizers

### Checklist d'intégration complète

**Configuration initiale :**
- [ ] Makefile avec targets `dev`, `test`, `release`
- [ ] Scripts d'automatisation dans `scripts/`
- [ ] Hooks Git pre-commit
- [ ] CI/CD configuré (GitHub Actions / GitLab CI)
- [ ] VS Code tasks et launch config (si applicable)

**Workflow quotidien :**
- [ ] Compiler avec sanitizers (`make dev`)
- [ ] Tester régulièrement (`make test`)
- [ ] Pre-commit check (`make pre-commit`)
- [ ] CI/CD passe avant merge

**Production :**
- [ ] Build sans sanitizers (`make release`)
- [ ] Vérification absence de symboles sanitizers
- [ ] Monitoring des erreurs en production

### Commandes essentielles

```bash
# Développement
make dev                    # Build avec sanitizers  
make test                   # Tests rapides  
make pre-commit            # Validation complète  

# CI/CD (local)
./scripts/dev_workflow.sh ci  # Simuler CI

# Production
make release               # Build optimisé
```

### Prochaines étapes

Vous êtes maintenant prêt à :
- ✅ Utiliser les sanitizers efficacement
- ✅ Automatiser leur intégration
- ✅ Détecter les bugs le plus tôt possible
- ✅ Livrer du code plus robuste

Dans les sections suivantes, vous découvrirez :
- **Utilisation de GDB** : Combiner sanitizers et debugging
- **Détection de fuites avec Valgrind** : Outil complémentaire
- **Analyse statique de code** : Détecter les bugs sans exécution

---

**🎯 Un workflow bien intégré transforme les sanitizers d'outils occasionnels en gardiens permanents de la qualité de votre code !**

⏭️ [Utilisation de GDB](/15-debogage-et-qualite/02-utilisation-gdb.md)
