...
Lorsque vous créez un outil en ligne de commande (CLI), la première étape consiste à analyser les arguments passés par l'utilisateur. C'est ce qu'on appelle le **parsing d'arguments**. Un bon parsing d'arguments rend votre outil professionnel et facile à utiliser.
...
Tout programme C commence par la fonction `main()`. Pour recevoir des arguments, on utilise cette signature complète :
...
- **`argc`** (argument count) : Le nombre total d'arguments passés au programme, incluant le nom du programme lui-même.
...
- Les arguments sont toujours des chaînes (`char*`), même pour les nombres
...
- Difficile à maintenir avec beaucoup d'options
...
`getopt()` est une fonction de la bibliothèque standard POSIX qui simplifie grandement le parsing d'arguments. Elle est disponible sur tous les systèmes UNIX/Linux.
...
- `optstring` : Une chaîne définissant les options acceptées

**Variables globales utilisées par getopt :**
- `optarg` : Pointe vers l'argument d'une option (si elle en attend un)
- `optind` : Index du prochain élément à traiter dans `argv`
- `opterr` : Si non-zéro, getopt affiche des messages d'erreur (défaut: 1)
- `optopt` : Contient le caractère d'option inconnu en cas d'erreur

### 2.2 Format de optstring

La chaîne `optstring` définit les options acceptées :

- **`"abc"`** : Les options `-a`, `-b`, `-c` sont acceptées (sans argument)
- **`"a:b:c"`** : `-a` et `-b` attendent un argument, `-c` non
- **`"a::b:c"`** : `-a` a un argument optionnel, `-b` obligatoire, `-c` aucun
- **`":abc"`** : Le `:` initial désactive les messages d'erreur automatiques
...
```c
// Désactiver les messages d'erreur automatiques
opterr = 0;
...
Pour des outils plus professionnels, on utilise souvent des options longues comme `--verbose` en plus des courtes `-v`. C'est le rôle de `getopt_long()`.
...
struct option {
    ...
    int *flag;           // NULL ou pointeur vers variable à modifier
    int val;             // Valeur à retourner (ou à placer dans *flag)
};
```

**Valeurs pour has_arg :**
- `no_argument` (0) : L'option ne prend pas d'argument
- `required_argument` (1) : L'option nécessite un argument
...
```c
// Vérifier qu'un argument numérique est valide
char *endptr;  
...
- **Options** : Liste détaillée avec descriptions
...
void afficher_aide(const char *nom_prog) {
    ...
    printf("Description:\n");
    printf("  Traite les fichiers spécifiés et génère un rapport.\n\n");
    printf("Options:\n");
    printf("  -h, --help              Afficher cette aide et quitter\n");
    printf("  -v, --verbose           Afficher des informations détaillées\n");
...
### 4.5 Gestion des options combinées

`getopt()` supporte automatiquement les options combinées :
...
### 5.1 Argument optionnel avec valeur par défaut

```c
char *config_file = "/etc/myapp.conf";  // Défaut
...
### 5.2 Option compteur (niveau de verbosité)
...
### 6.1 Bibliothèques tierces

Pour des projets plus complexes, considérez :

- **argp** (GNU) : Plus puissant que getopt
- **popt** : Utilisé par RPM et d'autres outils
- **argtable** : Approche déclarative
- **docopt.c** : Génération automatique depuis la documentation

### 6.2 Exemple avec argp (aperçu)

```c
#include <argp.h>

const char *argp_program_version = "programme 1.0";  
static char doc[] = "Programme de démonstration";  
...
// argp offre beaucoup plus de fonctionnalités mais est plus complexe
```
...
| Méthode | Cas d'usage |
|---------|-------------|
| **Parsing manuel** | Script très simple avec 1-2 options |
| **getopt()** | Programme standard avec options courtes |
| **getopt_long()** | Outil professionnel avec options longues |
| **Bibliothèques tierces** | Projets complexes avec sous-commandes |

### Checklist d'un bon parsing d'arguments

- ✅ **Validation** : Vérifier tous les arguments
- ✅ **Aide** : Option `-h/--help` claire et complète
- ✅ **Erreurs** : Messages d'erreur explicites
- ✅ **Standards** : Respecter les conventions POSIX
- ✅ **Retour** : Codes de retour appropriés
...
Le parsing d'arguments est la porte d'entrée de votre outil CLI. Un bon parsing rend votre programme :
- **Professionnel** : Interface cohérente avec les standards UNIX
- **Facile à utiliser** : Options intuitives et bien documentées
- **Robuste** : Gestion d'erreurs appropriée

`getopt_long()` est aujourd'hui le standard de facto pour les outils CLI en C sous Linux. Il offre le meilleur compromis entre simplicité d'utilisation et fonctionnalités.
...
- `getopt_long()` ajoute le support des options longues (recommandé)
- Toujours valider les arguments et gérer les erreurs
- Fournir une aide claire avec `-h/--help`

⏭️ [Interaction avec le système](/34-etudes-cas-devops/01.2-interaction-systeme.md)
