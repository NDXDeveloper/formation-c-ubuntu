...
Bien que `__attribute__((cleanup))` soit extrêmement utile, il est important de comprendre ses **limitations** et ses **impacts sur la portabilité** de votre code. Cette section vous aidera à prendre des décisions éclairées sur quand et comment utiliser cette fonctionnalité.
...
`__attribute__((cleanup))` n'est **pas dans le standard C**. C'est une extension spécifique à GCC (et Clang qui l'a implémentée par compatibilité).

**Standards C officiels :**
- ❌ ANSI C (C89/C90) : Non disponible
- ❌ C99 : Non disponible
- ❌ C11 : Non disponible
- ❌ C17 : Non disponible
- ❌ C23 : Non disponible (toujours pas dans le standard)

**Ce que ça signifie :**

Si vous écrivez du code qui doit être **strictement conforme au standard C**, vous ne pouvez pas utiliser `cleanup`.
...
|-------------|---------|-----------------|-------|
| **GCC** | Toutes versions récentes | ✅ Oui | Support natif et stable |
| **Clang** | 3.0+ | ✅ Oui | Compatible GCC |
| **Intel ICC** | Récentes | ✅ Oui | Compatible GCC |
| **MSVC** | Toutes | ❌ Non | Compilateur Microsoft Windows |
| **TinyCC** | Toutes | ❌ Non | Compilateur léger |
| **CompCert** | Toutes | ❌ Non | Compilateur certifié |
| **Compilateurs embarqués** | Variable | ⚠️ Dépend | IAR, Keil, etc. - à vérifier |
...
### 3. Ordre d'exécution des cleanup

**Le comportement :**

Les fonctions de cleanup sont appelées dans **l'ordre inverse de déclaration** (LIFO - Last In, First Out).
...
**Limitation :** Vous ne pouvez **pas contrôler** cet ordre. Si vous avez besoin d'un ordre spécifique, vous devez :
- Soit ajuster l'ordre de déclaration
- Soit gérer manuellement le nettoyage
...
Les fonctions de cleanup **ne sont PAS appelées** si vous utilisez `longjmp()` pour sortir d'un scope.
...
void fonction_avec_cleanup(void) {
    ...
    if (condition_erreur) {
        longjmp(env, 1);  // ⚠️ cleanup_free NE SERA PAS APPELÉ !
    }

    // cleanup_free sera appelé ici
}
...
int main(void) {
    if (setjmp(env) == 0) {
        ...
        printf("Erreur attrapée\n");
        // ⚠️ La mémoire de buffer a fuité !
    }
    return 0;
}
```
...
`longjmp()` restaure brutalement la pile d'exécution sans dérouler normalement les stack frames. Les destructeurs automatiques ne sont pas appelés.

**Solution :**

Si vous utilisez `setjmp/longjmp`, vous **devez gérer manuellement** les ressources :
...
Créer une fonction de nettoyage pour toute la structure :
...
Les variables globales et statiques n'ont pas de "fin de scope" au sens habituel. Elles existent pendant toute la durée du programme.
...
Avec les optimisations activées (`-O2`, `-O3`), le compilateur peut réorganiser le code, ce qui peut affecter le moment exact où les cleanup sont appelés.
...
void fonction(void) {
    ...
    // plutôt qu'à la fin du scope si buffer n'est plus utilisé

    faire_autre_chose();  // buffer n'est plus référencé
}
```

**Impact :**

En pratique, c'est rarement un problème, mais c'est important de comprendre que le cleanup peut être appelé **dès que la variable n'est plus utilisée**, pas nécessairement à la fin du scope.
...
Si votre code doit tourner sur plusieurs systèmes d'exploitation avec différents compilateurs :

| OS | Compilateur par défaut | Support cleanup |
|----|------------------------|-----------------|
| Linux | GCC | ✅ Oui |
| macOS | Clang | ✅ Oui |
| Windows | MSVC | ❌ Non |
| Windows | MinGW (GCC) | ✅ Oui |
...
// Détection du support de cleanup
...
// Fonctions de cleanup (toujours définies)
...
- ❌ Goto (certains le considèrent moins élégant)
...
#### 3. Nouveau projet où vous contrôlez l'environnement

```c
// Nouveau projet, toolchain moderne, Linux
// Pas de contraintes de portabilité legacy
...
### ❌ N'utilisez PAS cleanup quand :

#### 1. Code multi-compilateur critique

```c
// ❌ MAUVAIS : Bibliothèque publique
// Doit compiler avec MSVC, GCC, Clang, ICC, etc.

// cleanup_lib.h (bibliothèque publique)
// N'utilisez PAS cleanup dans les headers publics !

// ✅ BON : Code portable traditionnel
...
#### 2. Code embarqué avec compilateurs propriétaires

```c
// Microcontrôleurs avec compilateurs spécialisés
// (IAR, Keil, Renesas, etc.)

// ❌ RISQUÉ : cleanup peut ne pas être supporté
_cleanup_free_ uint8_t *buffer = malloc(256);

// ✅ SÛR : gestion manuelle
...
// ❌ NON CONFORME : Extension GCC
_cleanup_free_ char *ptr = malloc(100);

// ✅ CONFORME : C standard pur
char *ptr = malloc(100);  
if (ptr) {  
    // utilisation
    free(ptr);
...
void fonction_critique(void) {
    // ❌ DANGEREUX : cleanup ignoré par longjmp
    _cleanup_free_ char *buffer = malloc(1000);
...
1. Identifiez les fonctions à fort risque de fuites
2. Migrez une fonction à la fois
3. Testez avec Valgrind après chaque migration
...
### 2. Approche hybride (bibliothèque)

**Pattern pour bibliothèques avec API publique :**
...
```c
// lib_internal.c - Implémentation interne (peut utiliser cleanup)
...
int lib_process(lib_context_t *ctx, const char *input) {
    // ✅ OK : Cleanup dans l'implémentation interne
...
- Implémentation interne moderne
- Meilleur des deux mondes

---

## Détection du Support au Runtime
...
static inline void print_features(void) {
    ...
    #if HAS_CLEANUP
        printf("✅ cleanup automatique : Oui\n");
    #else
...
void exemple_scope_guard(void) {
    ...
    if (erreur) {
        return;  // Scope guard PAS exécuté !
    }
}
```

**Problème :** Ne fonctionne que pour un nettoyage unique et pas sur `return` prématuré.
...
```c
// Pattern wrapper pour éviter cleanup
...
void managed_run(void *resource, cleanup_fn cleanup,
                 void (*action)(void *)) {
    action(resource);
    cleanup(resource);  // Toujours appelé
}
...
**Inconvénients :** Très verbeux, difficile à maintenir.
...
```c
// Alternative : Allouer depuis un pool, libérer tout à la fin
typedef struct {
    char *memory;
    size_t used;
    size_t capacity;
...
void fonction_avec_arena(void) {
    ...
    // Une seule libération pour tout
...
**Inconvénients :**
- Toutes les allocations ont la même durée de vie
...
```
Votre code doit-il être portable sur Windows/MSVC ?
├─ OUI → N'utilisez PAS cleanup
│         Utilisez goto cleanup ou wrapper functions
│
└─ NON → Votre code utilise setjmp/longjmp ?
         ├─ OUI → N'utilisez PAS cleanup
         │         Gérez manuellement les ressources
         │
         └─ NON → Vous développez pour Linux/UNIX ?
                  ├─ OUI → ✅ Utilisez cleanup !
                  │         C'est parfait pour votre cas
                  │
                  └─ NON → Évaluez votre toolchain
...
**🟢 Utilisez cleanup sans hésiter :**
- Projets Linux/UNIX exclusifs
- Applications serveur
- Outils système
- Code moderne avec GCC/Clang
- Projets inspirés de systemd/GLib

**🟡 Utilisez avec précaution :**
- Bibliothèques avec API publique (interne OK, publique NON)
...
- Code embarqué avec compilateurs propriétaires
...
# Test avec différents compilateurs
...
| Aspect | Détail |
|--------|--------|
| **Standard** | ❌ Extension GCC, pas dans C standard |
| **GCC/Clang** | ✅ Pleinement supporté et stable |
| **MSVC** | ❌ Non supporté |
| **Performance** | ✅ Aucun overhead |
| **setjmp/longjmp** | ❌ Incompatible, cleanup ignoré |
| **Structures** | ❌ Membres non supportés |
| **Variables globales** | ❌ Non supporté |
| **Portabilité** | ⚠️ Linux/UNIX uniquement |

---

### Alternatives portables

1. **goto cleanup** (recommandé pour portabilité)
2. **Wrapper functions** (verbeux mais sûr)
3. **Arena allocation** (cas spécifiques)
4. **Code dupliqué avec #ifdef** (maintien difficile)

---

### Verdict final

`__attribute__((cleanup))` est une **excellente fonctionnalité** pour :
- ✅ Code Linux/UNIX moderne
- ✅ Projets avec toolchain contrôlé
- ✅ Améliorer la qualité du code C

Mais **ce n'est pas une solution universelle** pour :
- ❌ Code portable multi-plateforme
- ❌ Bibliothèques publiques critiques
- ❌ Environnements contraints

**Conseil :** Si vous développez pour Linux avec GCC/Clang, utilisez cleanup ! Si vous avez des contraintes de portabilité, documentez-les clairement et choisissez l'approche appropriée.
...
- Préparez des fallbacks si nécessaire
- Utilisez des tests automatisés pour vérifier la portabilité

⏭️ [Custom allocators](/24-gestion-memoire-avancee/02-custom-allocators.md)
