...
**clang-tidy** est un outil d'analyse statique et de linting développé par le projet LLVM/Clang. Plus puissant et moderne que cppcheck, il va au-delà de la simple détection de bugs : il propose des suggestions de modernisation du code, vérifie le respect des conventions, et peut même **corriger automatiquement** certains problèmes.

### Qu'est-ce qu'un linter ?

Un **linter** est un outil qui analyse votre code pour :
- Détecter des erreurs potentielles
- Vérifier le respect des conventions de codage
- Suggérer des améliorations de style
- Identifier du code obsolète ou dangereux

**clang-tidy** fait tout cela et plus encore !

### Pourquoi clang-tidy ?

- ✅ **Plus de 400 checks** couvrant tous les aspects du code C/C++
- ✅ **Corrections automatiques** avec l'option `--fix`
- ✅ **Intégration native** avec les compilateurs Clang
- ✅ **Modernisation du code** vers les standards récents (C11, C17, C23)
- ✅ **Extensible** via des plugins personnalisés
- ✅ **Support des bases de données de compilation** (compile_commands.json)
...
### Méthode 2 : Installer une version spécifique

Pour obtenir la dernière version de clang-tidy :
...
Créons un fichier `example.c` avec quelques problèmes :
...
**Note importante :** Le `--` sépare les options de clang-tidy des options du compilateur.
...
clang-tidy a détecté plusieurs problèmes de style et de sécurité !

---

## Catégories de checks

clang-tidy organise ses vérifications en **catégories**. Voici les principales :

### 1. **bugprone-*** : Détection de bugs potentiels

Détecte des patterns de code susceptibles de causer des bugs.

**Exemples :**
- `bugprone-infinite-loop` : Boucles infinies accidentelles
- `bugprone-use-after-move` : Utilisation après déplacement
- `bugprone-signed-char-misuse` : Mauvaise utilisation de char signé

```c
// Exemple détecté par bugprone-infinite-loop
int i = 0;  
while (i < 10) {  
    printf("%d\n", i);
    // Oubli de i++
...
Intègre l'analyseur statique de Clang pour détecter des bugs complexes.

**Exemples :**
- `clang-analyzer-core.NullDereference` : Déréférencement de NULL
- `clang-analyzer-deadcode.DeadStores` : Affectations inutiles
- `clang-analyzer-security.insecureAPI.*` : APIs dangereuses

```c
// Détecté par clang-analyzer-core.NullDereference
int *ptr = NULL;
*ptr = 5;  // Erreur !
```

### 3. **readability-*** : Lisibilité du code

Vérifie que le code respecte les bonnes pratiques de lisibilité.
...
```c
// Détecté par readability-magic-numbers
...
Suggère des améliorations pour utiliser les fonctionnalités modernes du langage.

**Exemples :**
- `modernize-use-nullptr` : Utiliser NULL au lieu de 0
- `modernize-avoid-c-arrays` : Préférer des structures
- `modernize-deprecated-headers` : Headers obsolètes

```c
// Détecté par modernize-use-nullptr
...
Identifie des opportunités d'optimisation.

**Exemples :**
- `performance-unnecessary-copy-initialization` : Copies inutiles
- `performance-for-range-copy` : Copies dans les boucles
- `performance-move-const-arg` : Déplacements constants

```c
// Détecté par performance-unnecessary-copy-initialization
void process(const char *str) {
    char copy[100];
    strcpy(copy, str);  // Copie peut-être inutile
    // Si on ne modifie jamais copy, utiliser directement str
}
```

### 6. **cert-*** : Règles de sécurité CERT

Vérifie la conformité avec les standards de sécurité CERT (Carnegie Mellon).

**Exemples :**
- `cert-err33-c` : Vérification des valeurs de retour
- `cert-str34-c` : Chaînes nullterminées
- `cert-mem30-c` : Gestion mémoire sécurisée

```c
// Détecté par cert-err33-c
malloc(100);  // Pas de vérification du retour !
...
### 7. **misc-*** : Vérifications diverses

Checks qui ne rentrent pas dans les autres catégories.

**Exemples :**
- `misc-unused-parameters` : Paramètres non utilisés
...
⚠️ **Attention :** Cela peut générer beaucoup de warnings !

#### Activer des catégories spécifiques

```bash
# Uniquement les checks de sécurité et de bugs
...
```bash
# Activer bugprone et modernize, mais désactiver modernize-use-trailing-return-type
...
L'option `--fix` applique les corrections suggérées :

```bash
clang-tidy -checks='modernize-use-nullptr' --fix example.c -- -std=c11
```

⚠️ **Attention :** Faites toujours un backup avant d'utiliser `--fix` !
...
Pour les projets complexes, créez un fichier `.clang-tidy` à la racine du projet :
...
Pour les projets utilisant CMake, clang-tidy peut utiliser la base de données de compilation.
...
Cela crée un fichier `compile_commands.json` contenant toutes les commandes de compilation.
...
L'option `-p` indique le répertoire contenant `compile_commands.json`.
...
### Exemple 1 : Détecter les variables non utilisées
...
### Exemple 3 : Vérifier les conventions de nommage
...
int MyFunction() {  // PascalCase non recommandé en C
...
### Exemple 4 : Détecter les APIs dangereuses
...
int main() {
    ...
    // strcpy est dangereux (pas de vérification de taille)
    strcpy(buffer, input);

    // gets est extrêmement dangereux (buffer overflow garanti)
    // gets(buffer);  // Commenté car obsolète
...
| **Corrections auto** | ❌ Non | ✅ Oui (--fix) |
| **Modernisation** | Limitée | Excellente |
| **Intégration compilateur** | Indépendant | Basé sur Clang |
| **Faux positifs** | Peu | Très peu |
...
# Obtenir les fichiers C modifiés
...
echo "🔍 Running clang-tidy on entire codebase..."
...
Les problèmes apparaîtront directement dans l'éditeur !
...
Puis ajoutez progressivement d'autres catégories.

### 2. Fixer en priorité les bugs critiques
...
5. **readability-*** (lisibilité)
6. **performance-*** (optimisation)

### 3. Utiliser --fix avec précaution

Testez toujours après avoir utilisé `--fix` :
...
Si vous désactivez un check, expliquez pourquoi dans le code :

```c
// NOLINTNEXTLINE(cert-err34-c) : Cette conversion est volontaire et sécurisée ici
...
```yaml
# Désactivation de readability-magic-numbers car nous utilisons beaucoup de constantes
# dans les calculs mathématiques et les rendre toutes nommées réduirait la lisibilité
Checks: '*,-readability-magic-numbers'
```

### 5. Créer des configurations par type de code

**Pour du code embarqué (`.clang-tidy-embedded`) :**
...
**Pour des utilitaires système (`.clang-tidy-system`) :**
...
### Problème 1 : "error: unable to handle compilation"
...
- Liste complète des checks : [https://clang.llvm.org/extra/clang-tidy/checks/list.html](https://clang.llvm.org/extra/clang-tidy/checks/list.html)
...
```bash
# Vérifier quelle configuration est appliquée
clang-tidy -dump-config example.c -- -std=c11

# Voir les explications détaillées d'un check
...
- ✅ **Plus de 400 checks** couvrant tous les aspects du code
- ✅ **Corrections automatiques** pour gagner du temps
- ✅ **Modernisation du code** vers les derniers standards
- ✅ **Intégration facile** dans tous les workflows
- ✅ **Configuration flexible** via fichiers YAML
- ✅ **Détection précise** avec très peu de faux positifs
...
**Commande complète pour un projet en production :**
...
**💡 Conseil final :** Combinez clang-tidy avec cppcheck, les sanitizers et Valgrind pour obtenir une **couverture maximale** de la qualité du code. clang-tidy excelle dans la détection de problèmes de style et de modernisation, tandis que Valgrind et les sanitizers détectent les problèmes à l'exécution.

**🎯 Workflow recommandé :**
1. **cppcheck** : Première passe rapide
...
5. **Valgrind** : Vérification finale des fuites mémoire

Avec cette combinaison, votre code C sera de qualité professionnelle ! 🚀

⏭️ [Configuration et intégration continue](/15-debogage-et-qualite/06.3-configuration-ci.md)
