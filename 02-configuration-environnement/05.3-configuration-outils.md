...
- **GCC** : Le compilateur qui transforme votre code en programme exécutable
- **GDB** : Le débogueur pour traquer et corriger les bugs
- **CMake** : Le système de build moderne pour gérer vos projets
- **Valgrind** : L'analyseur mémoire pour détecter les fuites et erreurs
...
#### Méthode 1 : Image pré-configurée (recommandée)
...
#### Méthode 3 : Version spécifique de GCC
...
|-----------|-------------|
| `includePath` | Dossiers où chercher les fichiers `.h` |
...
| `-g` | Inclut les symboles de débogage (pour GDB) |

### Tâche de compilation dans VS Code
...
- Exécuter votre programme pas à pas
- Placer des **breakpoints** (points d'arrêt)
...
#### Avec l'image Microsoft (déjà inclus)
...
|--------|---------------------------|
| `--cap-add=SYS_PTRACE` | Permet à GDB de s'attacher aux processus |
| `--security-opt seccomp=unconfined` | Désactive les restrictions de sécurité qui bloquent le débogage |
...
### Configuration de launch.json pour le débogage
...
```json
{
    ...
    "configurations": [
        ...
        {
            "name": "GDB: Attacher à un processus",
            "type": "cppdbg",
            "request": "attach",
            "program": "${workspaceFolder}/build/mon_programme",
...
| Paramètre | Description |
|-----------|-------------|
| `program` | Chemin vers l'exécutable à déboguer |
| `args` | Arguments passés au programme |
| `stopAtEntry` | S'arrêter au début de `main()` si `true` |
| `preLaunchTask` | Tâche à exécuter avant le débogage (compilation) |
| `miDebuggerPath` | Chemin vers GDB |

### Utilisation du débogueur

1. **Placer un breakpoint** : Cliquez dans la marge gauche à côté d'une ligne
2. **Lancer le débogage** : `F5` ou menu Run → Start Debugging
3. **Contrôles** :
...
### Configuration avancée de GDB

Pour des fonctionnalités avancées, créez un fichier `.gdbinit` à la racine du projet :
...
**Avantages par rapport à un Makefile manuel :**
...
| Extension | Fonction |
|-----------|----------|
| `ms-vscode.cmake-tools` | Intégration CMake complète dans VS Code |
...
### CMakeLists.txt avancé avec tests
...
| Paramètre | Description |
|-----------|-------------|
| `configureOnOpen` | Configure automatiquement à l'ouverture du projet |
| `buildDirectory` | Dossier pour les fichiers générés |
| `generator` | Type de fichiers de build (Makefile, Ninja) |
| `CMAKE_EXPORT_COMPILE_COMMANDS` | Génère un fichier pour IntelliSense |

### Tâches CMake pour VS Code
...
2. `Ctrl+Shift+P` → "CMake: Build" (ou `F7`)
3. `Ctrl+Shift+P` → "CMake: Debug" (ou cliquez sur l'icône de débogage en bas)
...
**Valgrind** est un outil d'analyse mémoire qui détecte :
- **Fuites mémoire** : Mémoire allouée mais jamais libérée
- **Accès invalides** : Lecture/écriture hors limites
...
#### Exemple de code avec problèmes
...
1. **Invalid write** : Écriture en dehors de la mémoire allouée (ligne 12)
2. **uninitialised value** : Utilisation d'une variable non initialisée (ligne 17)
3. **definitely lost** : Mémoire allouée jamais libérée (40 bytes = 10 int)
...
## Configuration complète du DevContainer
...
### Test de compilation et débogage
...
### Valgrind affiche des erreurs système
...
1. Générez `compile_commands.json` avec CMake :
   ```bash
   cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=ON ..
   ```

2. Vérifiez `.vscode/c_cpp_properties.json`
...
- ✅ **GCC** configuré avec les bons flags de compilation
- ✅ **GDB** fonctionnel dans Docker avec les permissions correctes
- ✅ **CMake** intégré à VS Code avec configuration automatique
- ✅ **Valgrind** prêt pour la détection de fuites mémoire
- ✅ **Tasks.json** avec toutes les tâches de build et analyse
- ✅ **Launch.json** pour le débogage dans VS Code
- ✅ Un **DevContainer complet** prêt à l'emploi
...
1. **GCC** : Toujours compiler avec `-Wall -Wextra -g` en développement
2. **GDB** : Nécessite `--cap-add=SYS_PTRACE` dans Docker
...
6. **launch.json** : Configure le débogage visuel dans VS Code
7. **c_cpp_properties.json** : Configure IntelliSense pour la complétion
8. **Testez votre configuration** : Compilez, déboguez et analysez un programme de test
9. **Reconstruisez le conteneur** après modification du `devcontainer.json`
10. **Versionnez** tous les fichiers de configuration (`.devcontainer/`, `.vscode/`)

---


⏭️ [Intégration VS Code et onboarding](/02-configuration-environnement/05.4-integration-vscode.md)
