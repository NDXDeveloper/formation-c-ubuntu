...
**bpftool** est l'outil en ligne de commande officiel du kernel Linux pour **inspecter, gérer, et debugger** les programmes eBPF. Si libbpf est la bibliothèque de développement, bpftool est le "couteau suisse" qui permet de voir et manipuler tout ce qui concerne eBPF sur un système.

Imaginez que vous ayez besoin de :
- Lister tous les programmes eBPF actifs sur votre système
- Inspecter le contenu d'une BPF map
- Debugger pourquoi votre programme eBPF ne se charge pas
- Générer du code skeleton pour libbpf
- Vérifier les informations BTF du kernel

**bpftool fait tout cela et bien plus encore**, et c'est un outil essentiel pour tout ingénieur travaillant avec eBPF.
...
2. **Gérer** : Charger, détacher, épingler des objets eBPF
3. **Debugger** : Examiner le contenu des maps, les stats des programmes
4. **Générer** : Créer des skeletons, extraire des BTF, dumper du bytecode
5. **Analyser** : Profiler les programmes, voir les verifier logs

### Analogie

Si eBPF était un système de fichiers :
- **libbpf** serait la bibliothèque pour écrire/lire des fichiers (comme `fopen`, `fwrite`)
- **bpftool** serait l'équivalent de `ls`, `cat`, `stat`, `find` — les outils pour inspecter et naviguer
...
```bash
# bpftool est généralement inclus dans linux-tools
sudo apt update
sudo apt install -y linux-tools-common linux-tools-generic

# Ou linux-tools pour votre kernel spécifique
...
**Note :** Sur Ubuntu, bpftool peut être dans `/usr/lib/linux-tools/*/bpftool`. Créer un lien symbolique si nécessaire :
...
bpftool utilise une structure hiérarchique intuitive :
...
| Objet | Description |
|-------|-------------|
| **prog** | Programmes eBPF chargés |
| **map** | BPF maps (structures de données) |
| **link** | Attachements de programmes aux hooks |
| **btf** | BPF Type Format (informations de types) |
| **gen** | Génération de code (skeleton, etc.) |
| **cgroup** | Programmes attachés aux cgroups |
| **perf** | Programmes attachés aux événements perf |
| **net** | Programmes réseau (XDP, TC) |
| **iter** | BPF iterators |
| **feature** | Features eBPF supportées par le kernel |
...
| **attach** / **detach** | Attacher/détacher un programme |
| **pin** / **unpin** | Épingler/désépingler dans le filesystem |
| **update** | Modifier une map |
| **lookup** | Rechercher dans une map |
| **create** | Créer un objet |
| **delete** | Supprimer une entrée de map |

---

## bpftool prog : Gérer les programmes eBPF
...
- **xlated** : Taille du bytecode après traduction
- **jited** : Taille du code JIT compilé
- **map_ids** : IDs des maps utilisées
- **btf_id** : ID des informations BTF
- **pids** : Processus qui ont chargé le programme
...
**C'est le code assembleur x86-64 qui s'exécute réellement sur le CPU !**

### Afficher les statistiques d'exécution

```bash
$ sudo bpftool prog show id 23 --json | jq '.run_time_ns, .run_cnt'
1234567890  # Temps total d'exécution (ns)
987654      # Nombre d'exécutions
```

### Épingler un programme dans le filesystem

L'épinglage (pinning) permet de garder un objet eBPF actif même après la fermeture du programme qui l'a créé.

```bash
# Créer le répertoire de pinning (convention)
sudo mkdir -p /sys/fs/bpf/myapp

# Épingler un programme
sudo bpftool prog pin id 23 /sys/fs/bpf/myapp/trace_execve

# Lister les objets épinglés
ls -la /sys/fs/bpf/myapp/
-rw------- 1 root root 0 Nov 27 10:30 trace_execve

# Réutiliser le programme épinglé
...
## bpftool map : Gérer les BPF maps
...
- **Taille clé/valeur**
- **max_entries** : Capacité maximale
- **memlock** : Mémoire verrouillée utilisée
- **pinned** : Chemin si épinglé

### Afficher le contenu d'une map

```bash
# Map de type hash : afficher toutes les entrées
...
$ sudo bpftool map lookup id 1 key 978 0 0 0

# Avec format hexadécimal
...
### Créer une map depuis la ligne de commande
...
```bash
# Obtenir toutes les clés et les supprimer
...
## bpftool link : Gérer les attachements

Les **links** représentent les connexions entre programmes eBPF et leurs points d'attache.
...
### Afficher les détails d'un link
...
```bash
# Attention : cela détache le programme du hook !
...
### Générer un vmlinux.h

```bash
# Extraire toutes les définitions de types en un header C
...
**Ce fichier est essentiel pour développer avec libbpf + CO-RE.**

---

## bpftool gen : Génération de code

### Générer un skeleton

Le skeleton est du code C généré automatiquement pour simplifier l'utilisation de libbpf :
...
### Générer un subskeleton (pour maps partagées)
...
### Générer un object file depuis BTF
...
## bpftool feature : Vérifier les capacités du kernel

### Lister toutes les features supportées
...
**Cas d'usage :** Vérifier la compatibilité avant de déployer un outil eBPF.

---

## bpftool net : Programmes réseau
...
### Détacher un programme XDP

```bash
# Méthode bpftool (à éviter, préférer l'outil qui l'a attaché)
$ sudo bpftool net detach xdp dev eth0
```

---

## bpftool cgroup : Programmes attachés aux cgroups
...
### Afficher les programmes d'un cgroup spécifique
...
### Attacher un programme à un cgroup
...
### Lister les programmes attachés aux événements perf
...
**Informations :**
- **PID** du processus qui a attaché
- **File descriptor** utilisé
- **Type** d'événement
...
**Le log du verifier indique exactement où et pourquoi le programme est rejeté.**

### 2. Monitoring : Quels programmes eBPF tournent sur mon système ?
...
```bash
# Cilium est installé, que fait-il exactement ?
$ sudo bpftool prog list | grep cilium

# Voir les maps utilisées
...
```bash
# Un programme eBPF suspect est chargé (ID 666)
...
# Vérifier les maps auxquelles il accède
...
799.99 ns par exécution
```

### 6. Développement : Workflow avec bpftool

```bash
# 1. Générer vmlinux.h
$ sudo bpftool btf dump file /sys/kernel/btf/vmlinux format c > vmlinux.h

# 2. Développer program.bpf.c
...
# 6. Générer le skeleton pour le code userspace
$ bpftool gen skeleton program.bpf.o > program.skel.h
...
```bash
# Utile pour transférer du bytecode
...
```bash
# Exécuter plusieurs commandes bpftool
$ cat commands.txt
prog list
map list
btf list
...
bpftool nécessite généralement des **privilèges root** ou les capabilities suivantes :

```bash
# Capabilities requises (kernel 5.8+)
CAP_BPF
CAP_PERFMON
CAP_NET_ADMIN  # Pour les programmes réseau
```

### Audit : Qui a chargé quel programme ?

```bash
# Voir l'UID qui a chargé chaque programme
...
### Prévention : Limiter l'accès à eBPF

```bash
# Désactiver eBPF pour les non-root (kernel 5.2+)
...
# Trouver le programme qui utilise le plus de mémoire
$ sudo bpftool prog list --json | jq 'max_by(.bytes_memlock)'
```

### bpftool + watch : Monitoring en temps réel

```bash
# Surveiller les changements de maps en temps réel
...
```bash
# Service systemd pour épingler un programme au boot
...
|--------|---------|-----------|
| **But** | Inspecter/gérer eBPF | Outils spécialisés |
| **Usage** | CLI générique | Scripts Python |
| **Installation** | Inclus kernel | Package séparé |
| **Niveau** | Bas niveau | Haut niveau |
| **Flexibilité** | Très flexible | Cas d'usage fixes |

**Complémentarité :** bpftool pour inspecter, BCC pour utiliser.
...
**Complémentarité :** bpftool pour voir ce qui tourne, bpftrace pour tracer.
...
- **libbpf-bootstrap examples** : Utilisation de bpftool pour le développement
...
bpftool prog pin id X /path    # Épingler
bpftool prog load file.o /path # Charger

bpftool gen skeleton file.o    # Générer skeleton
bpftool btf dump file vmlinux  # Générer vmlinux.h

bpftool feature                # Vérifier support
...
**bpftool** est l'outil indispensable pour tout ingénieur travaillant avec eBPF. C'est le "couteau suisse" qui permet de :

✅ **Inspecter** tout ce qui concerne eBPF sur un système
✅ **Debugger** les problèmes de chargement et d'exécution
✅ **Gérer** les objets eBPF (load, attach, pin)
✅ **Développer** avec génération de skeletons et vmlinux.h
✅ **Monitorer** les programmes en production
✅ **Auditer** l'utilisation d'eBPF pour la sécurité

Que vous développiez vos propres outils eBPF avec libbpf, que vous utilisiez des outils existants comme Cilium ou Falco, ou que vous débugiez des problèmes de performance, **bpftool sera votre compagnon quotidien**.

La maîtrise de bpftool, combinée avec libbpf (section 21.4.1), vous donne toutes les clés pour travailler professionnellement avec eBPF.

Dans la section suivante (21.4.3), nous explorerons la **compilation et le chargement** de programmes eBPF plus en détail, incluant le workflow complet de développement et les optimisations.

---

**💡 À retenir :**
- bpftool est l'outil CLI officiel pour gérer et inspecter eBPF
...
- Génération de skeletons et vmlinux.h pour le développement
- Format JSON pour l'automatisation
- Installé avec linux-tools sur la plupart des distributions

---

⏭️ [Compilation et chargement](/21-introduction-ebpf/04.3-compilation-chargement.md)
