🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 2.5.4 Intégration VS Code et onboarding

## Introduction

Vous avez maintenant un DevContainer correctement configuré avec tous les outils nécessaires (GCC, GDB, CMake, Valgrind). Mais comment faire pour que cette configuration soit **facilement utilisable** par vous-même et par d'autres personnes ?

Cette section couvre :
- L'installation de l'extension Dev Containers dans VS Code
- L'expérience utilisateur lors de l'ouverture d'un projet avec DevContainer
- Comment faciliter l'**onboarding** (l'intégration de nouveaux développeurs)
- Les bonnes pratiques pour une adoption réussie en équipe

**Analogie :** Vous avez construit une belle maison (le DevContainer). Maintenant, il faut créer un chemin d'accès facile, installer une sonnette claire, et préparer un guide d'accueil pour les visiteurs.

---

## Prérequis pour utiliser les DevContainers

### Sur la machine de chaque développeur

Pour utiliser un DevContainer, chaque développeur doit avoir :

1. **Docker** installé et fonctionnel
2. **VS Code** installé
3. **L'extension Dev Containers** installée dans VS Code

C'est tout ! Le reste est géré automatiquement par le DevContainer.

### Installation de Docker

#### Sur Ubuntu

```bash
# Installer Docker
sudo apt-get update  
sudo apt-get install -y docker.io  

# Ajouter l'utilisateur au groupe docker (évite d'utiliser sudo)
sudo usermod -aG docker $USER

# Redémarrer la session pour appliquer les changements
# (déconnectez-vous et reconnectez-vous, ou redémarrez)

# Vérifier l'installation
docker --version  
docker run hello-world  
```

#### Vérification

```bash
# Doit s'exécuter sans sudo et sans erreur
docker ps
```

Si vous obtenez une erreur de permission, redémarrez votre session ou votre ordinateur.

### Installation de l'extension Dev Containers

#### Méthode 1 : Depuis VS Code

1. Ouvrez VS Code
2. Cliquez sur l'icône Extensions (barre latérale gauche) ou `Ctrl+Shift+X`
3. Recherchez "Dev Containers"
4. Installez l'extension **Dev Containers** par Microsoft (identifiant : `ms-vscode-remote.remote-containers`)

#### Méthode 2 : Ligne de commande

```bash
code --install-extension ms-vscode-remote.remote-containers
```

#### Méthode 3 : Pack complet Remote Development

Microsoft propose un pack qui inclut plusieurs extensions utiles :

```bash
code --install-extension ms-vscode-remote.vscode-remote-extensionpack
```

Ce pack inclut :
- **Dev Containers** : Pour les conteneurs de développement
- **Remote - SSH** : Pour développer sur des serveurs distants
- **WSL** : Pour développer dans Windows Subsystem for Linux

### Vérification de l'installation

Après installation, vous devriez voir :
- Une nouvelle icône dans la barre latérale gauche (icône avec `><`)
- Une nouvelle zone dans la barre de statut en bas à gauche (coin vert ou bleu)

---

## Première ouverture d'un DevContainer

### Scénario : Vous clonez un projet existant

Imaginons que vous clonez un projet qui contient déjà une configuration DevContainer.

#### Étape 1 : Cloner le projet

```bash
git clone https://github.com/exemple/projet-c.git  
cd projet-c  
```

#### Étape 2 : Ouvrir dans VS Code

```bash
code .
```

#### Étape 3 : VS Code détecte le DevContainer

Si le projet contient un dossier `.devcontainer/`, VS Code affiche automatiquement une notification :

```
Folder contains a Dev Container configuration file.  
Reopen folder in container?  

[Reopen in Container]  [Don't Show Again]
```

**Cliquez sur "Reopen in Container".**

#### Étape 4 : Construction du conteneur

VS Code :
1. Télécharge l'image Docker de base (si pas déjà présente)
2. Construit le conteneur selon la configuration
3. Exécute les commandes `postCreateCommand`
4. Installe les extensions VS Code dans le conteneur
5. Se connecte au conteneur

**Première fois :** Peut prendre 2-10 minutes selon la connexion internet et la complexité du conteneur.

**Fois suivantes :** Quelques secondes seulement (le conteneur existe déjà).

#### Étape 5 : Vous êtes prêt !

Une fois connecté :
- La barre de statut en bas à gauche affiche le nom du conteneur
- Le terminal intégré s'exécute dans le conteneur
- Toutes les extensions sont installées
- Vous pouvez compiler, déboguer, etc.

### Indicateurs visuels dans VS Code

#### Barre de statut

Quand vous êtes dans un DevContainer, la barre de statut (en bas à gauche) affiche :

```
[Dev Container: Nom du conteneur]
```

Avec une icône distinctive (généralement verte ou bleue).

**Cliquez dessus** pour accéder aux options :
- Rouvrir localement
- Reconstruire le conteneur
- Voir les logs
- etc.

#### Explorateur de fichiers

L'explorateur de fichiers montre les fichiers du projet, mais ils sont accédés depuis le conteneur.

#### Terminal

Le terminal intégré (`Ctrl+` `) s'exécute **dans le conteneur**, pas sur votre machine hôte.

```bash
# Dans le terminal VS Code (dans le conteneur)
whoami
# vscode

cat /etc/os-release
# Ubuntu 22.04...

gcc --version
# gcc (Ubuntu 11.4.0...) 11.4.0
```

---

## Commandes importantes de l'extension Dev Containers

Accédez à toutes les commandes via `Ctrl+Shift+P` puis tapez "Dev Containers".

### Commandes principales

| Commande | Description |
|----------|-------------|
| **Reopen in Container** | Ouvre le dossier actuel dans un DevContainer |
| **Reopen Folder Locally** | Retourne à l'ouverture locale (hors conteneur) |
| **Rebuild Container** | Reconstruit le conteneur from scratch |
| **Rebuild and Reopen in Container** | Reconstruit et rouvre |
| **Open Folder in Container...** | Ouvre un autre dossier dans un conteneur |
| **Clone Repository in Container Volume** | Clone un repo directement dans un conteneur |

### Commandes de diagnostic

| Commande | Description |
|----------|-------------|
| **Show Container Log** | Affiche les logs de création du conteneur |
| **Inspect Container** | Affiche les détails du conteneur Docker |
| **Open Container Configuration File** | Ouvre le `devcontainer.json` |

### Raccourcis pratiques

Pour un accès rapide, cliquez sur l'icône verte/bleue en bas à gauche de VS Code. Un menu contextuel apparaît avec les options principales.

---

## Configurer l'ouverture automatique

### Paramètre recommandé

Pour que VS Code propose systématiquement d'ouvrir les DevContainers, configurez ce paramètre :

Dans vos settings VS Code (`Ctrl+,` → recherchez "dev containers") :

```json
{
    "dev.containers.reopenInContainer": "prompt"
}
```

**Options disponibles :**

| Valeur | Comportement |
|--------|--------------|
| `"prompt"` | Demande à chaque fois (recommandé) |
| `"always"` | Ouvre toujours dans le conteneur |
| `"never"` | N'ouvre jamais automatiquement |

### Forcer l'ouverture depuis la ligne de commande

```bash
# Ouvrir un projet directement dans son DevContainer
code --folder-uri vscode-remote://dev-container+$(printf '%s' '/chemin/vers/projet' | xxd -p)/workspace
```

C'est complexe, donc la méthode simple reste :
```bash
code /chemin/vers/projet
# Puis cliquer sur "Reopen in Container"
```

---

## Faciliter l'onboarding des nouveaux développeurs

L'**onboarding** est le processus d'intégration d'un nouveau membre dans l'équipe. Avec les DevContainers, cela peut être réduit de plusieurs jours à quelques minutes.

### Créer une documentation claire

#### README.md du projet

```markdown
# Mon Projet C

## 🚀 Démarrage rapide

### Prérequis

1. **Docker** : [Instructions d'installation](https://docs.docker.com/get-docker/)
2. **VS Code** : [Télécharger](https://code.visualstudio.com/)
3. **Extension Dev Containers** : Installer depuis VS Code (`ms-vscode-remote.remote-containers`)

### Installation

```bash
# 1. Cloner le projet
git clone https://github.com/votre-org/projet-c.git  
cd projet-c  

# 2. Ouvrir dans VS Code
code .

# 3. Quand VS Code demande "Reopen in Container?", cliquez "Oui"
#    (Ou: Ctrl+Shift+P → "Dev Containers: Reopen in Container")

# 4. Attendez la construction du conteneur (2-5 min la première fois)

# 5. C'est prêt ! Vous pouvez compiler et exécuter.
```

### Compiler et exécuter

```bash
# Option 1 : Avec CMake (recommandé)
mkdir -p build && cd build  
cmake ..  
make  
./mon_programme

# Option 2 : Raccourci VS Code
# Ctrl+Shift+B pour compiler
# F5 pour déboguer
```

### Vérifier l'installation

```bash
# Dans le terminal VS Code (dans le conteneur)
gcc --version      # Doit afficher GCC 11.x  
cmake --version    # Doit afficher CMake 3.x  
valgrind --version # Doit afficher Valgrind 3.x  
```

## 📁 Structure du projet

```
projet-c/
├── .devcontainer/     # Configuration DevContainer  
├── .vscode/           # Configuration VS Code  
├── src/               # Code source  
├── include/           # Headers  
├── tests/             # Tests unitaires  
├── build/             # Fichiers compilés (ignoré par Git)  
└── CMakeLists.txt     # Configuration CMake
```

## 🛠️ Commandes utiles

| Action | Commande |
|--------|----------|
| Compiler | `Ctrl+Shift+B` ou `cmake --build build` |
| Déboguer | `F5` |
| Tester | `cd build && ctest` |
| Valgrind | `valgrind --leak-check=full ./build/programme` |
| Formater | `Ctrl+Shift+I` |

## ❓ Problèmes fréquents

### Le conteneur ne démarre pas

1. Vérifiez que Docker fonctionne : `docker ps`
2. Reconstruisez : `Ctrl+Shift+P` → "Dev Containers: Rebuild Container"

### Permission denied

Sur Linux, ajoutez votre utilisateur au groupe Docker :
```bash
sudo usermod -aG docker $USER
# Puis déconnectez-vous et reconnectez-vous
```
```

### Fichier CONTRIBUTING.md

Créez un fichier `CONTRIBUTING.md` pour les contributeurs :

```markdown
# Guide de contribution

## Environnement de développement

Ce projet utilise un **DevContainer** pour garantir un environnement cohérent.

### Configuration automatique

Tout l'environnement est configuré automatiquement :
- ✅ Compilateur (GCC 11)
- ✅ Débogueur (GDB)
- ✅ Build system (CMake)
- ✅ Analyseur mémoire (Valgrind)
- ✅ Formateur de code (clang-format)
- ✅ Extensions VS Code

**Vous n'avez rien à installer manuellement** (sauf Docker et VS Code).

### Étapes pour contribuer

1. **Fork** le projet sur GitHub
2. **Clonez** votre fork
3. **Ouvrez** dans VS Code avec le DevContainer
4. **Créez** une branche pour votre fonctionnalité
5. **Développez** et testez
6. **Soumettez** une Pull Request

### Standards de code

Le projet utilise clang-format pour le formatage automatique.
- Le code est formaté automatiquement à la sauvegarde
- Style : LLVM avec indentation de 4 espaces
- Limite de colonnes : 80 caractères

### Tests

Avant de soumettre :
```bash
# Compiler
cmake --build build

# Exécuter les tests
cd build && ctest --output-on-failure

# Vérifier les fuites mémoire
valgrind --leak-check=full ./build/mon_programme
```

### Checklist PR

- [ ] Le code compile sans warnings
- [ ] Les tests passent
- [ ] Valgrind ne détecte pas de fuites
- [ ] Le code est formaté (automatique avec formatOnSave)
- [ ] La documentation est mise à jour si nécessaire
```

### Fichier .devcontainer/README.md

Documentez la configuration du DevContainer elle-même :

```markdown
# Configuration DevContainer

Ce dossier contient la configuration de l'environnement de développement.

## Contenu

- `devcontainer.json` : Configuration principale du DevContainer
- `Dockerfile` : (Optionnel) Image Docker personnalisée

## Ce qui est installé

| Outil | Version | Usage |
|-------|---------|-------|
| Ubuntu | 22.04 | Système de base |
| GCC | 11.x | Compilation |
| GDB | 12.x | Débogage |
| CMake | 3.22+ | Build system |
| Valgrind | 3.18+ | Analyse mémoire |
| clang-format | 14+ | Formatage code |
| Git | Latest | Versionnement |

## Extensions VS Code incluses

- C/C++ (IntelliSense, débogage)
- CMake Tools
- Error Lens
- GitLens
- etc.

## Modification

Si vous devez modifier l'environnement :

1. Modifiez `devcontainer.json`
2. Reconstruisez : `Ctrl+Shift+P` → "Rebuild Container"
3. Testez que tout fonctionne
4. Commitez les changements

## Support

En cas de problème :
1. Consultez les logs : `Ctrl+Shift+P` → "Show Container Log"
2. Essayez de reconstruire : `Ctrl+Shift+P` → "Rebuild Container"
3. Ouvrez une issue sur GitHub avec les logs
```

---

## Créer un script d'onboarding

### Script de bienvenue

Créez `.devcontainer/welcome.sh` :

```bash
#!/bin/bash

# Script de bienvenue exécuté au premier démarrage

clear

echo "╔══════════════════════════════════════════════════════════════╗"  
echo "║                                                              ║"  
echo "║   🎉 Bienvenue dans l'environnement de développement !       ║"  
echo "║                                                              ║"  
echo "╚══════════════════════════════════════════════════════════════╝"  
echo ""  
echo "📋 Outils disponibles :"  
echo "   • GCC $(gcc --version | head -1 | awk '{print $4}')"  
echo "   • GDB $(gdb --version | head -1 | awk '{print $4}')"  
echo "   • CMake $(cmake --version | head -1 | awk '{print $3}')"  
echo "   • Valgrind $(valgrind --version | awk '{print $2}')"  
echo ""  
echo "🚀 Commandes rapides :"  
echo "   • Compiler     : Ctrl+Shift+B"  
echo "   • Déboguer     : F5"  
echo "   • Terminal     : Ctrl+\`"  
echo "   • Formater     : Ctrl+Shift+I"  
echo ""  
echo "📁 Structure du projet :"  
echo "   • src/         : Code source"  
echo "   • include/     : Headers"  
echo "   • tests/       : Tests unitaires"  
echo "   • build/       : Fichiers compilés"  
echo ""  
echo "📚 Documentation :"  
echo "   • README.md           : Guide de démarrage"  
echo "   • CONTRIBUTING.md     : Guide de contribution"  
echo ""  
echo "💡 Astuce : Tapez 'make help' pour voir les commandes disponibles"  
echo ""  
echo "═══════════════════════════════════════════════════════════════"  
```

### Intégration dans devcontainer.json

```json
{
    "postAttachCommand": "bash .devcontainer/welcome.sh"
}
```

Ce script s'affichera chaque fois que VS Code se connecte au conteneur.

### Version moins intrusive

Si vous préférez un message discret :

```json
{
    "postAttachCommand": "echo '✓ Environnement prêt ! Tapez make help pour les commandes disponibles.'"
}
```

---

## Gérer les différents niveaux d'utilisateurs

### Utilisateurs débutants

Pour les débutants, fournissez des **guides pas à pas** avec captures d'écran.

#### Guide illustré (dans un Wiki ou documentation)

```markdown
## Installation pour débutants

### Étape 1 : Installer Docker

[Capture d'écran de l'installation Docker]

1. Téléchargez Docker Desktop depuis docker.com
2. Lancez l'installateur
3. Suivez les instructions à l'écran
4. Redémarrez votre ordinateur

### Étape 2 : Installer VS Code

[Capture d'écran]

1. Téléchargez VS Code depuis code.visualstudio.com
2. Installez-le

### Étape 3 : Installer l'extension

[Capture d'écran de l'installation d'extension]

1. Ouvrez VS Code
2. Cliquez sur l'icône des extensions (carré dans la barre de gauche)
3. Tapez "Dev Containers"
4. Cliquez sur "Install"

### Étape 4 : Ouvrir le projet

[Capture d'écran]

1. Ouvrez un terminal
2. Tapez : git clone [URL]
3. Tapez : code [nom-du-projet]
4. Quand VS Code demande "Reopen in Container", cliquez "Oui"
5. Attendez quelques minutes
6. C'est prêt !
```

### Utilisateurs avancés

Pour les utilisateurs avancés, fournissez des informations techniques :

```markdown
## Notes techniques

### Configuration Docker

Le DevContainer utilise les options suivantes :
- `--cap-add=SYS_PTRACE` : Pour GDB
- `--security-opt seccomp=unconfined` : Pour le débogage

### Performance

Pour améliorer les performances sur les gros projets :
- Utilisez un volume Docker pour le dossier build
- Activez BuildKit : `export DOCKER_BUILDKIT=1`

### Personnalisation

Vous pouvez ajouter vos propres extensions dans :
`.vscode/settings.json` (local, non commité)

Ou demander l'ajout d'extensions globales via PR sur `devcontainer.json`.
```

---

## Bonnes pratiques d'équipe

### 1. Un seul DevContainer par projet

Évitez d'avoir plusieurs configurations concurrentes. Un seul `devcontainer.json` bien maintenu.

### 2. Versionnez le DevContainer avec le code

Le `devcontainer.json` doit **toujours** être commité dans Git :

```bash
git add .devcontainer/  
git commit -m "Add/Update DevContainer configuration"  
```

Quand l'environnement évolue, le code et la configuration évoluent ensemble.

### 3. Testez les changements de configuration

Avant de commiter un changement de `devcontainer.json` :

```bash
# Supprimer le conteneur existant
# Ctrl+Shift+P → "Dev Containers: Rebuild Container"

# Vérifier que tout fonctionne
# - Compilation
# - Tests
# - Débogage
```

### 4. Communiquez les changements

Quand vous modifiez le DevContainer :

```markdown
## Changelog - DevContainer

### 2025-01-15
- Ajout de Valgrind
- Mise à jour de GCC vers la version 12
- **Action requise** : Rebuild Container après pull
```

### 5. Utilisez des images stables

Préférez des images avec des versions spécifiques :

```json
{
    "image": "mcr.microsoft.com/devcontainers/cpp:1-ubuntu-22.04"
}
```

Plutôt que :

```json
{
    "image": "mcr.microsoft.com/devcontainers/cpp:latest"
}
```

Les images `latest` peuvent changer et casser la reproductibilité.

### 6. Documentez les raisons des choix

Dans votre `devcontainer.json`, utilisez des commentaires :

```json
{
    "name": "Projet C",

    // Ubuntu 22.04 pour GCC 11 et glibc 2.35
    // Nécessaire pour la compatibilité avec le serveur de production
    "image": "mcr.microsoft.com/devcontainers/cpp:ubuntu-22.04",

    // Valgrind ajouté pour la détection de fuites (requis par la CI)
    "postCreateCommand": "apt-get update && apt-get install -y valgrind",

    // Options nécessaires pour GDB dans Docker
    "runArgs": [
        "--cap-add=SYS_PTRACE",
        "--security-opt", "seccomp=unconfined"
    ]
}
```

---

## Gérer les préférences personnelles

Chaque développeur peut avoir des préférences personnelles (thème, extensions supplémentaires) sans affecter le DevContainer partagé.

### Settings locaux (non commités)

Créez `.vscode/settings.json` et ajoutez-le à `.gitignore` :

```bash
echo ".vscode/settings.json" >> .gitignore
```

Chaque développeur peut avoir ses propres settings :

```json
{
    "workbench.colorTheme": "Monokai",
    "editor.fontSize": 16,
    "terminal.integrated.fontSize": 14
}
```

### Extensions personnelles

Les extensions listées dans `devcontainer.json` sont installées pour tous.

Pour des extensions personnelles, installez-les manuellement après la création du conteneur. Elles persisteront tant que le conteneur existe.

### Dotfiles personnalisés

VS Code permet d'injecter vos dotfiles personnels dans le conteneur.

Dans vos settings VS Code globaux :

```json
{
    "dotfiles.repository": "https://github.com/votre-username/dotfiles",
    "dotfiles.targetPath": "~/dotfiles",
    "dotfiles.installCommand": "install.sh"
}
```

Votre repository `dotfiles` peut contenir :
- `.bashrc` ou `.zshrc` personnalisé
- `.gitconfig`
- Alias personnalisés
- etc.

---

## Automatiser l'onboarding avec des scripts

### Script d'installation complet

Créez un script `setup.sh` à la racine du projet :

```bash
#!/bin/bash

# Script d'installation automatique
# Usage : curl -fsSL https://raw.githubusercontent.com/org/projet/main/setup.sh | bash

set -e

echo "🚀 Installation de l'environnement de développement"  
echo ""  

# Vérifier Docker
if ! command -v docker &> /dev/null; then
    echo "❌ Docker n'est pas installé."
    echo "   Installez Docker : https://docs.docker.com/get-docker/"
    exit 1
fi  
echo "✓ Docker détecté : $(docker --version)"  

# Vérifier VS Code
if ! command -v code &> /dev/null; then
    echo "❌ VS Code n'est pas installé ou pas dans le PATH."
    echo "   Installez VS Code : https://code.visualstudio.com/"
    exit 1
fi  
echo "✓ VS Code détecté : $(code --version | head -1)"  

# Installer l'extension Dev Containers
echo "📦 Installation de l'extension Dev Containers..."  
code --install-extension ms-vscode-remote.remote-containers --force  
echo "✓ Extension installée"  

# Cloner le projet (si pas déjà fait)
PROJECT_DIR="projet-c"  
if [ ! -d "$PROJECT_DIR" ]; then  
    echo "📥 Clonage du projet..."
    git clone https://github.com/org/projet-c.git "$PROJECT_DIR"
fi

# Ouvrir dans VS Code
echo ""  
echo "🎉 Installation terminée !"  
echo ""  
echo "Pour commencer :"  
echo "  cd $PROJECT_DIR"  
echo "  code ."  
echo "  → Cliquez sur 'Reopen in Container' quand demandé"  
echo ""  
```

### Utilisation

Les nouveaux développeurs peuvent exécuter :

```bash
curl -fsSL https://raw.githubusercontent.com/org/projet/main/setup.sh | bash
```

---

## Intégration avec GitHub/GitLab

### Badge dans le README

Ajoutez un badge pour indiquer que le projet supporte les DevContainers :

```markdown
[![Open in Dev Containers](https://img.shields.io/static/v1?label=Dev%20Containers&message=Open&color=blue&logo=visualstudiocode)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/org/projet)
```

### GitHub Codespaces

Si vous utilisez GitHub, les DevContainers fonctionnent aussi avec **GitHub Codespaces** (environnement de dev dans le cloud).

Le même `devcontainer.json` est utilisé. Les développeurs peuvent cliquer sur "Code" → "Open with Codespaces" pour avoir un environnement complet dans leur navigateur.

### Template de repository

Créez un **template repository** avec :
- `.devcontainer/devcontainer.json` pré-configuré
- `.vscode/` avec tasks et launch configs
- `CMakeLists.txt` de base
- `README.md` avec instructions
- Structure de dossiers (`src/`, `include/`, `tests/`)

Les nouveaux projets peuvent démarrer à partir de ce template.

---

## Dépannage de l'onboarding

### Problèmes courants et solutions

#### 1. "Docker daemon is not running"

**Symptôme :** VS Code indique que Docker n'est pas accessible.

**Solutions :**
```bash
# Démarrer Docker
sudo systemctl start docker

# Vérifier que Docker fonctionne
docker ps
```

#### 2. "Permission denied" sur Linux

**Symptôme :** Erreurs de permission lors de l'accès à Docker.

**Solution :**
```bash
# Ajouter l'utilisateur au groupe docker
sudo usermod -aG docker $USER

# Important : se déconnecter et reconnecter
# ou redémarrer l'ordinateur
```

#### 3. Le conteneur met trop de temps à démarrer

**Causes possibles :**
- Première utilisation (téléchargement de l'image)
- Connexion internet lente
- `postCreateCommand` complexe

**Solutions :**
- Pré-télécharger l'image : `docker pull mcr.microsoft.com/devcontainers/cpp:ubuntu-22.04`
- Optimiser `postCreateCommand`
- Utiliser une image pré-construite personnalisée

#### 4. Les extensions ne s'installent pas

**Symptôme :** Les extensions listées dans `devcontainer.json` ne sont pas installées.

**Solutions :**
1. Vérifiez les IDs des extensions (format : `publisher.extension-name`)
2. Reconstruisez le conteneur : `Ctrl+Shift+P` → "Rebuild Container"
3. Consultez les logs : `Ctrl+Shift+P` → "Show Container Log"

#### 5. IntelliSense ne fonctionne pas

**Symptôme :** Pas d'auto-complétion, erreurs partout.

**Solutions :**
1. Attendez que l'indexation se termine (icône de chargement en bas)
2. Vérifiez `c_cpp_properties.json`
3. Générez `compile_commands.json` avec CMake
4. Rechargez VS Code : `Ctrl+Shift+P` → "Developer: Reload Window"

#### 6. Le débogueur ne fonctionne pas

**Symptôme :** Erreur lors du lancement de GDB.

**Vérifications :**
1. Le programme est compilé avec `-g` ?
2. Les `runArgs` sont présents dans `devcontainer.json` ?
   ```json
   "runArgs": ["--cap-add=SYS_PTRACE", "--security-opt", "seccomp=unconfined"]
   ```
3. Le chemin du programme dans `launch.json` est correct ?

### Logs et diagnostic

Pour diagnostiquer les problèmes :

```bash
# Logs de l'extension Dev Containers
# Ctrl+Shift+P → "Dev Containers: Show Container Log"

# Logs Docker
docker logs <container-id>

# État des conteneurs
docker ps -a

# Espace disque Docker
docker system df
```

### Checklist de dépannage

```markdown
## Checklist de dépannage DevContainer

### Docker
- [ ] Docker est installé (`docker --version`)
- [ ] Docker est démarré (`docker ps` fonctionne)
- [ ] L'utilisateur est dans le groupe docker (Linux)
- [ ] Assez d'espace disque (`docker system df`)

### VS Code
- [ ] VS Code est à jour
- [ ] Extension Dev Containers installée
- [ ] Extension C/C++ installée

### Projet
- [ ] Le dossier `.devcontainer/` existe
- [ ] `devcontainer.json` est valide (pas d'erreur de syntaxe)
- [ ] L'image Docker existe ou est accessible

### Connexion
- [ ] Connexion internet active (pour télécharger les images)
- [ ] Pas de proxy/firewall bloquant Docker Hub
```

---

## Workflow quotidien

### Démarrage de journée

```
1. Ouvrir VS Code
2. Ouvrir le projet (Ctrl+O ou via terminal)
3. Si demandé, "Reopen in Container"
4. Attendre quelques secondes
5. Coder !
```

### Fin de journée

```
1. Sauvegarder les fichiers
2. Commit et push si nécessaire
3. Fermer VS Code
4. (Le conteneur s'arrête automatiquement)
```

### Mise à jour de l'environnement

Quand un collègue met à jour le `devcontainer.json` :

```
1. git pull
2. VS Code peut demander de reconstruire
3. Sinon : Ctrl+Shift+P → "Dev Containers: Rebuild Container"
4. Attendre la reconstruction
5. Continuer le travail
```

---

## Récapitulatif

À ce stade, vous savez :

- ✅ Installer les prérequis (Docker, VS Code, extension)
- ✅ Ouvrir un projet dans son DevContainer
- ✅ Utiliser les commandes principales de l'extension
- ✅ Créer une documentation d'onboarding efficace
- ✅ Gérer les préférences personnelles vs partagées
- ✅ Dépanner les problèmes courants
- ✅ Établir un workflow quotidien

**Prochaine étape :** Dans la section 2.5.5, nous verrons comment les DevContainers deviennent la "source de vérité" pour l'environnement de développement et comment les intégrer avec la CI/CD.

---

## Points clés à retenir

1. **Trois prérequis seulement** : Docker, VS Code, extension Dev Containers
2. **"Reopen in Container"** : La commande magique pour entrer dans le DevContainer
3. **Documentation claire** : README avec étapes précises = onboarding réussi
4. **Script de bienvenue** : Guide les nouveaux arrivants
5. **Versionnez le DevContainer** : Il évolue avec le code
6. **Préférences personnelles** : Utilisez `.vscode/settings.json` (non commité)
7. **Logs pour debug** : "Show Container Log" révèle les problèmes
8. **Patience au premier lancement** : Téléchargement de l'image = quelques minutes
9. **Rebuild après modification** : Toujours reconstruire après changement de config
10. **Même environnement pour tous** : C'est l'objectif et le bénéfice principal

**Citation :**
> "The best onboarding is no onboarding. With DevContainers, developers clone and code."
> — Philosophie DevContainer

---


⏭️ [DevContainers comme source de vérité](/02-configuration-environnement/05.5-devcontainers-source-verite.md)
