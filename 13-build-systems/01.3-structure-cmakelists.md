🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 13.1.3 Structure d'un CMakeLists.txt

## Introduction

Un fichier `CMakeLists.txt` bien structuré est comme une recette de cuisine bien écrite : il doit être facile à lire, logique dans son organisation, et permettre de comprendre rapidement ce que fait le projet. Dans cette section, nous allons apprendre à structurer correctement ce fichier, des projets simples aux plus complexes.

## Anatomie d'un CMakeLists.txt minimal

Reprenons notre Hello World et décomposons-le en sections logiques :

```cmake
# ============================================
# Section 1 : Métadonnées du projet
# ============================================
cmake_minimum_required(VERSION 3.15)
project(HelloCMake VERSION 1.0.0 LANGUAGES C)

# ============================================
# Section 2 : Définition des targets
# ============================================
add_executable(hello main.c)
```

Même pour un projet minimal, cette organisation en sections facilite la lecture.

## Les sections d'un CMakeLists.txt complet

Un projet réaliste suit généralement cette structure en **7 sections** :

```cmake
# ============================================
# 1. MÉTADONNÉES ET CONFIGURATION MINIMALE
# ============================================
cmake_minimum_required(VERSION 3.15)
project(MonProjet VERSION 1.2.3 LANGUAGES C)

# ============================================
# 2. OPTIONS DU PROJET
# ============================================
option(BUILD_TESTS "Compiler les tests unitaires" ON)
option(ENABLE_SANITIZERS "Activer AddressSanitizer" OFF)

# ============================================
# 3. CONFIGURATION DU COMPILATEUR
# ============================================
set(CMAKE_C_STANDARD 11)
set(CMAKE_C_STANDARD_REQUIRED ON)
add_compile_options(-Wall -Wextra -pedantic)

# ============================================
# 4. RECHERCHE DES DÉPENDANCES
# ============================================
find_package(OpenSSL REQUIRED)
find_package(Threads REQUIRED)

# ============================================
# 5. DÉFINITION DES TARGETS (BIBLIOTHÈQUES)
# ============================================
add_library(ma_lib
    src/utils.c
    src/parser.c
)
target_include_directories(ma_lib PUBLIC include)

# ============================================
# 6. DÉFINITION DES TARGETS (EXÉCUTABLES)
# ============================================
add_executable(mon_app src/main.c)
target_link_libraries(mon_app ma_lib OpenSSL::SSL Threads::Threads)

# ============================================
# 7. TESTS ET INSTALLATION (OPTIONNEL)
# ============================================
if(BUILD_TESTS)
    enable_testing()
    add_subdirectory(tests)
endif()

install(TARGETS mon_app DESTINATION bin)
```

Passons en revue chaque section en détail.

---

## Section 1 : Métadonnées et configuration minimale

### cmake_minimum_required()

```cmake
cmake_minimum_required(VERSION 3.15)
```

**Obligatoire.** Doit être la **première ligne** du fichier.

**Pourquoi ?**
- Garantit que l'utilisateur a une version compatible de CMake
- Définit le comportement de CMake (les versions ont des politiques différentes)
- Évite les erreurs cryptiques dues à des incompatibilités

**Quelle version choisir ?**

| Version | Date | Recommandation |
|---------|------|----------------|
| 3.10 | 2017 | Minimum absolu (Ubuntu 18.04) |
| 3.15 | 2019 | ✅ **Recommandé pour débuter** (bon compromis) |
| 3.20 | 2021 | Moderne, mais exclut Ubuntu 20.04 par défaut |
| 3.25+ | 2023+ | Très récent, nécessite installation manuelle |

> 💡 **Conseil** : Utilisez 3.15 sauf si vous avez besoin d'une fonctionnalité spécifique d'une version plus récente.

### project()

```cmake
project(MonProjet VERSION 1.0.0 LANGUAGES C)
```

**Obligatoire.** Déclare le nom et les métadonnées du projet.

**Syntaxe complète :**
```cmake
project(<nom>
    VERSION <version>
    DESCRIPTION "<description>"
    LANGUAGES <langages>
)
```

**Exemples :**

```cmake
# Minimal
project(MiniParser C)

# Avec version
project(LogParser VERSION 2.1.4 LANGUAGES C)

# Complet
project(WebServer
    VERSION 3.0.0
    DESCRIPTION "Serveur HTTP léger pour systèmes embarqués"
    LANGUAGES C
)

# Multi-langage
project(MixApp LANGUAGES C CXX)  # C et C++
```

**Ce que fait project() :**
1. Définit des variables CMake utiles :
   - `PROJECT_NAME` → `"MonProjet"`
   - `PROJECT_VERSION` → `"1.0.0"`
   - `PROJECT_SOURCE_DIR` → Chemin du répertoire source
   - `PROJECT_BINARY_DIR` → Chemin du répertoire de build
2. Détecte automatiquement les compilateurs pour les langages spécifiés
3. Initialise l'environnement de build

---

## Section 2 : Options du projet

Les options permettent à l'utilisateur de personnaliser le build :

```cmake
option(BUILD_TESTS "Compiler les tests unitaires" ON)
option(BUILD_DOCS "Générer la documentation Doxygen" OFF)
option(ENABLE_ASAN "Activer AddressSanitizer (debug)" OFF)
option(STATIC_LINKING "Lier statiquement les bibliothèques" OFF)
```

**Syntaxe :**
```cmake
option(<nom> "<description>" <valeur_par_defaut>)
```

**Utilisation :**

L'utilisateur peut activer/désactiver ces options lors de la configuration :

```bash
# Activer les tests
cmake -B build -DBUILD_TESTS=ON

# Désactiver la documentation
cmake -B build -DBUILD_DOCS=OFF

# Activer plusieurs options
cmake -B build -DBUILD_TESTS=ON -DENABLE_ASAN=ON
```

**Utilisation dans le CMakeLists.txt :**

```cmake
option(BUILD_TESTS "Compiler les tests" ON)

# Plus loin dans le fichier...
if(BUILD_TESTS)
    enable_testing()
    add_subdirectory(tests)
endif()
```

**Bonnes pratiques pour les options :**
- Noms en MAJUSCULES avec underscore : `BUILD_TESTS`, `ENABLE_FEATURE`
- Descriptions claires et concises
- Valeur par défaut pertinente (généralement `ON` pour les tests, `OFF` pour les features expérimentales)

---

## Section 3 : Configuration du compilateur

### Standard du langage

```cmake
# Utiliser le standard C11
set(CMAKE_C_STANDARD 11)
set(CMAKE_C_STANDARD_REQUIRED ON)
```

**Explications :**
- `CMAKE_C_STANDARD` : Version du standard C à utiliser (99, 11, 17, 23)
- `CMAKE_C_STANDARD_REQUIRED` : Si `ON`, échoue si le compilateur ne supporte pas ce standard

**Équivalent en flags gcc :**
```cmake
# Ce que CMake génère automatiquement
# gcc -std=c11 ...
```

### Flags de compilation globaux

```cmake
# Ajouter des warnings
add_compile_options(-Wall -Wextra -pedantic)

# Flags spécifiques au mode Debug
if(CMAKE_BUILD_TYPE STREQUAL "Debug")
    add_compile_options(-g -O0)
endif()

# Flags spécifiques au mode Release
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    add_compile_options(-O3 -DNDEBUG)
endif()
```

**Différence entre les méthodes :**

| Méthode | Portée | Usage |
|---------|--------|-------|
| `add_compile_options()` | **Tous les targets** du projet | Flags globaux (warnings, standard) |
| `target_compile_options()` | **Un seul target** | Flags spécifiques à une bibliothèque/exécutable |

**Exemple avec target_compile_options :**
```cmake
add_executable(mon_app main.c)

# Ajouter -DDEBUG_MODE uniquement pour mon_app
target_compile_options(mon_app PRIVATE -DDEBUG_MODE)
```

### Configuration selon le système d'exploitation

```cmake
if(WIN32)
    # Options spécifiques à Windows
    add_compile_definitions(_CRT_SECURE_NO_WARNINGS)
elseif(UNIX)
    # Options spécifiques à Linux/macOS
    add_compile_options(-fPIC)
endif()
```

---

## Section 4 : Recherche des dépendances

Les projets réels utilisent souvent des bibliothèques externes (OpenSSL, libcurl, threads, etc.).

### find_package() : Recherche automatique

```cmake
# Rechercher OpenSSL (optionnel)
find_package(OpenSSL)

# Rechercher OpenSSL (obligatoire, échoue si absent)
find_package(OpenSSL REQUIRED)

# Rechercher plusieurs bibliothèques
find_package(CURL REQUIRED)
find_package(Threads REQUIRED)
find_package(ZLIB)
```

**Ce que fait find_package() :**
1. Cherche la bibliothèque dans les chemins standards du système
2. Si trouvée, définit des variables et des **targets importés** :
   - Pour OpenSSL : `OpenSSL::SSL`, `OpenSSL::Crypto`
   - Pour Threads : `Threads::Threads`
   - Pour CURL : `CURL::libcurl`
3. Si `REQUIRED` et non trouvée : CMake arrête avec une erreur claire

**Vérifier si une bibliothèque a été trouvée :**

```cmake
find_package(ZLIB)

if(ZLIB_FOUND)
    message(STATUS "ZLIB trouvé : ${ZLIB_VERSION}")
else()
    message(WARNING "ZLIB non trouvé, compression désactivée")
endif()
```

### Utilisation des bibliothèques trouvées

```cmake
add_executable(mon_app main.c)

# Lier OpenSSL à mon_app
target_link_libraries(mon_app OpenSSL::SSL OpenSSL::Crypto)
```

CMake gère automatiquement :
- Les chemins d'include (`-I/usr/include/openssl`)
- Les bibliothèques à lier (`-lssl -lcrypto`)
- Les dépendances transitives

---

## Section 5 : Définition des targets (bibliothèques)

### Créer une bibliothèque

```cmake
add_library(ma_lib
    src/utils.c
    src/parser.c
    src/network.c
)
```

**Types de bibliothèques :**

```cmake
# Bibliothèque statique (.a sur Linux)
add_library(ma_lib STATIC src/utils.c)

# Bibliothèque dynamique (.so sur Linux)
add_library(ma_lib SHARED src/utils.c)

# Laisse CMake décider (recommandé)
add_library(ma_lib src/utils.c)
```

### Définir les répertoires d'include

```cmake
add_library(ma_lib src/utils.c src/parser.c)

# Les fichiers .h sont dans include/
target_include_directories(ma_lib PUBLIC include)
```

**Niveaux de visibilité :**

| Mot-clé | Signification | Usage |
|---------|---------------|-------|
| `PUBLIC` | Visible par la lib **et** ceux qui l'utilisent | Headers d'API publique |
| `PRIVATE` | Visible uniquement par la lib elle-même | Headers internes |
| `INTERFACE` | Visible uniquement par ceux qui utilisent la lib | Headers "header-only" |

**Exemple pratique :**

```cmake
add_library(ma_lib
    src/utils.c      # Implémentation
    src/parser.c
)

# API publique : include/mylib/api.h
target_include_directories(ma_lib PUBLIC include)

# Headers internes : src/internal.h
target_include_directories(ma_lib PRIVATE src)
```

Structure du projet :
```
mon_projet/
├── include/
│   └── mylib/
│       └── api.h          # PUBLIC : utilisateurs de ma_lib peuvent l'inclure
├── src/
│   ├── utils.c
│   ├── parser.c
│   └── internal.h         # PRIVATE : seulement pour l'implémentation
└── CMakeLists.txt
```

---

## Section 6 : Définition des targets (exécutables)

### Créer un exécutable

```cmake
add_executable(mon_app src/main.c)
```

### Lier avec des bibliothèques

```cmake
add_executable(mon_app src/main.c)

# Lier avec la bibliothèque interne
target_link_libraries(mon_app ma_lib)

# Lier avec des bibliothèques externes
target_link_libraries(mon_app
    ma_lib           # Notre bibliothèque
    OpenSSL::SSL     # OpenSSL
    Threads::Threads # pthread
    m                # libm (mathématiques)
)
```

### Ajouter des defines

```cmake
add_executable(mon_app src/main.c)

# Définir VERSION_STRING="1.0.0" uniquement pour mon_app
target_compile_definitions(mon_app PRIVATE
    VERSION_STRING="${PROJECT_VERSION}"
    DEBUG_ENABLED
)
```

Cela génère :
```c
// Équivalent en C
#define VERSION_STRING "1.0.0"
#define DEBUG_ENABLED
```

---

## Section 7 : Tests et installation

### Tests avec CTest

```cmake
# Activer le support des tests
enable_testing()

# Ajouter un test
add_test(NAME test_basic COMMAND mon_app --test)

# Ou utiliser add_executable pour les tests
add_executable(test_utils tests/test_utils.c)
target_link_libraries(test_utils ma_lib)
add_test(NAME test_utils COMMAND test_utils)
```

Exécution :
```bash
cmake --build build
ctest --test-dir build
```

### Installation

```cmake
# Installer l'exécutable dans /usr/local/bin
install(TARGETS mon_app DESTINATION bin)

# Installer les headers publics
install(DIRECTORY include/ DESTINATION include)

# Installer la bibliothèque
install(TARGETS ma_lib
    LIBRARY DESTINATION lib
    ARCHIVE DESTINATION lib
)
```

Utilisation :
```bash
cmake --build build
sudo cmake --install build  # Installe dans /usr/local
```

---

## Exemple complet : Projet structuré

Voici un `CMakeLists.txt` complet pour un projet réaliste :

```cmake
# ============================================
# 1. MÉTADONNÉES
# ============================================
cmake_minimum_required(VERSION 3.15)

project(LogParser
    VERSION 2.1.0
    DESCRIPTION "Parser de logs haute performance"
    LANGUAGES C
)

# ============================================
# 2. OPTIONS
# ============================================
option(BUILD_TESTS "Compiler les tests unitaires" ON)
option(BUILD_DOCS "Générer la documentation Doxygen" OFF)
option(ENABLE_ASAN "Activer AddressSanitizer" OFF)

# ============================================
# 3. CONFIGURATION COMPILATEUR
# ============================================
set(CMAKE_C_STANDARD 11)
set(CMAKE_C_STANDARD_REQUIRED ON)

# Warnings
add_compile_options(-Wall -Wextra -pedantic)

# AddressSanitizer (si activé)
if(ENABLE_ASAN)
    add_compile_options(-fsanitize=address -fno-omit-frame-pointer)
    add_link_options(-fsanitize=address)
endif()

# ============================================
# 4. DÉPENDANCES
# ============================================
find_package(Threads REQUIRED)

# PCRE2 pour les regex (optionnel)
find_package(PCRE2)
if(PCRE2_FOUND)
    message(STATUS "PCRE2 trouvé : support des regex activé")
else()
    message(WARNING "PCRE2 non trouvé : regex désactivées")
endif()

# ============================================
# 5. BIBLIOTHÈQUE INTERNE
# ============================================
add_library(logparser_core
    src/parser.c
    src/tokenizer.c
    src/stats.c
)

# Headers publics
target_include_directories(logparser_core PUBLIC
    ${PROJECT_SOURCE_DIR}/include
)

# Headers privés
target_include_directories(logparser_core PRIVATE
    ${PROJECT_SOURCE_DIR}/src
)

# Lier avec les dépendances
target_link_libraries(logparser_core PUBLIC Threads::Threads)

if(PCRE2_FOUND)
    target_link_libraries(logparser_core PUBLIC PCRE2::8BIT)
    target_compile_definitions(logparser_core PUBLIC HAVE_PCRE2)
endif()

# ============================================
# 6. EXÉCUTABLE PRINCIPAL
# ============================================
add_executable(logparser src/main.c)

target_link_libraries(logparser
    logparser_core
    m  # libm pour les calculs statistiques
)

# Version dans le code
target_compile_definitions(logparser PRIVATE
    VERSION="${PROJECT_VERSION}"
)

# ============================================
# 7. TESTS
# ============================================
if(BUILD_TESTS)
    enable_testing()

    # Test du parser
    add_executable(test_parser tests/test_parser.c)
    target_link_libraries(test_parser logparser_core)
    add_test(NAME test_parser COMMAND test_parser)

    # Test du tokenizer
    add_executable(test_tokenizer tests/test_tokenizer.c)
    target_link_libraries(test_tokenizer logparser_core)
    add_test(NAME test_tokenizer COMMAND test_tokenizer)
endif()

# ============================================
# 8. INSTALLATION
# ============================================
install(TARGETS logparser DESTINATION bin)
install(DIRECTORY include/ DESTINATION include)
install(TARGETS logparser_core
    LIBRARY DESTINATION lib
    ARCHIVE DESTINATION lib
)

# ============================================
# 9. INFORMATIONS DE BUILD
# ============================================
message(STATUS "")
message(STATUS "Configuration de LogParser ${PROJECT_VERSION}")
message(STATUS "  Build type:      ${CMAKE_BUILD_TYPE}")
message(STATUS "  Compilateur C:   ${CMAKE_C_COMPILER_ID} ${CMAKE_C_COMPILER_VERSION}")
message(STATUS "  Tests:           ${BUILD_TESTS}")
message(STATUS "  PCRE2:           ${PCRE2_FOUND}")
message(STATUS "  AddressSanitizer: ${ENABLE_ASAN}")
message(STATUS "")
```

Sortie lors de la configuration :
```
-- Configuration de LogParser 2.1.0
  Build type:      Release
  Compilateur C:   GNU 11.4.0
  Tests:           ON
  PCRE2:           TRUE
  AddressSanitizer: OFF

-- Configuring done
-- Generating done
```

---

## Bonnes pratiques de structuration

### 1. Commentaires et sections

```cmake
# ============================================
# TITRE DE SECTION (tout en majuscules)
# ============================================

# Commentaire explicatif sur plusieurs lignes :
# Cette section configure les options de sécurité
# pour le build en production
```

### 2. Ordre des commandes

✅ **Bon ordre** :
```cmake
1. cmake_minimum_required()
2. project()
3. options et variables
4. configuration compilateur
5. find_package()
6. add_library() / add_executable()
7. target_*() pour configurer les targets
8. tests et installation
```

❌ **Mauvais ordre** :
```cmake
add_executable(app main.c)          # ❌ Avant project()
project(MyApp)
cmake_minimum_required(VERSION 3.15) # ❌ Devrait être en premier
```

### 3. Nommage cohérent

```cmake
# Variables : SCREAMING_SNAKE_CASE
set(MY_CUSTOM_PATH "/usr/local")

# Targets : snake_case ou kebab-case
add_library(my_lib src/utils.c)
add_executable(my-app src/main.c)

# Options : SCREAMING_SNAKE_CASE avec préfixe
option(MYPROJECT_BUILD_TESTS "..." ON)
```

### 4. Éviter les chemins absolus

❌ **Mauvais** :
```cmake
target_include_directories(my_lib PUBLIC
    /home/user/projet/include  # ❌ Ne fonctionne que sur votre machine
)
```

✅ **Bon** :
```cmake
target_include_directories(my_lib PUBLIC
    ${PROJECT_SOURCE_DIR}/include  # ✅ Relatif au projet
)
```

### 5. Messages informatifs

```cmake
message(STATUS "Configuration du projet...")
message(WARNING "OpenSSL non trouvé, HTTPS désactivé")
message(FATAL_ERROR "Compilateur C++ requis mais non trouvé")
```

**Types de messages :**
- `STATUS` : Information normale (en blanc/gris)
- `WARNING` : Avertissement (en jaune)
- `FATAL_ERROR` : Erreur bloquante, arrête CMake (en rouge)

---

## Variables CMake utiles

CMake définit automatiquement de nombreuses variables :

### Variables de chemins

```cmake
PROJECT_SOURCE_DIR    # Répertoire du CMakeLists.txt principal
PROJECT_BINARY_DIR    # Répertoire de build (ex: build/)
CMAKE_CURRENT_SOURCE_DIR  # Répertoire du CMakeLists.txt actuel
CMAKE_CURRENT_BINARY_DIR  # Répertoire de build actuel
```

### Variables de configuration

```cmake
CMAKE_BUILD_TYPE      # Debug, Release, RelWithDebInfo, MinSizeRel
CMAKE_C_COMPILER      # Chemin du compilateur C
CMAKE_C_COMPILER_ID   # GNU, Clang, MSVC...
CMAKE_SYSTEM_NAME     # Linux, Windows, Darwin (macOS)
```

### Utilisation

```cmake
message(STATUS "Build directory: ${PROJECT_BINARY_DIR}")

if(CMAKE_BUILD_TYPE STREQUAL "Debug")
    target_compile_definitions(my_app PRIVATE DEBUG_MODE)
endif()

if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
    target_link_libraries(my_app dl)  # libdl uniquement sur Linux
endif()
```

---

## Structure pour projets multi-modules

Pour des projets plus grands, on utilise plusieurs `CMakeLists.txt` :

```
mon_projet/
├── CMakeLists.txt              # Principal (racine)
├── src/
│   ├── CMakeLists.txt         # Pour les sources
│   ├── main.c
│   └── utils.c
├── lib/
│   ├── CMakeLists.txt         # Pour les bibliothèques
│   ├── parser/
│   │   ├── CMakeLists.txt
│   │   └── parser.c
│   └── network/
│       ├── CMakeLists.txt
│       └── network.c
└── tests/
    ├── CMakeLists.txt         # Pour les tests
    └── test_utils.c
```

**CMakeLists.txt principal (racine) :**
```cmake
cmake_minimum_required(VERSION 3.15)
project(MonProjet C)

# Inclure les sous-répertoires
add_subdirectory(lib)
add_subdirectory(src)
add_subdirectory(tests)
```

**lib/CMakeLists.txt :**
```cmake
# Inclure les modules
add_subdirectory(parser)
add_subdirectory(network)
```

**lib/parser/CMakeLists.txt :**
```cmake
add_library(parser parser.c)
target_include_directories(parser PUBLIC ${CMAKE_CURRENT_SOURCE_DIR})
```

Cette organisation permet de maintenir des fichiers CMake lisibles même pour de très gros projets.

---

## Récapitulatif : Checklist d'un bon CMakeLists.txt

✅ **Structure :**
- [ ] `cmake_minimum_required()` en première ligne
- [ ] `project()` avec version et description
- [ ] Sections logiques avec commentaires
- [ ] Ordre cohérent des commandes

✅ **Configuration :**
- [ ] Standard C défini (`CMAKE_C_STANDARD`)
- [ ] Options utilisateur avec `option()`
- [ ] Warnings activés (`-Wall -Wextra`)

✅ **Targets :**
- [ ] Noms explicites pour les targets
- [ ] `target_include_directories()` avec PUBLIC/PRIVATE
- [ ] `target_link_libraries()` pour les dépendances

✅ **Portabilité :**
- [ ] Pas de chemins absolus
- [ ] Variables CMake (`${PROJECT_SOURCE_DIR}`)
- [ ] Dépendances gérées avec `find_package()`

✅ **Maintenance :**
- [ ] Commentaires pour les sections complexes
- [ ] Messages informatifs (`message(STATUS ...)`)
- [ ] Tests activables (`option(BUILD_TESTS ...)`)

---

## Conclusion

La structure d'un `CMakeLists.txt` peut sembler rigide au début, mais elle garantit :
- **Lisibilité** : N'importe qui peut comprendre le projet rapidement
- **Maintenabilité** : Facile d'ajouter de nouvelles fonctionnalités
- **Portabilité** : Fonctionne sur Linux, Windows, macOS sans modification
- **Scalabilité** : Peut gérer des projets de 10 à 10 000 fichiers

Avec cette structure en tête, vous êtes prêt à créer des projets CMake professionnels !

---


⏭️ [Targets, properties et generators](/13-build-systems/01.4-targets-properties.md)
