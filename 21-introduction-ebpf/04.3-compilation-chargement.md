🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 21.4.3 Compilation et Chargement

## Introduction

La **compilation** et le **chargement** d'un programme eBPF sont les étapes cruciales qui transforment votre code C en bytecode exécutable dans le kernel Linux. Comprendre ce processus est essentiel pour :

- Debugger les erreurs de compilation
- Optimiser les performances de vos programmes
- Comprendre les messages d'erreur du verifier
- Choisir les bonnes options de compilation

Ce processus peut sembler complexe au premier abord, mais nous allons le décomposer étape par étape de manière accessible.

---

## Vue d'ensemble du processus

Le parcours d'un programme eBPF, de l'écriture à l'exécution, passe par plusieurs étapes :

```
┌─────────────────────────────────────────────────────────────────┐
│                    1. DÉVELOPPEMENT                             │
├─────────────────────────────────────────────────────────────────┤
│  program.bpf.c (Code source C)                                  │
│  ├─ #include <linux/bpf.h>                                      │
│  ├─ #include <bpf/bpf_helpers.h>                                │
│  └─ SEC("kprobe/...") int my_program(...) { ... }               │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    2. COMPILATION                               │
├─────────────────────────────────────────────────────────────────┤
│  Clang/LLVM                                                     │
│  ├─ Préprocesseur : Expansion des macros, includes              │
│  ├─ Compilation : C → LLVM IR (Intermediate Representation)     │
│  ├─ Optimisation : Passes d'optimisation LLVM                   │
│  └─ Backend BPF : LLVM IR → Bytecode eBPF                       │
│                                                                 │
│  Sortie : program.bpf.o (Fichier objet ELF)                     │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    3. CHARGEMENT (Userspace)                    │
├─────────────────────────────────────────────────────────────────┤
│  libbpf / loader.c                                              │
│  ├─ Lecture du fichier .o (ELF parsing)                         │
│  ├─ Extraction du bytecode eBPF                                 │
│  ├─ Relocation des symboles (CO-RE)                             │
│  ├─ Création des maps                                           │
│  └─ Syscall bpf(BPF_PROG_LOAD, ...)                             │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    4. VÉRIFICATION (Kernel)                     │
├─────────────────────────────────────────────────────────────────┤
│  eBPF Verifier                                                  │
│  ├─ Vérification de la sécurité                                 │
│  │  ├─ Pas de boucles infinies                                  │
│  │  ├─ Pas d'accès mémoire invalides                            │
│  │  ├─ Pas de pointeurs kernel exposés                          │
│  │  └─ Terminaison garantie                                     │
│  ├─ Analyse statique du flot de contrôle                        │
│  └─ Validation des accès maps                                   │
│                                                                 │
│  Si OK → Continuer                                              │
│  Si KO → Rejet avec log détaillé                                │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    5. JIT COMPILATION                           │
├─────────────────────────────────────────────────────────────────┤
│  JIT Compiler (Just-In-Time)                                    │
│  ├─ Bytecode eBPF → Code machine natif (x86-64, ARM64, etc.)    │
│  ├─ Optimisations spécifiques à l'architecture                  │
│  └─ Code prêt à s'exécuter directement sur le CPU               │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    6. ATTACHEMENT                               │
├─────────────────────────────────────────────────────────────────┤
│  Attachement au hook                                            │
│  ├─ Tracepoint, kprobe, uprobe, XDP, TC, etc.                   │
│  ├─ Création d'un bpf_link                                      │
│  └─ Le programme est maintenant actif                           │
└────────────────────────┬────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│                    7. EXÉCUTION                                 │
├─────────────────────────────────────────────────────────────────┤
│  Le programme eBPF s'exécute à chaque fois que l'événement      │
│  auquel il est attaché se produit                               │
└─────────────────────────────────────────────────────────────────┘
```

Explorons maintenant chaque étape en détail.

---

## Étape 1 : Préparation du code source

### Structure d'un programme eBPF minimal

```c
// program.bpf.c
#include <linux/bpf.h>
#include <bpf/bpf_helpers.h>

// Section spéciale qui indique le type de programme et où l'attacher
SEC("kprobe/sys_execve")  
int trace_execve(struct pt_regs *ctx) {  
    // Logique du programme
    bpf_printk("execve() called!\n");
    return 0;
}

// Licence obligatoire (GPL pour accéder à certaines fonctions kernel)
char LICENSE[] SEC("license") = "GPL";
```

### Les includes essentiels

**linux/bpf.h** : Définitions de base eBPF
```c
#include <linux/bpf.h>
// Contient : types eBPF, enums, structures de base
```

**bpf/bpf_helpers.h** : Fonctions helper
```c
#include <bpf/bpf_helpers.h>
// Contient : bpf_printk, bpf_map_lookup_elem, etc.
```

**vmlinux.h** : Types kernel complets (CO-RE)
```c
#include "vmlinux.h"
// Contient : TOUTES les structures kernel (task_struct, socket, etc.)
// Généré via : bpftool btf dump file /sys/kernel/btf/vmlinux format c > vmlinux.h
```

### La macro SEC() : Définir le type de programme

```c
// Syntaxe : SEC("type/hook_name")

// Exemples :
SEC("kprobe/sys_execve")           // Kprobe sur sys_execve  
SEC("tracepoint/sched/sched_process_exec")  // Tracepoint  
SEC("xdp")                         // XDP (attach manuel)  
SEC("tc")                          // Traffic Control  
SEC("cgroup/skb")                  // Cgroup skb filter  
```

**Important :** La section détermine automatiquement le type de programme lors du chargement.

### La licence

```c
char LICENSE[] SEC("license") = "GPL";
```

**Pourquoi GPL ?**
- Certaines fonctions kernel sont "GPL-only"
- Si votre programme les utilise, il DOIT être GPL
- Sinon, vous obtenez une erreur au chargement

**Licences acceptées :**
- `"GPL"`
- `"GPL v2"`
- `"Dual BSD/GPL"`
- `"Dual MIT/GPL"`

---

## Étape 2 : Compilation avec Clang

### Commande de compilation de base

```bash
clang -O2 -target bpf -c program.bpf.c -o program.bpf.o
```

**Décomposition :**
- `clang` : Compilateur LLVM/Clang (requis pour eBPF)
- `-O2` : Niveau d'optimisation (recommandé)
- `-target bpf` : Cible le backend BPF de LLVM
- `-c` : Compiler sans linker (produire un .o)
- `program.bpf.c` : Fichier source
- `-o program.bpf.o` : Fichier de sortie

### Options de compilation importantes

#### 1. **Optimisation (-O2)**

```bash
# Sans optimisation (NE PAS FAIRE)
clang -O0 -target bpf -c program.bpf.c -o program.bpf.o
# Problème : Code non optimisé peut dépasser la limite du verifier

# Avec optimisation (RECOMMANDÉ)
clang -O2 -target bpf -c program.bpf.c -o program.bpf.o
# Bénéfice : Code compact, passe le verifier
```

**Niveaux d'optimisation :**
- `-O0` : Pas d'optimisation (debugging uniquement)
- `-O1` : Optimisations basiques
- `-O2` : Optimisations recommandées (défaut)
- `-O3` : Optimisations agressives (rarement nécessaire)
- `-Os` : Optimiser pour la taille (utile pour programmes complexes)

#### 2. **Debug symbols (-g)**

```bash
clang -O2 -g -target bpf -c program.bpf.c -o program.bpf.o
```

**Bénéfices :**
- ✅ Meilleurs messages d'erreur du verifier
- ✅ Code source visible dans `bpftool prog dump xlated`
- ✅ Debugging plus facile
- ✅ BTF inclus automatiquement

**Recommandation :** Toujours utiliser `-g`, même en production (overhead négligeable).

#### 3. **Include paths (-I)**

```bash
# Ajouter un répertoire d'includes
clang -O2 -target bpf -I./include -c program.bpf.c -o program.bpf.o

# Plusieurs répertoires
clang -O2 -target bpf -I./include -I./headers -c program.bpf.c -o program.bpf.o
```

**Cas d'usage :** Inclure vmlinux.h ou headers custom.

#### 4. **Defines (-D)**

```bash
# Définir une macro de préprocesseur
clang -O2 -target bpf -DDEBUG -c program.bpf.c -o program.bpf.o
```

**Utilisation dans le code :**
```c
#ifdef DEBUG
    bpf_printk("Debug: value=%d\n", value);
#endif
```

#### 5. **Warnings (-Wall, -Wextra)**

```bash
clang -O2 -target bpf -Wall -Wextra -c program.bpf.c -o program.bpf.o
```

**Recommandation :** Toujours activer les warnings.

### Commande complète recommandée

```bash
clang -O2 -g \
    -target bpf \
    -Wall -Wextra \
    -I./include \
    -D__TARGET_ARCH_x86 \
    -c program.bpf.c \
    -o program.bpf.o
```

### Vérifier la compilation

```bash
# Vérifier que c'est bien un objet ELF
$ file program.bpf.o
program.bpf.o: ELF 64-bit LSB relocatable, eBPF, version 1 (SYSV), not stripped

# Voir les sections
$ llvm-objdump -h program.bpf.o
Sections:  
Idx Name          Size     VMA              Type  
  0               00000000 0000000000000000
  1 .text         00000000 0000000000000000 TEXT
  2 kprobe/sys_execve 000000b0 0000000000000000 TEXT
  3 .rodata       0000001d 0000000000000000 DATA
  4 .BTF          00000abc 0000000000000000
  5 .BTF.ext      00000123 0000000000000000

# Voir le bytecode
$ llvm-objdump -d program.bpf.o
```

---

## Étape 3 : Le fichier objet ELF

Le fichier `.bpf.o` est un **fichier ELF** (Executable and Linkable Format) contenant :

### Structure du fichier

```
program.bpf.o (Fichier ELF)
├── Section .text              (vide généralement)
├── Section kprobe/sys_execve  (bytecode eBPF du programme)
├── Section .maps              (définitions des BPF maps)
├── Section .rodata            (données read-only)
├── Section .data              (données read-write)
├── Section .bss               (données non initialisées)
├── Section .BTF               (informations de types)
├── Section .BTF.ext           (extensions BTF pour CO-RE)
├── Section license            (chaîne de licence)
└── Relocation entries         (pour CO-RE)
```

### Inspecter les sections

```bash
# Lister les sections
$ llvm-readelf -S program.bpf.o

Section Headers:
  [Nr] Name              Type            Address          Off    Size
  [ 1] .text             PROGBITS        0000000000000000 000040 000000
  [ 2] kprobe/sys_execve PROGBITS        0000000000000000 000040 0000b0
  [ 3] .rodata           PROGBITS        0000000000000000 0000f0 00001d
  [ 4] license           PROGBITS        0000000000000000 00010d 000004
  [ 5] .maps             PROGBITS        0000000000000000 000120 000020
  [ 6] .BTF              PROGBITS        0000000000000000 000140 000abc
  [ 7] .BTF.ext          PROGBITS        0000000000000000 000bfc 000123
```

### Extraire le bytecode

```bash
# Disassembler le bytecode eBPF
$ llvm-objdump -d program.bpf.o

program.bpf.o:  file format elf64-bpf

Disassembly of section kprobe/sys_execve:

0000000000000000 <trace_execve>:
       0:       85 00 00 00 0e 00 00 00 call 14
       1:       b7 00 00 00 00 00 00 00 r0 = 0
       2:       95 00 00 00 00 00 00 00 exit
```

**Lecture du bytecode :**
- Chaque ligne = 1 instruction (8 bytes)
- Format : `code dst src off imm`
- Exemple : `85 00 00 00 0e 00 00 00` = `call 14` (appeler helper #14)

---

## Étape 4 : Chargement avec libbpf

### Le code userspace minimal

```c
// loader.c
#include <stdio.h>
#include <unistd.h>
#include <bpf/libbpf.h>

int main() {
    struct bpf_object *obj;
    struct bpf_program *prog;
    struct bpf_link *link;
    int err;

    // 1. Ouvrir le fichier objet
    obj = bpf_object__open_file("program.bpf.o", NULL);
    if (libbpf_get_error(obj)) {
        fprintf(stderr, "Failed to open BPF object\n");
        return 1;
    }

    // 2. Charger dans le kernel
    err = bpf_object__load(obj);
    if (err) {
        fprintf(stderr, "Failed to load BPF object: %d\n", err);
        goto cleanup;
    }

    // 3. Trouver le programme
    prog = bpf_object__find_program_by_name(obj, "trace_execve");
    if (!prog) {
        fprintf(stderr, "Failed to find program\n");
        goto cleanup;
    }

    // 4. Attacher
    link = bpf_program__attach(prog);
    if (libbpf_get_error(link)) {
        fprintf(stderr, "Failed to attach program\n");
        goto cleanup;
    }

    printf("Successfully attached! Press Ctrl-C to stop.\n");

    // 5. Garder actif
    pause();  // ou while(1) sleep(1);

    // 6. Cleanup
    bpf_link__destroy(link);
cleanup:
    bpf_object__close(obj);
    return err != 0;
}
```

### Compilation du loader

```bash
gcc -Wall -O2 loader.c -o loader -lbpf -lelf -lz
```

**Bibliothèques requises :**
- `-lbpf` : libbpf
- `-lelf` : libelf (parsing ELF)
- `-lz` : zlib (compression)

### Étapes du chargement en détail

#### 1. **bpf_object__open_file()** : Parsing ELF

```c
obj = bpf_object__open_file("program.bpf.o", NULL);
```

**Ce qui se passe :**
- Lecture du fichier ELF
- Parsing des sections
- Extraction du bytecode eBPF
- Lecture des définitions de maps
- Lecture des informations BTF
- Préparation des relocations CO-RE

#### 2. **bpf_object__load()** : Chargement kernel

```c
err = bpf_object__load(obj);
```

**Ce qui se passe :**

**a. Relocation CO-RE**
```
Si BTF présent :
  ├─ Lire le BTF du kernel (/sys/kernel/btf/vmlinux)
  ├─ Comparer avec le BTF du programme
  ├─ Calculer les offsets corrects pour le kernel actuel
  └─ Appliquer les relocations au bytecode
```

**b. Création des maps**
```
Pour chaque map définie :
  ├─ syscall bpf(BPF_MAP_CREATE, ...)
  ├─ Allouer la mémoire kernel
  └─ Retourner un file descriptor
```

**c. Chargement du programme**
```
syscall bpf(BPF_PROG_LOAD, ...)
  ├─ Passer le bytecode au kernel
  ├─ Spécifier le type de programme
  ├─ Lier avec les maps créées
  └─ Retourner un file descriptor
```

**d. Vérification par le verifier**
```
Le verifier analyse le programme :
  ├─ Pas de boucles infinies ?
  ├─ Accès mémoire valides ?
  ├─ Utilisation correcte des helpers ?
  ├─ Terminaison garantie ?
  └─ Si OK → JIT compilation
     Si KO → Rejet avec log
```

#### 3. **bpf_program__attach()** : Attachement

```c
link = bpf_program__attach(prog);
```

**Ce qui se passe :**
- Lecture de la section SEC() pour déterminer le type
- Attachement automatique selon le type :
  - `kprobe/...` → Attacher au kprobe
  - `tracepoint/...` → Attacher au tracepoint
  - `xdp` → Attacher à une interface (nécessite params)
- Création d'un bpf_link
- Le programme est maintenant actif

---

## Étape 5 : Le verifier eBPF

Le **verifier** est le gardien qui s'assure qu'un programme eBPF est sûr.

### Les vérifications principales

#### 1. **Pas de boucles infinies**

```c
// ❌ REJETÉ : Boucle potentiellement infinie
SEC("kprobe/...")  
int bad_program(struct pt_regs *ctx) {  
    while (1) {  // Boucle infinie !
        // ...
    }
    return 0;
}
```

**Solution :** Utiliser des boucles bornées avec `#pragma unroll` :

```c
// ✅ ACCEPTÉ : Boucle déroulée
SEC("kprobe/...")  
int good_program(struct pt_regs *ctx) {  
    #pragma unroll
    for (int i = 0; i < 10; i++) {  // Maximum 10 itérations
        // ...
    }
    return 0;
}
```

#### 2. **Limite de complexité**

**Le verifier limite le nombre d'instructions analysées.**

```bash
# Kernel ancien (< 5.2) : 4096 instructions
# Kernel 5.2+ : 1 million d'instructions

# Si dépassé :
BPF program is too large. Processed 1000001 insn
```

**Solutions :**
- Simplifier le code
- Utiliser `-O2` ou `-Os`
- Découper en plusieurs programmes

#### 3. **Accès mémoire sûrs**

```c
// ❌ REJETÉ : Accès potentiellement hors limites
SEC("kprobe/...")  
int bad_memory_access(struct pt_regs *ctx) {  
    char buffer[10];
    int index = get_user_input();  // Valeur non contrôlée
    buffer[index] = 'A';  // Peut écrire n'importe où !
    return 0;
}
```

**Solution :** Vérifier les bornes :

```c
// ✅ ACCEPTÉ : Vérification des bornes
SEC("kprobe/...")  
int good_memory_access(struct pt_regs *ctx) {  
    char buffer[10];
    int index = get_user_input();
    if (index >= 0 && index < 10) {  // Vérification explicite
        buffer[index] = 'A';
    }
    return 0;
}
```

#### 4. **Pointeurs valides**

```c
// ❌ REJETÉ : Déréférencement de pointeur non vérifié
SEC("kprobe/...")  
int bad_pointer(struct pt_regs *ctx) {  
    struct task_struct *task = bpf_get_current_task();
    int pid = task->pid;  // Si task est NULL → crash !
    return 0;
}
```

**Solution :** Utiliser les helpers de lecture sécurisée :

```c
// ✅ ACCEPTÉ : Lecture sécurisée
SEC("kprobe/...")  
int good_pointer(struct pt_regs *ctx) {  
    struct task_struct *task = bpf_get_current_task();
    int pid;
    bpf_probe_read_kernel(&pid, sizeof(pid), &task->pid);
    return 0;
}
```

### Interpréter les erreurs du verifier

#### Exemple 1 : Variable non initialisée

```
Error log:
0: (bf) r1 = r10
1: (07) r1 += -8
2: (7a) *(u64 *)(r1 +0) = 0
3: (85) call bpf_map_lookup_elem#1
4: (15) if r0 == 0x0 goto pc+2
 R0_w=map_value(id=1,off=0,ks=4,vs=8,imm=0)
6: (79) r1 = *(u64 *)(r0 +0)
 R0_w=map_value(id=1,off=0,ks=4,vs=8,imm=0)
7: (bf) r2 = r1
invalid read from stack off -8+8 size 8
```

**Traduction :** Lecture d'une variable non initialisée sur la pile.

#### Exemple 2 : Accès hors limites

```
Error log:  
invalid access to packet, off=14 size=4, R1(id=0,off=14,r=14)  
R1 offset is outside of the packet  
```

**Traduction :** Tentative d'accès au-delà de la fin d'un paquet réseau.

### Activer les logs détaillés

```c
// Dans le code C
libbpf_set_print(libbpf_print_fn);

static int libbpf_print_fn(enum libbpf_print_level level,
                           const char *format, va_list args) {
    if (level >= LIBBPF_DEBUG) {
        return vfprintf(stderr, format, args);
    }
    return 0;
}
```

**Ou avec bpftool :**

```bash
sudo bpftool --debug prog load program.bpf.o /sys/fs/bpf/test
```

---

## Étape 6 : JIT Compilation

Une fois le verifier satisfait, le bytecode eBPF est **JIT compilé** en code machine natif.

### Vérifier le JIT

```bash
# Voir si le JIT est activé
$ cat /proc/sys/net/core/bpf_jit_enable
1  # 1 = activé, 0 = désactivé

# Activer si nécessaire
$ sudo sysctl -w net.core.bpf_jit_enable=1
```

### Comparaison : Interprété vs JIT

| Aspect | Interprété (JIT off) | JIT compilé (JIT on) |
|--------|---------------------|----------------------|
| **Performance** | 3-5x plus lent | Proche du code C natif |
| **Démarrage** | Instantané | Compilation (< 1ms) |
| **Portabilité** | Fonctionne partout | Dépend de l'architecture |
| **Usage** | Debugging | Production |

### Voir le code JIT généré

```bash
# Dumper le code assembleur natif
$ sudo bpftool prog dump jited id 23

int trace_execve(struct pt_regs * ctx):  
bpf_prog_a3f21b4c5d6e7f89_trace_execve:  
; int trace_execve(struct pt_regs *ctx) {
   0:   push   %rbp
   1:   mov    %rsp,%rbp
   4:   sub    $0x10,%rsp
   8:   callq  0xffffffffa0001234
  13:   xor    %eax,%eax
  15:   leaveq
  16:   retq
```

**C'est ce code qui s'exécute réellement sur le CPU !**

---

## Workflow complet de développement

### Makefile moderne

```makefile
# Makefile pour projet eBPF

# Outils
CLANG := clang  
BPFTOOL := bpftool  
CC := gcc  

# Flags
BPF_CFLAGS := -O2 -g -target bpf -Wall -Wextra  
BPF_INCLUDES := -I./include -I.  

USER_CFLAGS := -Wall -O2  
USER_LIBS := -lbpf -lelf -lz  

# Fichiers
BPF_SRC := program.bpf.c  
BPF_OBJ := $(BPF_SRC:.bpf.c=.bpf.o)  
SKEL := $(BPF_SRC:.bpf.c=.skel.h)  

USER_SRC := loader.c  
USER_BIN := loader  

# Cible par défaut
all: $(USER_BIN)

# Générer vmlinux.h (une fois)
vmlinux.h:
	$(BPFTOOL) btf dump file /sys/kernel/btf/vmlinux format c > $@

# Compiler le programme eBPF
$(BPF_OBJ): $(BPF_SRC) vmlinux.h
	$(CLANG) $(BPF_CFLAGS) $(BPF_INCLUDES) -c $< -o $@

# Générer le skeleton
$(SKEL): $(BPF_OBJ)
	$(BPFTOOL) gen skeleton $< > $@

# Compiler le loader userspace
$(USER_BIN): $(USER_SRC) $(SKEL)
	$(CC) $(USER_CFLAGS) $< -o $@ $(USER_LIBS)

# Tester le chargement
test: $(BPF_OBJ)
	sudo $(BPFTOOL) prog load $< /sys/fs/bpf/test
	@echo "Success! Cleaning up..."
	sudo rm /sys/fs/bpf/test

# Cleanup
clean:
	rm -f $(BPF_OBJ) $(SKEL) $(USER_BIN)

# Nettoyer tout (y compris vmlinux.h)
distclean: clean
	rm -f vmlinux.h

.PHONY: all test clean distclean
```

### Utilisation

```bash
# Compilation complète
make

# Tester juste le chargement
make test

# Exécuter
sudo ./loader

# Nettoyer
make clean
```

---

## Optimisations de compilation

### 1. **Utiliser -Os pour des programmes complexes**

Si le verifier rejette votre programme pour cause de complexité :

```bash
# Au lieu de -O2
clang -Os -target bpf -c program.bpf.c -o program.bpf.o
```

`-Os` optimise pour la **taille** plutôt que la vitesse, produisant moins d'instructions.

### 2. **Inlining manuel avec __always_inline**

```c
// Forcer l'inlining d'une fonction
static __always_inline int my_helper(int x) {
    return x * 2;
}

SEC("kprobe/...")  
int my_program(struct pt_regs *ctx) {  
    int result = my_helper(42);  // Sera inline
    return 0;
}
```

### 3. **Éviter les fonctions non inline**

```c
// ❌ PROBLÈME : Fonction externe (peut ne pas être inline)
static int external_function(int x) {
    return x * 2;
}

// ✅ SOLUTION : Marquer __always_inline
static __always_inline int inline_function(int x) {
    return x * 2;
}
```

### 4. **Loop unrolling**

```c
// Dérouler les boucles
#pragma unroll
for (int i = 0; i < 10; i++) {
    // Le compilateur génère 10 copies de ce code
}
```

---

## Debugging de la compilation

### Problème 1 : "clang: error: unknown target triple 'bpf'"

**Cause :** Clang installé sans support eBPF.

**Solution :**
```bash
# Vérifier la version de clang
clang --version
# Doit être 10.0+

# Vérifier le support BPF
llc --version | grep bpf

# Réinstaller si nécessaire
sudo apt install clang llvm
```

### Problème 2 : "undefined reference to 'bpf_probe_read'"

**Cause :** Fonction helper pas correctement liée.

**Solution :**
```c
// S'assurer d'inclure les headers
#include <bpf/bpf_helpers.h>

// Utiliser la macro SEC() correctement
SEC("kprobe/...")
```

### Problème 3 : "invalid argument" au chargement

**Cause :** Souvent une section license manquante.

**Solution :**
```c
// Ajouter à la fin du fichier .bpf.c
char LICENSE[] SEC("license") = "GPL";
```

### Problème 4 : Erreurs de relocation CO-RE

**Cause :** BTF non disponible ou incompatible.

**Solution :**
```bash
# Vérifier BTF kernel
ls /sys/kernel/btf/vmlinux

# Si absent, mettre à jour le kernel (5.4+)

# Vérifier BTF dans le .o
llvm-objdump -h program.bpf.o | grep BTF
```

---

## Checklist de déploiement

### Avant la production

- [ ] Code compilé avec `-O2` (ou `-Os`)
- [ ] Debug symbols activés (`-g`)
- [ ] Tous les warnings corrigés (`-Wall -Wextra`)
- [ ] Licence définie (`char LICENSE[] SEC("license") = "GPL"`)
- [ ] Programme passe le verifier
- [ ] Testé sur le kernel cible
- [ ] BTF disponible (pour CO-RE)
- [ ] JIT activé (`bpf_jit_enable=1`)
- [ ] Logging approprié (pas de bpf_printk en prod)
- [ ] Gestion d'erreurs robuste dans le loader

### Tests de compatibilité

```bash
# Les conteneurs Docker partagent le kernel de l'hôte,
# il faut donc utiliser des VMs pour tester différents kernels :
# - VM Ubuntu 20.04 (kernel 5.4)
# - VM Ubuntu 22.04 (kernel 5.15)
# - VM Ubuntu 24.04 (kernel 6.x)

# Dans chaque VM
./loader  # Doit fonctionner partout (CO-RE)
```

---

## Ressources avancées

### Documentation officielle

- **LLVM BPF backend** : https://llvm.org/docs/BPFBackend.html
- **BPF verifier** : https://www.kernel.org/doc/html/latest/bpf/verifier.html
- **libbpf API** : https://libbpf.readthedocs.io/

### Outils utiles

```bash
# Analyser un fichier objet eBPF
llvm-objdump -h program.bpf.o     # Sections  
llvm-objdump -d program.bpf.o     # Disassemblage  
llvm-readelf -a program.bpf.o     # Toutes les infos  
```

### Debugging avancé

```bash
# Tracer le processus de chargement
strace -e bpf ./loader

# Voir tous les syscalls eBPF
sudo bpftrace -e 'tracepoint:syscalls:sys_enter_bpf { printf("%s\n", comm); }'
```

---

## Conclusion

La **compilation et le chargement** d'un programme eBPF peuvent sembler complexes, mais en comprenant chaque étape, le processus devient maîtrisable :

- ✅ **Compilation** : Clang avec `-O2 -g -target bpf`
- ✅ **Fichier objet** : ELF contenant bytecode, maps, BTF
- ✅ **Chargement** : libbpf lit l'ELF, applique CO-RE, charge dans le kernel
- ✅ **Vérification** : Le verifier valide la sécurité
- ✅ **JIT** : Compilation en code natif pour performance maximale
- ✅ **Attachement** : Le programme devient actif sur le hook

La clé du succès est de :
- Toujours compiler avec `-O2 -g`
- Comprendre les messages du verifier
- Tester sur différents kernels (CO-RE)
- Utiliser un Makefile moderne
- Activer les logs en cas de problème

Avec ces connaissances, vous êtes maintenant capable de compiler, charger, et debugger n'importe quel programme eBPF. Dans la section suivante (21.5), nous créerons notre premier programme eBPF complet en pratique.

---

**💡 À retenir :**
- Compilation : `clang -O2 -g -target bpf -c program.bpf.c -o program.bpf.o`
- Le verifier valide la sécurité (pas de boucles infinies, accès mémoire sûrs)
- CO-RE rend les programmes portables entre versions kernel
- JIT compilation pour performances natives
- libbpf simplifie le chargement et la gestion
- Toujours tester le chargement avec `make test` ou bpftool

---

⏭️ [Premier programme eBPF](/21-introduction-ebpf/05-premier-programme.md)
