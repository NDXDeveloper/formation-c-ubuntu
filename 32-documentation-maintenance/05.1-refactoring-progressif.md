🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 32.5.1 Refactoring progressif

## Introduction

Vous héritez d'un projet C écrit il y a 10 ans. Le code fonctionne, mais c'est un cauchemar :
- Fonctions de 500 lignes
- Variables nommées `a`, `b`, `tmp`, `data`
- Aucune documentation
- Macros partout
- Goto dans tous les sens
- Allocation mémoire anarchique

Votre première réaction : *"Il faut tout réécrire !"*

**STOP** 🛑

Réécrire complètement un projet legacy est presque toujours une **mauvaise idée**. C'est long, risqué, et vous allez reproduire les mêmes bugs (plus de nouveaux). La bonne approche est le **refactoring progressif** : améliorer le code petit à petit, sans tout casser.

Dans cette section, nous allons apprendre :
- Pourquoi le refactoring progressif est préférable à la réécriture
- Les techniques de refactoring pour le code C legacy
- Comment refactoriser en toute sécurité
- Les patterns de refactoring les plus utiles
- Comment mesurer les progrès

---

## Le mythe de la grande réécriture

### Pourquoi "tout réécrire" échoue souvent

**Histoire vraie** : Netscape a décidé en 1998 de tout réécrire. Résultat :
- 3 ans de développement perdus
- Concurrence (Internet Explorer) a pris le marché
- Le projet a failli couler l'entreprise
- Joel Spolsky a écrit un article célèbre : "Things You Should Never Do"

**Problèmes de la réécriture complète** :

1. **Sous-estimation** : "Ça prendra 3 mois" → 2 ans en réalité
2. **Perte de connaissances** : Le vieux code contient des années de bugfixes
3. **Bugs réintroduits** : Tous les bugs corrigés au fil des ans reviennent
4. **Nouveaux bugs** : Le nouveau code en ajoute de nouveaux
5. **Fonctionnalités oubliées** : Des cas limites gérés par l'ancien code sont oubliés
6. **Coût d'opportunité** : Pendant la réécriture, zéro nouvelle fonctionnalité

### L'approche progressive

**Principe** : Améliorer le code **petit à petit**, en gardant le système fonctionnel à chaque étape.

```
Code legacy (fonctionnel mais laid)
    ↓
Refactoring 1 : Renommer les variables
    ↓
Refactoring 2 : Extraire des fonctions
    ↓
Refactoring 3 : Simplifier la logique
    ↓
...
    ↓
Code propre (fonctionnel et maintenable)
```

**Avantages** :
- ✅ Système toujours fonctionnel
- ✅ Améliorations continues et visibles
- ✅ Risques réduits (changements isolés)
- ✅ Connaissance préservée (bugfixes conservés)
- ✅ Possibilité d'arrêter à tout moment (code déjà meilleur)

---

## Principes du refactoring sûr

### 1. Le code doit toujours compiler et fonctionner

**Règle d'or** : Après chaque étape de refactoring, le code doit :
- ✅ Compiler sans erreurs ni warnings
- ✅ Passer tous les tests
- ✅ Se comporter exactement comme avant

**Workflow** :
```bash
# 1. Faire un petit refactoring
vim src/parser.c

# 2. Compiler
make clean && make

# 3. Tester
make test

# 4. Si OK, commiter
git add src/parser.c  
git commit -m "refactor: rename variable 'a' to 'buffer_size'"  

# 5. Répéter
```

### 2. Petits pas, pas géants

❌ **Mauvais** : Refactoriser 10 fichiers d'un coup
```
Commit : "Refactor everything"
- 20 files changed
- 2847 lines added, 2314 lines deleted
```

✅ **Bon** : Un changement logique à la fois
```
Commit 1: "refactor: rename variable 'n' to 'element_count'"
- 1 file changed
- 15 lines changed

Commit 2: "refactor: extract validate_input() function"
- 1 file changed
- 45 lines added, 30 lines deleted

Commit 3: "refactor: simplify error handling with early returns"
- 1 file changed
- 23 lines changed
```

### 3. Tests avant et après

**Idéal** : Avoir des tests automatisés

```bash
# Avant refactoring
make test
# ✅ All tests pass (12/12)

# Refactoring
vim src/parser.c

# Après refactoring
make test
# ✅ All tests pass (12/12)
```

**Si pas de tests** : Tester manuellement de manière rigoureuse
```bash
# Créer un script de test manuel
cat > test_manual.sh << 'EOF'
#!/bin/bash
./mon_programme test1.txt > output1.txt
diff output1.txt expected1.txt || exit 1

./mon_programme test2.txt > output2.txt
diff output2.txt expected2.txt || exit 1

echo "✅ All manual tests pass"  
EOF  

chmod +x test_manual.sh
./test_manual.sh
```

### 4. Conserver le comportement exact

**Refactoring** ≠ **Changement de fonctionnalité**

```c
// AVANT (legacy)
int parse(char *input) {
    int i, n = 0;
    for (i = 0; input[i]; i++) {
        if (input[i] == ',') n++;
    }
    return n;
}

// APRÈS (refactorisé) - MÊME comportement
int count_commas(const char *input) {
    if (input == NULL) return 0;  // ⚠️ ATTENTION : nouveau comportement !

    int comma_count = 0;
    for (int i = 0; input[i] != '\0'; i++) {
        if (input[i] == ',') {
            comma_count++;
        }
    }
    return comma_count;
}
```

**Problème** : Le check `NULL` change le comportement ! L'ancien code crashait sur `NULL`, le nouveau retourne 0.

**Solution** : Si vous voulez ajouter le check `NULL`, faites-le dans un commit séparé :

```bash
# Commit 1 : Refactoring pur (comportement identique)
git commit -m "refactor: rename and clarify parse() to count_commas()"

# Commit 2 : Amélioration (nouveau comportement)
git commit -m "feat: add NULL check in count_commas()"
```

### 5. Documenter l'intention

Dans les commits de refactoring, expliquez **pourquoi** :

❌ **Mauvais** :
```bash
git commit -m "refactor: change function"
```

✅ **Bon** :
```bash
git commit -m "refactor: extract validate_input() for clarity

The parse() function was 300 lines long and hard to understand.  
Extracting input validation into a separate function improves  
readability and makes it easier to add validation rules."  
```

---

## Techniques de refactoring pour le code C

### 1. Renommage de variables et fonctions

**Problème** : Noms cryptiques ou trompeurs

```c
// AVANT
int f(int a, int b, char *s) {
    int n = a * b;
    char *p = s;
    // ...
}
```

**Solution** : Noms descriptifs

```c
// APRÈS
int calculate_buffer_size(int width, int height, char *format) {
    int total_pixels = width * height;
    char *format_ptr = format;
    // ...
}
```

**Stratégie** :
1. Identifier les pires noms (a, b, tmp, data, n, i utilisés hors boucles)
2. Renommer un par un
3. Compiler et tester après chaque renommage
4. Utiliser l'outil de refactoring de votre IDE (Ctrl+R dans VS Code)

**Outils** :
```bash
# Rechercher tous les usages d'une variable
grep -rn "variable_a_renommer" src/

# Renommer avec sed (prudence !)
sed -i 's/\bvariable_a_renommer\b/nouveau_nom/g' src/fichier.c
```

### 2. Extraction de fonction

**Problème** : Fonctions trop longues (>50 lignes)

```c
// AVANT (fonction de 200 lignes)
int process_data(char *input) {
    // 50 lignes : validation de l'input
    if (input == NULL) return -1;
    if (strlen(input) < 10) return -1;
    // ... 48 lignes de validation ...

    // 80 lignes : parsing
    char *token = strtok(input, ",");
    // ... 78 lignes de parsing ...

    // 70 lignes : traitement
    for (int i = 0; i < count; i++) {
        // ... 68 lignes de traitement ...
    }

    return 0;
}
```

**Solution** : Extraire des sous-fonctions

```c
// APRÈS
static int validate_input(const char *input) {
    if (input == NULL) return -1;
    if (strlen(input) < 10) return -1;
    // ... 48 lignes de validation ...
    return 0;
}

static int parse_tokens(char *input, token_t **tokens, int *count) {
    char *token = strtok(input, ",");
    // ... 78 lignes de parsing ...
    return 0;
}

static int process_tokens(token_t *tokens, int count) {
    for (int i = 0; i < count; i++) {
        // ... 68 lignes de traitement ...
    }
    return 0;
}

int process_data(char *input) {
    if (validate_input(input) < 0) return -1;

    token_t *tokens = NULL;
    int count = 0;
    if (parse_tokens(input, &tokens, &count) < 0) return -1;

    if (process_tokens(tokens, count) < 0) {
        free(tokens);
        return -1;
    }

    free(tokens);
    return 0;
}
```

**Avantages** :
- Chaque fonction a une responsabilité claire
- Plus facile à tester
- Plus facile à comprendre
- Réutilisable

**Stratégie** :
1. Identifier les blocs logiques dans la fonction
2. Copier le premier bloc dans une nouvelle fonction
3. Remplacer le bloc par un appel à la nouvelle fonction
4. Compiler et tester
5. Répéter pour les autres blocs

### 3. Simplification de la logique

**Problème** : Logique complexe et imbriquée

```c
// AVANT
int check_valid(int a, int b, int c) {
    if (a > 0) {
        if (b > 0) {
            if (c > 0) {
                if (a + b > c && b + c > a && a + c > b) {
                    return 1;
                } else {
                    return 0;
                }
            } else {
                return 0;
            }
        } else {
            return 0;
        }
    } else {
        return 0;
    }
}
```

**Solution** : Early returns (retours anticipés)

```c
// APRÈS
int is_valid_triangle(int side_a, int side_b, int side_c) {
    // Vérifications de base
    if (side_a <= 0) return 0;
    if (side_b <= 0) return 0;
    if (side_c <= 0) return 0;

    // Inégalité triangulaire
    if (side_a + side_b <= side_c) return 0;
    if (side_b + side_c <= side_a) return 0;
    if (side_a + side_c <= side_b) return 0;

    return 1;
}
```

**Autre exemple** : Inverser les conditions

```c
// AVANT (logique inversée, difficile à lire)
if (!error && data != NULL) {
    process(data);
}

// APRÈS (logique positive, plus claire)
if (error || data == NULL) {
    return -1;  // Early return
}
process(data);
```

### 4. Remplacement de magic numbers

**Problème** : Nombres littéraux sans signification

```c
// AVANT
if (status == 3) {
    buffer = malloc(1024);
    for (int i = 0; i < 256; i++) {
        // ...
    }
}
```

**Solution** : Constantes nommées

```c
// APRÈS
#define STATUS_READY 3
#define BUFFER_SIZE 1024
#define MAX_ITERATIONS 256

if (status == STATUS_READY) {
    buffer = malloc(BUFFER_SIZE);
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        // ...
    }
}
```

**Encore mieux** : Enum pour les états

```c
typedef enum {
    STATUS_IDLE = 0,
    STATUS_INITIALIZING = 1,
    STATUS_RUNNING = 2,
    STATUS_READY = 3,
    STATUS_ERROR = 4
} status_t;

#define BUFFER_SIZE 1024
#define MAX_ITERATIONS 256

if (status == STATUS_READY) {
    buffer = malloc(BUFFER_SIZE);
    for (int i = 0; i < MAX_ITERATIONS; i++) {
        // ...
    }
}
```

### 5. Élimination de code mort

**Problème** : Code inutilisé qui pollue

```c
// AVANT
int old_parse(char *input) {
    // Fonction qui n'est plus appelée nulle part
    return 0;
}

#if 0  // Code commenté depuis des années
void ancient_function() {
    // ...
}
#endif

int current_parse(char *input) {
    // Nouvelle fonction utilisée
    // ...

    // int old_result = old_parse(input);  // Code commenté

    return 0;
}
```

**Solution** : Supprimer impitoyablement

```c
// APRÈS
int current_parse(char *input) {
    // ...
    return 0;
}
```

**Justification** :
- Git conserve l'historique : vous pouvez retrouver l'ancien code si besoin
- Le code mort perturbe la lecture
- Il peut contenir des bugs qui n'affectent personne mais inquiètent

**Stratégie** :
```bash
# Trouver les fonctions non utilisées
cppcheck --enable=unusedFunction src/

# Ou avec GCC (lors de la compilation)
gcc -Wall -Wextra -Wunused src/*.c
```

### 6. Réduction de la portée des variables

**Problème** : Variables déclarées trop tôt

```c
// AVANT
void process() {
    int i, j, k;
    char buffer[1024];
    FILE *file;

    // 100 lignes de code...

    // 'i' n'est utilisé que maintenant
    for (i = 0; i < 10; i++) {
        // ...
    }

    // 50 lignes de code...

    // 'file' n'est utilisé que maintenant
    file = fopen("data.txt", "r");
}
```

**Solution** : Déclarer au plus près de l'utilisation

```c
// APRÈS
void process() {
    // Code...

    // Déclaration juste avant utilisation
    for (int i = 0; i < 10; i++) {  // C99+ permet la déclaration dans la boucle
        // ...
    }

    // Plus de code...

    FILE *file = fopen("data.txt", "r");
    if (file == NULL) {
        // Gestion erreur
        return;
    }
    // Utilisation de file
    fclose(file);
}
```

### 7. Remplacement de goto par des structures de contrôle

**Problème** : goto rend le flux difficile à suivre

```c
// AVANT
int process(char *data) {
    char *buffer = NULL;
    FILE *file = NULL;
    int ret = 0;

    if (data == NULL) {
        ret = -1;
        goto cleanup;
    }

    buffer = malloc(1024);
    if (buffer == NULL) {
        ret = -1;
        goto cleanup;
    }

    file = fopen("output.txt", "w");
    if (file == NULL) {
        ret = -1;
        goto cleanup;
    }

    // Traitement...

cleanup:
    if (buffer) free(buffer);
    if (file) fclose(file);
    return ret;
}
```

**Solution** : Extraction de fonctions et early returns

```c
// APRÈS
static char* allocate_buffer(void) {
    char *buffer = malloc(1024);
    if (buffer == NULL) {
        fprintf(stderr, "Failed to allocate buffer\n");
    }
    return buffer;
}

static FILE* open_output_file(void) {
    FILE *file = fopen("output.txt", "w");
    if (file == NULL) {
        fprintf(stderr, "Failed to open file\n");
    }
    return file;
}

int process(char *data) {
    if (data == NULL) {
        return -1;
    }

    char *buffer = allocate_buffer();
    if (buffer == NULL) {
        return -1;
    }

    FILE *file = open_output_file();
    if (file == NULL) {
        free(buffer);
        return -1;
    }

    // Traitement...

    fclose(file);
    free(buffer);
    return 0;
}
```

**Note** : Parfois, goto est acceptable pour le nettoyage en C. Si vous gardez goto, limitez-le à `goto cleanup` à la fin :

```c
int process(char *data) {
    char *buffer = NULL;
    FILE *file = NULL;
    int ret = -1;

    if (data == NULL) goto cleanup;

    buffer = malloc(1024);
    if (buffer == NULL) goto cleanup;

    file = fopen("output.txt", "w");
    if (file == NULL) goto cleanup;

    // Traitement...
    ret = 0;  // Succès

cleanup:
    free(buffer);  // free(NULL) est safe
    if (file) fclose(file);
    return ret;
}
```

### 8. Unification de la gestion d'erreurs

**Problème** : Plusieurs façons de gérer les erreurs

```c
// AVANT
int func1() {
    if (error) return -1;  // Retourne -1
}

int func2() {
    if (error) return 0;   // Retourne 0 (!)
}

int func3() {
    if (error) return NULL;  // Retourne NULL
}

void func4() {
    if (error) exit(1);  // Exit brutal
}
```

**Solution** : Convention cohérente

```c
// APRÈS : Convention unifiée
// 0 = succès, -1 = erreur, errno set

int func1() {
    if (error) {
        errno = EINVAL;
        return -1;
    }
    return 0;
}

int func2() {
    if (error) {
        errno = EIO;
        return -1;
    }
    return 0;
}

// Pour les fonctions retournant pointeur :
// NULL = erreur, errno set
char* func3() {
    if (error) {
        errno = ENOMEM;
        return NULL;
    }
    return result;
}
```

---

## Stratégie de refactoring progressif

### Phase 1 : Préparation (1-2 jours)

#### 1.1 Comprendre le code existant

```bash
# Analyser la structure du projet
tree -L 3 src/

# Compter les lignes de code
cloc src/

# Identifier les fichiers les plus gros
find src/ -name "*.c" -exec wc -l {} \; | sort -rn | head -10

# Trouver les fonctions les plus longues
cscope -R -b
# Ou utiliser ctags
```

#### 1.2 Ajouter des tests (si absent)

**Priorité** : Tests de non-régression

```c
// tests/regression_test.c
// Capture le comportement actuel (même s'il est buggé)

void test_parse_current_behavior() {
    // Test 1 : Cas normal
    char *result = parse("input1");
    assert(strcmp(result, "expected1") == 0);

    // Test 2 : Cas limite
    result = parse("");
    assert(result == NULL);  // Comportement actuel

    // Test 3 : Cas d'erreur
    result = parse(NULL);
    // Actuellement, ça crash - mais on documente ce comportement
    // Pour l'instant, on skip ce test
}
```

#### 1.3 Créer une branche de refactoring

```bash
git checkout -b refactor/cleanup-parser
```

### Phase 2 : Quick wins (1 semaine)

Commencer par les refactorings faciles et visibles :

#### 2.1 Renommer les variables cryptiques

```bash
# Jour 1-2 : Renommer dans parser.c
git commit -m "refactor: rename 'n' to 'token_count' in parser.c"  
git commit -m "refactor: rename 'buf' to 'line_buffer' in parser.c"  
```

#### 2.2 Ajouter des constantes

```bash
# Jour 3 : Remplacer magic numbers
git commit -m "refactor: replace magic numbers with named constants"
```

#### 2.3 Simplifier les conditions

```bash
# Jour 4 : Early returns
git commit -m "refactor: simplify error handling with early returns"
```

#### 2.4 Supprimer le code mort

```bash
# Jour 5 : Nettoyage
git commit -m "refactor: remove unused functions and dead code"
```

### Phase 3 : Restructuration (2-4 semaines)

#### 3.1 Extraction de fonctions

**Principe** : Une fonction = une responsabilité

```c
// Avant : fonction monolithique
void process_file(const char *filename) {
    // 300 lignes qui font tout
}

// Après : découpé en sous-fonctions
void process_file(const char *filename) {
    FILE *file = open_and_validate_file(filename);
    if (file == NULL) return;

    char *content = read_file_content(file);
    fclose(file);
    if (content == NULL) return;

    data_t *parsed = parse_content(content);
    free(content);
    if (parsed == NULL) return;

    process_data(parsed);
    free_data(parsed);
}
```

#### 3.2 Créer des types clairs

```c
// Avant : paramètres multiples et confus
int process(char *a, int b, char *c, int d, int e);

// Après : structure qui encapsule
typedef struct {
    char *input_file;
    char *output_file;
    int buffer_size;
    int max_iterations;
    int verbosity_level;
} process_config_t;

int process(const process_config_t *config);
```

#### 3.3 Modulariser le code

```
Avant :  
src/  
  └── main.c  (5000 lignes)

Après :  
src/  
  ├── main.c       (200 lignes)
  ├── parser.c     (800 lignes)
  ├── processor.c  (600 lignes)
  ├── output.c     (400 lignes)
  └── utils.c      (300 lignes)
```

### Phase 4 : Optimisation (2 semaines)

Une fois le code propre, optimiser :

```c
// Exemple : Optimisation après refactoring
// Avant refactoring : code illisible et lent
// Après refactoring : code lisible mais toujours lent
// Maintenant : on peut optimiser en toute confiance

// Version lisible (après refactoring)
for (int i = 0; i < count; i++) {
    if (is_valid(data[i])) {  // Fonction extraite
        process(data[i]);
    }
}

// Version optimisée (après avoir profilé)
// On découvre que is_valid() est appelé trop souvent
bool valid[count];  
for (int i = 0; i < count; i++) {  
    valid[i] = is_valid(data[i]);
}
for (int i = 0; i < count; i++) {
    if (valid[i]) {
        process(data[i]);
    }
}
```

---

## Mesurer les progrès

### Métriques de qualité du code

#### 1. Complexité cyclomatique

Mesure le nombre de chemins dans le code.

```bash
# Avec lizard
pip install lizard  
lizard src/  

# Résultat :
# Function: parse_line
# NLOC: 45  CCN: 12  <-- Complexité cyclomatique
```

**Interprétation** :
- CCN 1-10 : Simple, facile à tester
- CCN 11-20 : Complexe, difficile à tester
- CCN 21+ : Très complexe, refactoriser impérativement

**Objectif** : Réduire le CCN moyen du projet

#### 2. Longueur des fonctions

```bash
# Script pour trouver les fonctions longues
cat > find_long_functions.sh << 'EOF'
#!/bin/bash
for file in src/*.c; do
    # Simplification : compte les lignes entre { et }
    awk '/^[a-zA-Z_].*\(.*\)/ { fname=$1; start=NR }
         /^}/ { if (start) print fname, NR-start, FILENAME; start=0 }' "$file"
done | sort -k2 -rn | head -20  
EOF  

chmod +x find_long_functions.sh
./find_long_functions.sh
```

**Objectif** : Aucune fonction >100 lignes

#### 3. Couverture de tests

```bash
# Avec gcov/lcov
gcc -fprofile-arcs -ftest-coverage src/*.c -o test
./test
gcov src/*.c  
lcov --capture --directory . --output-file coverage.info  
genhtml coverage.info --output-directory coverage_html  
```

**Objectif** : >80% de couverture

#### 4. Nombre de warnings

```bash
# Compiler avec warnings maximaux
make clean  
make CFLAGS="-Wall -Wextra -Wpedantic -Werror"  
```

**Objectif** : Zéro warning

### Tableau de bord de progrès

```markdown
# Refactoring Progress Report

## Semaine 1 (2025-01-08)
- ✅ Renommer variables : parser.c, utils.c
- ✅ Supprimer code mort : 450 lignes
- ✅ Ajouter constantes : 23 magic numbers → constantes

Métriques :
- Longueur moy. fonctions : 87 → 65 lignes
- CCN moyen : 15.2 → 12.8
- Warnings : 34 → 12

## Semaine 2 (2025-01-15)
- ✅ Extraction fonctions : process_data() découpé en 5 fonctions
- ✅ Simplification : 15 early returns ajoutés
- 🚧 En cours : Modularisation de network.c

Métriques :
- Longueur moy. fonctions : 65 → 48 lignes
- CCN moyen : 12.8 → 9.4
- Warnings : 12 → 3

## Objectifs semaine 3
- [ ] Finir modularisation network.c
- [ ] Ajouter tests unitaires pour parser
- [ ] Réduire CCN des 5 fonctions restantes >15
```

---

## Gérer les risques

### Risque 1 : Casser le code en production

**Mitigation** :
```bash
# 1. Tests de régression automatisés
make test

# 2. Tests manuels exhaustifs
./run_manual_tests.sh

# 3. Review du code
git diff main..refactor/cleanup-parser | less

# 4. Déploiement progressif
# - D'abord en staging
# - Puis en production avec feature flag
# - Rollback facile si problème
```

### Risque 2 : Refactoring qui n'en finit pas

**Mitigation** :
- ⏰ Time-box : 1 mois maximum
- 🎯 Objectifs clairs et mesurables
- 🚦 Critères d'arrêt définis :
  ```
  Stop quand :
  - CCN moyen < 10
  - Fonctions < 50 lignes
  - 80% couverture de tests
  - 0 warning compilation
  ```

### Risque 3 : Conflit avec les nouvelles features

**Mitigation** :
```bash
# Option 1 : Freezer les features pendant le refactoring
# - Période dédiée au refactoring (2-4 semaines)
# - Pas de nouvelles features

# Option 2 : Refactoring continu
# - Règle : Toucher un fichier → le nettoyer d'abord
# - "Leave the code better than you found it"

# Option 3 : Branche long-term
git checkout -b refactor/long-term
# Merge régulier de main dans la branche
git merge main
# Puis merge de refactor quand prêt
```

---

## Bonnes pratiques

### 1. Boy Scout Rule

> *"Leave the code cleaner than you found it"* (Laisse le code plus propre que tu ne l'as trouvé)

Principe : À chaque fois que vous touchez un fichier, améliorez-le un peu.

```bash
# Vous devez corriger un bug dans old_code.c
vim src/old_code.c

# Avant de corriger le bug :
# 1. Renommer 2-3 variables mal nommées
# 2. Extraire une petite fonction si possible
# 3. Ajouter des commentaires

# Puis corriger le bug

# Commit 1 : refactoring
git commit -m "refactor: improve readability of old_code.c"

# Commit 2 : bugfix
git commit -m "fix: correct buffer overflow in parse_line()"
```

### 2. Refactoriser avec un buddy

Refactoriser à deux réduit les erreurs :
- Un code, l'autre review en temps réel
- Discussion sur les meilleures approches
- Détection immédiate des erreurs

### 3. Refactoriser en dehors des deadlines

❌ **Mauvais timing** :
```
Release dans 2 jours
→ "On a le temps de refactoriser ce gros fichier !"
→ Bug introduit
→ Panic, rush, dette technique accrue
```

✅ **Bon timing** :
```
Release vient de sortir
→ Période calme
→ Temps pour refactoriser tranquillement
→ Tests approfondis
→ Prêt pour la prochaine release
```

### 4. Documenter les décisions

```c
/**
 * parse_line() - Parse une ligne de configuration
 *
 * REFACTORING NOTES (2025-01-15):
 * Cette fonction a été refactorisée de 200 à 50 lignes.
 *
 * Changements :
 * - Extraction de validate_line() pour la validation
 * - Extraction de split_tokens() pour le parsing
 * - Simplification de la logique avec early returns
 *
 * Ancien comportement préservé, sauf :
 * - Maintenant retourne NULL sur input NULL (avant : segfault)
 * - errno est maintenant set correctement
 *
 * Tests : tests/test_parser.c (95% coverage)
 */
int parse_line(const char *line);
```

### 5. Garder les anciens tests

```c
// tests/legacy_behavior_test.c
// Ces tests documentent l'ancien comportement
// Même si le comportement était buggé, on le documente

void test_old_behavior_empty_string() {
    // LEGACY: L'ancienne version retournait -1 sur chaîne vide
    // CURRENT: On retourne 0 (chaîne vide est valide)

    int result = parse("");
    assert(result == 0);  // Nouveau comportement

    // Ancien comportement documenté :
    // assert(result == -1);
}
```

---

## Exemple complet : Refactoriser une fonction legacy

### Étape 0 : Code initial

```c
// parser.c - Code legacy (écrit en 2010)
int p(char *s) {
    int i, j, k = 0, n = 0;
    char *t, *p, b[1024];

    if (!s) return -1;

    for (i = 0; s[i]; i++) {
        if (s[i] == ',') n++;
    }

    t = malloc((n + 1) * sizeof(char*));

    p = s;
    j = 0;
    for (i = 0; s[i]; i++) {
        if (s[i] == ',') {
            b[j] = 0;
            ((char**)t)[k++] = strdup(b);
            j = 0;
        } else {
            b[j++] = s[i];
        }
    }

    if (j > 0) {
        b[j] = 0;
        ((char**)t)[k] = strdup(b);
    }

    // ... traitement de t ...

    return 0;
}
```

**Problèmes** :
- Noms cryptiques (p, s, t, n, k, j, i, b)
- Logique difficile à suivre
- Pas de gestion d'erreur malloc
- Buffer overflow possible (b[1024])
- Magic numbers
- Responsabilités multiples

### Étape 1 : Ajouter des tests

```c
// tests/test_parser.c
void test_parse_comma_separated() {
    // Test actuel : documenter le comportement
    int result = p("apple,banana,cherry");
    assert(result == 0);

    result = p("single");
    assert(result == 0);

    result = p(NULL);
    assert(result == -1);
}
```

### Étape 2 : Renommer

```c
// Commit : "refactor: rename cryptic variables in p()"
int p(char *input_string) {
    int char_index, buffer_index, token_index = 0, comma_count = 0;
    char *token_array, *string_ptr, temp_buffer[1024];

    if (!input_string) return -1;

    // Compter les virgules
    for (char_index = 0; input_string[char_index]; char_index++) {
        if (input_string[char_index] == ',') comma_count++;
    }

    // Allouer tableau de tokens
    token_array = malloc((comma_count + 1) * sizeof(char*));

    // Parser les tokens
    string_ptr = input_string;
    buffer_index = 0;
    for (char_index = 0; input_string[char_index]; char_index++) {
        if (input_string[char_index] == ',') {
            temp_buffer[buffer_index] = '\0';
            ((char**)token_array)[token_index++] = strdup(temp_buffer);
            buffer_index = 0;
        } else {
            temp_buffer[buffer_index++] = input_string[char_index];
        }
    }

    if (buffer_index > 0) {
        temp_buffer[buffer_index] = '\0';
        ((char**)token_array)[token_index] = strdup(temp_buffer);
    }

    // ... traitement de token_array ...

    return 0;
}
```

### Étape 3 : Renommer la fonction

```c
// Commit : "refactor: rename p() to parse_comma_separated_values()"
int parse_comma_separated_values(char *input_string) {
    // ... même code ...
}
```

### Étape 4 : Extraire count_commas

```c
// Commit : "refactor: extract count_commas() helper"

static int count_commas(const char *input_string) {
    int comma_count = 0;
    for (int i = 0; input_string[i] != '\0'; i++) {
        if (input_string[i] == ',') {
            comma_count++;
        }
    }
    return comma_count;
}

int parse_comma_separated_values(char *input_string) {
    if (!input_string) return -1;

    int comma_count = count_commas(input_string);

    // Allouer tableau de tokens
    token_array = malloc((comma_count + 1) * sizeof(char*));

    // ... reste du code ...
}
```

### Étape 5 : Ajouter gestion d'erreur

```c
// Commit : "refactor: add error handling for malloc()"

int parse_comma_separated_values(char *input_string) {
    if (input_string == NULL) {
        errno = EINVAL;
        return -1;
    }

    int comma_count = count_commas(input_string);

    char **token_array = malloc((comma_count + 1) * sizeof(char*));
    if (token_array == NULL) {
        errno = ENOMEM;
        return -1;
    }

    // ... reste du code ...
}
```

### Étape 6 : Extraire parse_tokens

```c
// Commit : "refactor: extract parse_tokens() helper"

#define MAX_TOKEN_LENGTH 1024

static char** parse_tokens(const char *input_string, int *token_count) {
    *token_count = count_commas(input_string) + 1;

    char **tokens = malloc(*token_count * sizeof(char*));
    if (tokens == NULL) {
        return NULL;
    }

    char temp_buffer[MAX_TOKEN_LENGTH];
    int buffer_index = 0;
    int token_index = 0;

    for (int i = 0; input_string[i] != '\0'; i++) {
        if (input_string[i] == ',') {
            temp_buffer[buffer_index] = '\0';
            tokens[token_index++] = strdup(temp_buffer);
            buffer_index = 0;
        } else {
            if (buffer_index < MAX_TOKEN_LENGTH - 1) {
                temp_buffer[buffer_index++] = input_string[i];
            }
        }
    }

    if (buffer_index > 0) {
        temp_buffer[buffer_index] = '\0';
        tokens[token_index] = strdup(temp_buffer);
    }

    return tokens;
}

int parse_comma_separated_values(char *input_string) {
    if (input_string == NULL) {
        errno = EINVAL;
        return -1;
    }

    int token_count;
    char **tokens = parse_tokens(input_string, &token_count);
    if (tokens == NULL) {
        errno = ENOMEM;
        return -1;
    }

    // ... traitement des tokens ...

    // Libérer mémoire
    for (int i = 0; i < token_count; i++) {
        free(tokens[i]);
    }
    free(tokens);

    return 0;
}
```

### Étape 7 : Version finale propre

```c
// parser.c - Code refactorisé (2025)

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>

#define MAX_TOKEN_LENGTH 1024

/**
 * count_commas() - Compte le nombre de virgules dans une chaîne
 * @input: Chaîne d'entrée (ne doit pas être NULL)
 *
 * Return: Nombre de virgules
 */
static int count_commas(const char *input) {
    int count = 0;
    for (int i = 0; input[i] != '\0'; i++) {
        if (input[i] == ',') {
            count++;
        }
    }
    return count;
}

/**
 * parse_tokens() - Parse une chaîne CSV en tokens
 * @input: Chaîne d'entrée (ne doit pas être NULL)
 * @token_count: Pointeur où stocker le nombre de tokens
 *
 * Alloue un tableau de chaînes (à libérer par l'appelant).
 *
 * Return: Tableau de tokens, ou NULL si erreur (errno set)
 */
static char** parse_tokens(const char *input, int *token_count) {
    *token_count = count_commas(input) + 1;

    char **tokens = malloc(*token_count * sizeof(char*));
    if (tokens == NULL) {
        return NULL;
    }

    char temp_buffer[MAX_TOKEN_LENGTH];
    int buffer_index = 0;
    int token_index = 0;

    for (int i = 0; input[i] != '\0'; i++) {
        if (input[i] == ',') {
            // Fin d'un token
            temp_buffer[buffer_index] = '\0';
            tokens[token_index] = strdup(temp_buffer);
            if (tokens[token_index] == NULL) {
                // Échec strdup : libérer ce qui a été alloué
                for (int j = 0; j < token_index; j++) {
                    free(tokens[j]);
                }
                free(tokens);
                return NULL;
            }
            token_index++;
            buffer_index = 0;
        } else {
            // Ajouter caractère au buffer (avec protection overflow)
            if (buffer_index < MAX_TOKEN_LENGTH - 1) {
                temp_buffer[buffer_index++] = input[i];
            }
        }
    }

    // Dernier token (s'il reste des caractères)
    if (buffer_index > 0) {
        temp_buffer[buffer_index] = '\0';
        tokens[token_index] = strdup(temp_buffer);
        if (tokens[token_index] == NULL) {
            for (int j = 0; j < token_index; j++) {
                free(tokens[j]);
            }
            free(tokens);
            return NULL;
        }
    }

    return tokens;
}

/**
 * parse_comma_separated_values() - Parse une chaîne CSV
 * @input: Chaîne d'entrée au format "val1,val2,val3"
 *
 * Parse la chaîne et traite chaque token.
 *
 * Return: 0 en cas de succès, -1 en cas d'erreur (errno set)
 */
int parse_comma_separated_values(const char *input) {
    if (input == NULL) {
        errno = EINVAL;
        return -1;
    }

    int token_count;
    char **tokens = parse_tokens(input, &token_count);
    if (tokens == NULL) {
        errno = ENOMEM;
        return -1;
    }

    // Traitement des tokens
    for (int i = 0; i < token_count; i++) {
        printf("Token %d: %s\n", i, tokens[i]);
        // ... traitement ...
    }

    // Libération mémoire
    for (int i = 0; i < token_count; i++) {
        free(tokens[i]);
    }
    free(tokens);

    return 0;
}
```

**Résultat** :
- ✅ Noms clairs et explicites
- ✅ Fonctions courtes et focalisées
- ✅ Gestion d'erreur complète
- ✅ Protection buffer overflow
- ✅ Documentation Doxygen
- ✅ Comportement préservé (tests passent)
- ✅ Libération mémoire correcte

---

## Conclusion

Le refactoring progressif est un **art** autant qu'une **science**. Les principes clés :

1. **Petits pas** : Changements incrémentaux
2. **Tests** : Vérifier après chaque étape
3. **Commits atomiques** : Un changement logique = un commit
4. **Patience** : Rome ne s'est pas faite en un jour
5. **Pragmatisme** : Savoir quand arrêter

**Citation de Martin Fowler** (auteur du livre de référence *"Refactoring"*) :

> *"Any fool can write code that a computer can understand. Good programmers write code that humans can understand."*

Le refactoring progressif transforme le code que **les machines comprennent** en code que **les humains comprennent**.

---

**Section suivante** : 32.5.2 Ajout de tests - Nous verrons comment ajouter des tests à du code legacy pour sécuriser le refactoring.

⏭️ [Ajout de tests](/32-documentation-maintenance/05.2-ajout-tests.md)
