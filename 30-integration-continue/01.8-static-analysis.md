🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 30.1.8 Static analysis

## Introduction

L'**analyse statique** (static analysis) est l'examen automatique de votre code source **sans l'exécuter**. C'est comme avoir un expert qui relit votre code et signale les bugs potentiels, les problèmes de style, les vulnérabilités de sécurité et les mauvaises pratiques avant même la compilation.

Contrairement aux tests dynamiques (sanitizers, Valgrind) qui détectent les problèmes pendant l'exécution, l'analyse statique trouve des bugs **avant** que le code ne soit compilé ou testé. C'est votre **premier filet de sécurité**.

---

## Pourquoi l'analyse statique est essentielle ?

### Les avantages

1. **Détection précoce** : Trouve les bugs avant la compilation et l'exécution
2. **Couverture complète** : Analyse tout le code, même les chemins rarement exécutés
3. **Aucun runtime** : Pas besoin d'exécuter le programme
4. **Qualité du code** : Détecte les mauvaises pratiques et améliore la lisibilité
5. **Sécurité** : Identifie les vulnérabilités potentielles
6. **Économie de temps** : Détecte les problèmes avant qu'ils ne deviennent coûteux

### Ce que l'analyse statique détecte

| Catégorie | Exemples |
|-----------|----------|
| **Bugs logiques** | Conditions toujours vraies/fausses, code mort |
| **Problèmes mémoire** | Fuites potentielles, use-after-free |
| **Comportements indéfinis** | Division par zéro, débordements |
| **Mauvaises pratiques** | Variables non initialisées, fonctions inutilisées |
| **Vulnérabilités** | Buffer overflows, format string bugs |
| **Style** | Nommage, formatage, complexité |

---

## Les outils d'analyse statique pour C

### 1. clang-tidy 🔵 (Recommandé)

**L'outil moderne et le plus complet.**

- Basé sur l'infrastructure Clang/LLVM
- Plus de 400 vérifications disponibles
- Peut suggérer des corrections automatiques
- S'intègre parfaitement avec CMake

**Installation :**
```bash
sudo apt-get install clang-tidy
```

**Utilisation typique :** CI quotidienne, pre-commit hooks

### 2. cppcheck 🟢

**Outil indépendant du compilateur.**

- Fonctionne sans compilation
- Analyse rapide
- Faux positifs modérés
- Gratuit et open-source

**Installation :**
```bash
sudo apt-get install cppcheck
```

**Utilisation typique :** CI rapide, vérification locale

### 3. Clang Static Analyzer 🟡

**Analyseur en profondeur de Clang.**

- Analyse très approfondie
- Trouve des bugs complexes
- Plus lent que clang-tidy
- Intégré à Clang

**Utilisation :** Analyse hebdomadaire approfondie

### 4. Autres outils (commerciaux)

- **Coverity** : Standard industriel (très puissant, commercial)
- **PVS-Studio** : Excellent mais commercial
- **SonarQube** : Plateforme complète de qualité de code
- **Infer** : Par Facebook, open-source

---

## clang-tidy : L'outil principal

### Installation

```bash
sudo apt-get update
sudo apt-get install clang-tidy
```

Vérifier la version :
```bash
clang-tidy --version
```

Sortie typique :
```
LLVM version 15.0.0
```

### Utilisation de base

```bash
clang-tidy src/main.c -- -Iinclude
```

**Syntaxe :**
- `src/main.c` : Fichier à analyser
- `--` : Séparateur
- `-Iinclude` : Options du compilateur (chemins d'inclusion, etc.)

### Avec une base de données de compilation

Pour les projets complexes, utilisez `compile_commands.json` :

```bash
# Générer avec CMake
cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

# Analyser avec la base de données
clang-tidy -p build src/main.c
```

**Avantage :** clang-tidy connaît automatiquement tous les flags de compilation.

---

## Configuration de clang-tidy

### Fichier .clang-tidy

Créez un fichier `.clang-tidy` à la racine de votre projet :

**.clang-tidy**
```yaml
---
# Vérifications à activer
Checks: >
  clang-diagnostic-*,
  clang-analyzer-*,
  bugprone-*,
  performance-*,
  readability-*,
  modernize-*,
  -readability-magic-numbers,
  -readability-identifier-length

# Traiter les warnings comme des erreurs
WarningsAsErrors: ''

# Options pour les vérifications
CheckOptions:
  - key: readability-identifier-naming.FunctionCase
    value: lower_case
  - key: readability-identifier-naming.VariableCase
    value: lower_case
  - key: readability-identifier-naming.ConstantCase
    value: UPPER_CASE
```

**Explications :**

- `Checks:` : Liste des vérifications activées
  - `*` : Toutes les vérifications de cette catégorie
  - `-name` : Désactive une vérification spécifique
- `WarningsAsErrors:` : Transforme certains warnings en erreurs
- `CheckOptions:` : Configure des règles spécifiques

### Catégories de vérifications

| Catégorie | Description | Exemples |
|-----------|-------------|----------|
| `bugprone-*` | Bugs courants | Use-after-move, integer overflow |
| `clang-analyzer-*` | Analyseur statique | Null pointer, memory leaks |
| `performance-*` | Optimisations | Copies inutiles, boucles lentes |
| `readability-*` | Lisibilité | Nommage, complexité, magic numbers |
| `modernize-*` | Modernisation | Suggestions C moderne |
| `cert-*` | Sécurité CERT | Bonnes pratiques sécurité |
| `cppcoreguidelines-*` | Core Guidelines | Standards de qualité |

### Lister toutes les vérifications disponibles

```bash
clang-tidy --list-checks
```

Sortie partielle :
```
Enabled checks:
    bugprone-argument-comment
    bugprone-assert-side-effect
    bugprone-bool-pointer-implicit-conversion
    ...
```

---

## Exemple d'analyse clang-tidy

### Code avec problèmes

**bad_code.c**
```c
#include <stdio.h>
#include <stdlib.h>

int calculate(int x) {
    int result;  // Variable non initialisée !

    if (x == 0) {
        return 0;
    }

    result = 100 / x;
    return result;
}

int main() {
    int *ptr = malloc(10 * sizeof(int));
    // Oups ! Pas de vérification de NULL

    ptr[0] = calculate(5);

    // Oups ! Pas de free()
    return 0;
}
```

### Analyse avec clang-tidy

```bash
clang-tidy bad_code.c --
```

### Rapport

```
bad_code.c:5:9: warning: variable 'result' is used uninitialized [clang-analyzer-core.uninitialized.Assign]
    int result;
        ^
bad_code.c:15:20: note: Assuming 'ptr' is non-null
    int *ptr = malloc(10 * sizeof(int));
               ^
bad_code.c:18:5: warning: Potential leak of memory pointed to by 'ptr' [clang-analyzer-unix.Malloc]
    ptr[0] = calculate(5);
    ^
bad_code.c:21:5: warning: Memory allocated by malloc() should be deallocated [cppcoreguidelines-no-malloc]
    return 0;
    ^
```

**Le rapport indique :**
1. Variable non initialisée (ligne 5)
2. Pas de vérification du retour de `malloc` (ligne 15)
3. Fuite mémoire potentielle (ligne 18)
4. Recommandation d'éviter `malloc` brut (ligne 21)

---

## Workflow GitHub Actions avec clang-tidy

### Workflow de base

```yaml
name: Analyse Statique - clang-tidy

on: [push, pull_request]

jobs:
  clang-tidy:
    runs-on: ubuntu-latest

    steps:
    - name: Checkout du code
      uses: actions/checkout@v4

    - name: Installer clang-tidy
      run: |
        sudo apt-get update
        sudo apt-get install -y clang-tidy cmake

    - name: Configurer le projet
      run: |
        cmake -B build \
          -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

    - name: Lancer clang-tidy
      run: |
        clang-tidy -p build src/*.c
```

### Workflow avec rapport détaillé

```yaml
name: clang-tidy Analysis

on:
  push:
    branches: [ main ]
  pull_request:
    branches: [ main ]

jobs:
  static-analysis:
    runs-on: ubuntu-latest

    steps:
    - name: Checkout
      uses: actions/checkout@v4

    - name: Installer les outils
      run: |
        sudo apt-get update
        sudo apt-get install -y clang-tidy-15 cmake
        sudo update-alternatives --install /usr/bin/clang-tidy clang-tidy /usr/bin/clang-tidy-15 100

    - name: Configurer avec compile_commands.json
      run: |
        cmake -B build \
          -DCMAKE_C_COMPILER=clang \
          -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

    - name: Vérifier compile_commands.json
      run: |
        if [ ! -f build/compile_commands.json ]; then
          echo "❌ compile_commands.json non généré"
          exit 1
        fi
        echo "✅ compile_commands.json trouvé"

    - name: Analyser avec clang-tidy
      run: |
        clang-tidy -p build src/*.c 2>&1 | tee clang-tidy-report.txt

    - name: Vérifier les erreurs critiques
      run: |
        if grep -q "error:" clang-tidy-report.txt; then
          echo "❌ Erreurs critiques détectées"
          exit 1
        fi
        echo "✅ Aucune erreur critique"

    - name: Uploader le rapport
      if: always()
      uses: actions/upload-artifact@v4
      with:
        name: clang-tidy-report
        path: clang-tidy-report.txt
```

---

## cppcheck : Analyse indépendante

### Installation

```bash
sudo apt-get install cppcheck
```

### Utilisation de base

```bash
cppcheck --enable=all --inconclusive --suppress=missingIncludeSystem src/
```

**Options :**

- `--enable=all` : Active toutes les vérifications
- `--inconclusive` : Signale même les problèmes incertains
- `--suppress=missingIncludeSystem` : Ignore les headers système manquants
- `src/` : Répertoire à analyser

### Options avancées

```bash
cppcheck \
  --enable=warning,style,performance,portability \
  --std=c11 \
  --template=gcc \
  --verbose \
  --force \
  src/
```

**Explications :**

- `--enable=` : Catégories de vérifications
- `--std=c11` : Standard C à utiliser
- `--template=gcc` : Format de sortie compatible GCC
- `--force` : Vérifie même les fichiers avec erreurs de préprocesseur

### Rapport XML pour CI

```bash
cppcheck --enable=all --xml --xml-version=2 src/ 2> cppcheck-report.xml
```

---

## Workflow GitHub Actions avec cppcheck

```yaml
name: Analyse Statique - cppcheck

on: [push, pull_request]

jobs:
  cppcheck:
    runs-on: ubuntu-latest

    steps:
    - name: Checkout
      uses: actions/checkout@v4

    - name: Installer cppcheck
      run: |
        sudo apt-get update
        sudo apt-get install -y cppcheck

    - name: Vérifier la version
      run: cppcheck --version

    - name: Analyser le code
      run: |
        cppcheck \
          --enable=all \
          --inconclusive \
          --suppress=missingIncludeSystem \
          --std=c11 \
          --template='[{file}:{line}] ({severity}) {id}: {message}' \
          --error-exitcode=1 \
          src/ 2>&1 | tee cppcheck-report.txt

    - name: Uploader le rapport
      if: always()
      uses: actions/upload-artifact@v4
      with:
        name: cppcheck-report
        path: cppcheck-report.txt
```

**Option importante :** `--error-exitcode=1` fait échouer le workflow si des erreurs sont détectées.

---

## Clang Static Analyzer (scan-build)

### Installation

```bash
sudo apt-get install clang-tools
```

### Utilisation avec Make

```bash
scan-build make
```

### Utilisation avec CMake

```bash
scan-build cmake -B build
scan-build cmake --build build
```

### Workflow GitHub Actions

```yaml
name: Clang Static Analyzer

on: [push, pull_request]

jobs:
  scan-build:
    runs-on: ubuntu-latest

    steps:
    - uses: actions/checkout@v4

    - name: Installer scan-build
      run: |
        sudo apt-get update
        sudo apt-get install -y clang-tools cmake

    - name: Analyser avec scan-build
      run: |
        scan-build --status-bugs cmake -B build
        scan-build --status-bugs cmake --build build
```

**Option `--status-bugs`** : Fait échouer la commande si des bugs sont détectés.

---

## Matrice d'outils : Analyse complète

Pour une analyse exhaustive, combinez plusieurs outils :

```yaml
name: Analyse Statique Complète

on:
  push:
    branches: [ main ]
  pull_request:
    branches: [ main ]

jobs:
  static-analysis:
    name: ${{ matrix.tool }}
    runs-on: ubuntu-latest

    strategy:
      fail-fast: false
      matrix:
        tool:
          - name: clang-tidy
            command: clang-tidy -p build src/*.c

          - name: cppcheck
            command: cppcheck --enable=all --error-exitcode=1 src/

          - name: scan-build
            command: scan-build --status-bugs cmake --build build

    steps:
    - name: Checkout
      uses: actions/checkout@v4

    - name: Installer les outils
      run: |
        sudo apt-get update
        sudo apt-get install -y clang-tidy cppcheck clang-tools cmake

    - name: Configurer le projet
      if: matrix.tool.name != 'cppcheck'
      run: cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

    - name: Lancer ${{ matrix.tool.name }}
      run: ${{ matrix.tool.command }} 2>&1 | tee ${{ matrix.tool.name }}-report.txt

    - name: Uploader le rapport
      if: always()
      uses: actions/upload-artifact@v4
      with:
        name: ${{ matrix.tool.name }}-report
        path: ${{ matrix.tool.name }}-report.txt
```

---

## Configuration CMake pour l'analyse statique

Intégrez l'analyse statique directement dans votre CMakeLists.txt :

**CMakeLists.txt**

```cmake
cmake_minimum_required(VERSION 3.10)
project(MonProjet C)

# Option pour activer clang-tidy
option(ENABLE_CLANG_TIDY "Enable clang-tidy analysis" OFF)

if(ENABLE_CLANG_TIDY)
    find_program(CLANG_TIDY_EXE NAMES clang-tidy)

    if(CLANG_TIDY_EXE)
        message(STATUS "clang-tidy trouvé: ${CLANG_TIDY_EXE}")
        set(CMAKE_C_CLANG_TIDY ${CLANG_TIDY_EXE})
    else()
        message(WARNING "clang-tidy non trouvé")
    endif()
endif()

# Option pour activer cppcheck
option(ENABLE_CPPCHECK "Enable cppcheck analysis" OFF)

if(ENABLE_CPPCHECK)
    find_program(CPPCHECK_EXE NAMES cppcheck)

    if(CPPCHECK_EXE)
        message(STATUS "cppcheck trouvé: ${CPPCHECK_EXE}")
        set(CMAKE_C_CPPCHECK
            ${CPPCHECK_EXE}
            --enable=warning,performance,portability
            --std=c11
            --suppress=missingIncludeSystem
        )
    else()
        message(WARNING "cppcheck non trouvé")
    endif()
endif()

# Votre projet
add_executable(app src/main.c)
```

**Utilisation dans le workflow :**

```yaml
- name: Configurer avec clang-tidy
  run: cmake -B build -DENABLE_CLANG_TIDY=ON

- name: Compiler (analyse automatique)
  run: cmake --build build
```

L'analyse se fait automatiquement pendant la compilation !

---

## Annotations dans les Pull Requests

Pour annoter directement les fichiers dans les PRs, utilisez des actions spécialisées :

### Avec reviewdog

```yaml
name: reviewdog

on: [pull_request]

jobs:
  clang-tidy:
    runs-on: ubuntu-latest

    steps:
    - uses: actions/checkout@v4

    - name: Installer clang-tidy
      run: sudo apt-get install -y clang-tidy cmake

    - name: Configurer
      run: cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

    - name: Analyser avec reviewdog
      uses: reviewdog/action-clang-tidy@v1
      with:
        github_token: ${{ secrets.GITHUB_TOKEN }}
        reporter: github-pr-review
        level: warning
        clang_tidy_flags: -p build
```

**Résultat :** Les problèmes détectés apparaissent comme des commentaires dans la PR !

---

## Suppressions et faux positifs

### Supprimer des warnings spécifiques avec clang-tidy

Dans le code source :

```c
// NOLINT(bugprone-narrowing-conversions)
int x = 3.14;

// Supprimer plusieurs vérifications
// NOLINTNEXTLINE(readability-magic-numbers, cert-err34-c)
int magic = 42;
```

### Supprimer avec cppcheck

```c
// cppcheck-suppress uninitvar
int x;
use(x);
```

### Fichier de suppressions cppcheck

**cppcheck-suppressions.txt**
```
uninitvar:src/legacy.c
memleak:src/external.c:42
```

**Utilisation :**
```bash
cppcheck --suppressions-list=cppcheck-suppressions.txt src/
```

---

## Vérifications de sécurité spécifiques

### Avec clang-tidy CERT

Activez les vérifications CERT (Computer Emergency Response Team) :

**.clang-tidy**
```yaml
Checks: cert-*
```

**Vérifications CERT importantes :**

- `cert-err33-c` : Vérifier les valeurs de retour
- `cert-str31-c` : Ne pas modifier les string literals
- `cert-mem33-c` : Allocations/désallocations correctes
- `cert-env33-c` : Ne pas appeler `system()` avec des entrées utilisateur

### Avec cppcheck

```bash
cppcheck --enable=warning,style,performance,portability --addon=cert src/
```

---

## Intégration avec SonarCloud

Pour une analyse plus complète incluant la qualité de code :

```yaml
name: SonarCloud Analysis

on:
  push:
    branches: [ main ]
  pull_request:

jobs:
  sonarcloud:
    runs-on: ubuntu-latest

    steps:
    - uses: actions/checkout@v4
      with:
        fetch-depth: 0  # Nécessaire pour SonarCloud

    - name: Installer les outils
      run: |
        sudo apt-get update
        sudo apt-get install -y gcc cmake gcov

    - name: Configurer
      run: |
        cmake -B build \
          -DCMAKE_BUILD_TYPE=Debug \
          -DCMAKE_C_FLAGS="--coverage"

    - name: Compiler
      run: cmake --build build

    - name: Lancer les tests
      run: |
        cd build
        ctest

    - name: Générer la couverture
      run: |
        cd build
        gcov CMakeFiles/app.dir/src/*.gcno

    - name: SonarCloud Scan
      uses: SonarSource/sonarcloud-github-action@master
      env:
        GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        SONAR_TOKEN: ${{ secrets.SONAR_TOKEN }}
```

**Note :** Nécessite une configuration `sonar-project.properties` et un compte SonarCloud.

---

## Métriques de complexité

### Avec lizard (outil de métriques)

```yaml
- name: Installer lizard
  run: pip install lizard

- name: Analyser la complexité
  run: |
    lizard src/ -l c --CCN 15 --arguments 5 --length 1000
```

**Métriques :**
- **CCN** : Complexité cyclomatique (≤ 15 recommandé)
- **arguments** : Nombre de paramètres (≤ 5 recommandé)
- **length** : Longueur de fonction (≤ 100 lignes recommandé)

### Avec clang-tidy

```yaml
Checks: readability-function-cognitive-complexity
CheckOptions:
  - key: readability-function-cognitive-complexity.Threshold
    value: 25
```

---

## Workflow complet : Qualité de code

Voici un workflow production-ready combinant tous les outils :

```yaml
name: Analyse de Qualité de Code

on:
  push:
    branches: [ main, develop ]
  pull_request:
    branches: [ main ]

jobs:
  static-analysis:
    name: Analyse Statique
    runs-on: ubuntu-latest

    steps:
    - name: Checkout
      uses: actions/checkout@v4
      with:
        fetch-depth: 0

    - name: Installer les outils
      run: |
        sudo apt-get update
        sudo apt-get install -y \
          clang-tidy-15 \
          cppcheck \
          clang-tools \
          cmake \
          gcc
        pip install lizard

    - name: Configurer le projet
      run: |
        cmake -B build \
          -DCMAKE_C_COMPILER=clang \
          -DCMAKE_EXPORT_COMPILE_COMMANDS=ON

    # 1. clang-tidy
    - name: Analyse clang-tidy
      continue-on-error: true
      run: |
        echo "=== clang-tidy Analysis ==="
        clang-tidy-15 -p build src/*.c 2>&1 | tee clang-tidy-report.txt

    # 2. cppcheck
    - name: Analyse cppcheck
      continue-on-error: true
      run: |
        echo "=== cppcheck Analysis ==="
        cppcheck \
          --enable=all \
          --inconclusive \
          --std=c11 \
          --suppress=missingIncludeSystem \
          --template='[{file}:{line}] ({severity}) {id}: {message}' \
          src/ 2>&1 | tee cppcheck-report.txt

    # 3. scan-build
    - name: Analyse scan-build
      continue-on-error: true
      run: |
        echo "=== Clang Static Analyzer ==="
        scan-build --status-bugs cmake --build build 2>&1 | tee scan-build-report.txt

    # 4. Complexité
    - name: Analyse de complexité
      run: |
        echo "=== Code Complexity Analysis ==="
        lizard src/ -l c --CCN 15 --arguments 5 | tee complexity-report.txt

    # 5. Résumé
    - name: Générer le résumé
      if: always()
      run: |
        echo "# 📊 Résumé de l'analyse statique" > summary.md
        echo "" >> summary.md

        echo "## clang-tidy" >> summary.md
        if [ -f clang-tidy-report.txt ]; then
          WARNINGS=$(grep -c "warning:" clang-tidy-report.txt || echo "0")
          ERRORS=$(grep -c "error:" clang-tidy-report.txt || echo "0")
          echo "- Warnings: $WARNINGS" >> summary.md
          echo "- Errors: $ERRORS" >> summary.md
        fi
        echo "" >> summary.md

        echo "## cppcheck" >> summary.md
        if [ -f cppcheck-report.txt ]; then
          ISSUES=$(grep -c "(" cppcheck-report.txt || echo "0")
          echo "- Issues trouvés: $ISSUES" >> summary.md
        fi
        echo "" >> summary.md

        echo "## Complexité" >> summary.md
        cat complexity-report.txt | tail -5 >> summary.md

        cat summary.md

    # 6. Vérifier les erreurs critiques
    - name: Vérifier les erreurs
      run: |
        CRITICAL=0

        if grep -q "error:" clang-tidy-report.txt; then
          echo "❌ Erreurs critiques dans clang-tidy"
          CRITICAL=1
        fi

        if grep -q "\[error\]" cppcheck-report.txt; then
          echo "❌ Erreurs critiques dans cppcheck"
          CRITICAL=1
        fi

        if [ $CRITICAL -eq 1 ]; then
          echo ""
          echo "⚠️  Des erreurs critiques ont été détectées."
          echo "    Consultez les rapports pour plus de détails."
          exit 1
        else
          echo "✅ Aucune erreur critique détectée"
        fi

    # 7. Upload des rapports
    - name: Uploader tous les rapports
      if: always()
      uses: actions/upload-artifact@v4
      with:
        name: static-analysis-reports
        path: |
          clang-tidy-report.txt
          cppcheck-report.txt
          scan-build-report.txt
          complexity-report.txt
          summary.md
        retention-days: 30

    # 8. Commenter la PR
    - name: Commenter la PR avec le résumé
      if: github.event_name == 'pull_request' && always()
      uses: actions/github-script@v7
      with:
        script: |
          const fs = require('fs');
          const summary = fs.readFileSync('summary.md', 'utf8');
          github.rest.issues.createComment({
            owner: context.repo.owner,
            repo: context.repo.repo,
            issue_number: context.issue.number,
            body: summary
          })
```

---

## Bonnes pratiques

### 1. Commencer progressivement

```yaml
# Première étape : mode informatif seulement
- name: clang-tidy (info only)
  continue-on-error: true
  run: clang-tidy -p build src/*.c

# Après quelques semaines : mode strict
- name: clang-tidy (strict)
  run: clang-tidy -p build src/*.c
```

### 2. Configurer les règles selon le projet

Adaptez `.clang-tidy` à votre contexte :

```yaml
# Projet embarqué : focus performance
Checks: performance-*, bugprone-*

# Projet sécurité : focus CERT
Checks: cert-*, clang-analyzer-security-*

# Projet legacy : vérifications minimales
Checks: bugprone-*, clang-analyzer-*
```

### 3. Utiliser compile_commands.json

```cmake
set(CMAKE_EXPORT_COMPILE_COMMANDS ON)
```

Évite les faux positifs liés aux includes manquants.

### 4. Exécuter localement avant le push

```bash
# Pre-commit check
clang-tidy -p build src/*.c && git commit
```

### 5. Ne pas viser la perfection immédiatement

Commencez avec quelques vérifications, augmentez progressivement.

### 6. Documenter les suppressions

```c
// NOLINT: Legacy code, refactoring planifié pour v2.0
int old_function(void) { ... }
```

### 7. Automatiser complètement

L'analyse statique doit tourner **à chaque push**, pas manuellement.

---

## Erreurs courantes et solutions

### Erreur : "compile_commands.json not found"

**Solution :**
```yaml
- name: Générer compile_commands.json
  run: cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
```

### Erreur : Trop de faux positifs

**Solutions :**
1. Affiner les règles dans `.clang-tidy`
2. Utiliser des suppressions ciblées
3. Vérifier que compile_commands.json est correct

### Erreur : "clang-tidy: command not found"

**Solution :**
```yaml
- name: Installer clang-tidy
  run: sudo apt-get install -y clang-tidy
```

### Analyse trop lente

**Solutions :**
1. Limiter aux fichiers modifiés :
   ```bash
   git diff --name-only origin/main | grep '\.c$' | xargs clang-tidy -p build
   ```
2. Exécuter en nocturne pour analyse complète
3. Paralléliser avec `run-clang-tidy`

---

## Comparaison des outils

| Critère | clang-tidy | cppcheck | scan-build |
|---------|------------|----------|------------|
| **Vitesse** | Moyenne | Rapide | Lente |
| **Précision** | Excellente | Bonne | Excellente |
| **Faux positifs** | Peu | Modérés | Peu |
| **Configuration** | Flexible | Simple | Limitée |
| **Fixes automatiques** | ✅ Oui | ❌ Non | ❌ Non |
| **CI quotidienne** | ✅ | ✅ | ⚠️ |
| **Analyse profonde** | ⚠️ | ❌ | ✅ |

**Recommandation :**
- **CI quotidienne** : clang-tidy + cppcheck
- **Analyse hebdomadaire** : scan-build
- **Analyse de sécurité** : clang-tidy (cert-*)

---

## Métriques de qualité

### Objectifs raisonnables

| Métrique | Objectif | Excellent |
|----------|----------|-----------|
| Warnings clang-tidy | < 10 | 0 |
| Erreurs cppcheck | 0 | 0 |
| Complexité cyclomatique | < 15 | < 10 |
| Longueur de fonction | < 100 lignes | < 50 lignes |
| Paramètres de fonction | ≤ 5 | ≤ 3 |

### Suivre l'évolution

```yaml
- name: Comparer avec la branche main
  run: |
    git checkout main
    clang-tidy -p build src/*.c 2>&1 | grep -c "warning:" > main_warnings.txt

    git checkout ${{ github.sha }}
    clang-tidy -p build src/*.c 2>&1 | grep -c "warning:" > pr_warnings.txt

    MAIN=$(cat main_warnings.txt)
    PR=$(cat pr_warnings.txt)

    if [ $PR -gt $MAIN ]; then
      echo "⚠️ Nouvelles warnings introduites : $((PR - MAIN))"
    else
      echo "✅ Pas de nouvelles warnings"
    fi
```

---

## Résumé

**L'analyse statique trouve des bugs avant l'exécution du code.**

**Outils essentiels :**
- **clang-tidy** : Analyse complète, moderne, recommandé
- **cppcheck** : Rapide, indépendant du compilateur
- **scan-build** : Analyse profonde pour investigations

**Configuration minimale :**

1. Créer `.clang-tidy` avec les règles
2. Générer `compile_commands.json` avec CMake
3. Lancer `clang-tidy -p build src/*.c`
4. Intégrer dans le workflow CI

**Workflow typique :**

```yaml
- cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
- cmake --build build
- clang-tidy -p build src/*.c
```

**Bonnes pratiques :**
- Commencer progressivement
- Configurer selon votre contexte
- Automatiser complètement
- Documenter les suppressions

---

## Prochaines étapes

Maintenant que vous avez l'analyse statique, vous pouvez :

- Publier des **artifacts de build**
- Configurer des **releases automatiques**
- Créer des **rapports de tests** consolidés
- Mettre en place des **notifications**

Ces sujets sont couverts dans les sections suivantes.

---

## Ressources complémentaires

- [clang-tidy Documentation](https://clang.llvm.org/extra/clang-tidy/)
- [cppcheck Manual](https://cppcheck.sourceforge.io/manual.html)
- [Clang Static Analyzer](https://clang-analyzer.llvm.org/)
- [CERT C Coding Standard](https://wiki.sei.cmu.edu/confluence/display/c/SEI+CERT+C+Coding+Standard)

---

**🎯 Points clés à retenir :**

- ✅ L'analyse statique détecte les bugs sans exécuter le code
- ✅ clang-tidy est l'outil le plus complet et moderne
- ✅ Générer compile_commands.json avec CMake
- ✅ Configurer les règles dans `.clang-tidy`
- ✅ Commencer avec peu de règles, augmenter progressivement
- ✅ Combiner plusieurs outils pour une couverture maximale
- ✅ Automatiser dans le CI à chaque commit
- ✅ L'analyse statique complète les tests dynamiques (sanitizers, Valgrind)

⏭️ [Publication d'artifacts](/30-integration-continue/01.9-publication-artifacts.md)
