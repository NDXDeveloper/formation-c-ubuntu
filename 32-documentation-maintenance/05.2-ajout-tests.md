...
Vous héritez d'un projet C de 50 000 lignes. Zéro test. Aucun. Vous devez corriger un bug, mais vous avez peur :
- *"Si je change ça, est-ce que je vais casser autre chose ?"*
- *"Comment savoir si ma correction fonctionne vraiment ?"*
- *"Comment m'assurer que je ne réintroduis pas le bug dans 6 mois ?"*

Sans tests, vous codez **à l'aveugle**. Chaque modification est une roulette russe. Vous passez plus de temps à déboguer qu'à développer.

La solution ? **Ajouter des tests**. Mais comment tester du code qui n'a jamais été conçu pour être testé ?

Dans cette section, nous allons apprendre :
- Pourquoi les tests sont cruciaux pour le code legacy
- Les types de tests adaptés au legacy
- Les frameworks de test C (Unity, CUnit, Check)
- Comment rendre le code legacy testable
- La stratégie progressive d'ajout de tests
...
### Le problème du code non testé

**Code legacy sans tests** :

```
Changement nécessaire (bugfix, feature)
    ↓
...
Changement nécessaire
    ↓
Écrire un test qui échoue (reproduit le bug)
    ↓
Modifier le code
    ↓
Tests automatisés
...
✅ Ça marche
    ↓
Le test empêchera la régression
```
...
Les tests sont un **filet de sécurité** qui vous permet de modifier le code sans peur :

```c
// Sans tests : peur de toucher
int legacy_function() {
    // 200 lignes de code spaghetti
    // "Je ne touche à rien, ça marche"
}

// Avec tests : confiance pour refactoriser
int legacy_function() {
    // Tests me protègent, je peux améliorer
}
```
...
// Ce test documente : parse("") retourne une chaîne vide, pas NULL
```

C'est mieux que des commentaires qui deviennent obsolètes.

#### 3. Détection précoce des bugs

Les tests détectent les bugs **avant** la production :

```bash
# Sans tests
Developer → Commit → CI Build → Deploy → Production → 💥 Bug découvert par users

# Avec tests
Developer → Commit → CI Build → ❌ Tests fail → Fix → ✅ Tests pass → Deploy
```
...
int old_messy_code() { ... }  
test_old_messy_code(); // ✅ Pass  

// Après refactoring : tests passent toujours
int clean_refactored_code() { ... }  
test_clean_refactored_code(); // ✅ Pass  
...
**Problème** : Le code legacy n'a **pas été conçu** pour être testé.

Caractéristiques typiques :
- 🚫 Couplage fort (dépendances partout)
- 🚫 Fonctions géantes (impossible à tester unitairement)
- 🚫 État global (variables globales)
- 🚫 Dépendances système (fichiers, réseau, base de données)
- 🚫 Logique et I/O mélangées
...
Pour tester ça, il faut :
- Un fichier de config
- Un serveur réseau
- Une base de données
- Des permissions système

❌ Trop complexe pour un test unitaire !
...
**Objectif** : Documenter le comportement **actuel** (même s'il est buggé).
...
void test_current_behavior_empty_input() {
    // Ce que le code fait actuellement (même si c'est un bug)
    int result = legacy_parse("");

    // On documente : actuellement ça retourne -1
    // (on pourrait penser que ça devrait retourner 0, mais ce n'est pas le cas)
    assert_int_equals(-1, result);
}

void test_current_behavior_null_input() {
    // Actuellement ça crash (segfault)
    // On ne peut pas tester ça facilement, on documente juste
...
- Documente le comportement réel
- Base pour le refactoring

**Workflow** :
```
1. Écrire des tests de caractérisation (comportement actuel)
2. Refactoriser le code
3. Tests passent toujours → comportement préservé
4. Corriger les bugs (modifier les tests en conséquence)
```

### 2. Tests unitaires

**Objectif** : Tester une fonction **isolément**.

**Principe** : Une fonction, un test, pas de dépendances externes.
...
**Défis avec le code legacy** :
- Fonctions trop grosses
- Dépendances externes
- État global

→ Il faut refactoriser pour rendre testable (voir plus loin).

### 3. Tests d'intégration

**Objectif** : Tester plusieurs composants ensemble.

**Principe** : Vérifier que les modules interagissent correctement.
...
- Testent le système "tel qu'il est"
- Mais plus lents et plus fragiles

### 4. Tests de non-régression

**Objectif** : Empêcher le retour de bugs déjà corrigés.
...
void test_bug_123_buffer_overflow() {
    ...
    // Avant le fix, ça crashait
    // Après le fix, ça doit retourner une erreur proprement
...
**Règle d'or** : Chaque bug fixé doit avoir un test qui l'empêchera de revenir.
...
| Framework | Complexité | Dépendances | Mocking | Popularité |
|-----------|------------|-------------|---------|------------|
| **Unity** | Faible | Aucune | Non (externe) | ⭐⭐⭐⭐⭐ |
| **Check** | Moyenne | libcheck | Non | ⭐⭐⭐⭐ |
| **CUnit** | Moyenne | libcunit | Non | ⭐⭐⭐ |
| **cmocka** | Moyenne | libcmocka | Oui | ⭐⭐⭐⭐ |
| **assert.h** | Très faible | Standard C | Non | ⭐⭐ |

**Recommandation pour débutants** : **Unity** (simple, portable, zéro dépendance).
...
# Ou télécharger juste unity.c et unity.h
...
```
test_example.c:28:test_add_function_positive_numbers:PASS  
test_example.c:29:test_add_function_zero:PASS
...
**Avantage** : Isole chaque test dans un processus séparé (segfault dans un test n'arrête pas les autres).
...
START_TEST(test_crash) {
    // Ce test va crasher mais n'arrêtera pas les autres
    int *p = NULL;
    *p = 42;  // Segfault isolé
}
END_TEST
...
### cmocka : Avec mocking intégré

**Avantage** : Support du mocking pour isoler les dépendances.
...
### Problème : Dépendances hardcodées

**Code intestable** :

```c
// Impossible à tester : dépendance hardcodée
int process_data() {
    FILE *f = fopen("/etc/myapp/config.txt", "r");
    if (f == NULL) return -1;

    // Lit depuis le fichier réel
    char buffer[256];
...
**Principe** : Passer les dépendances en paramètres.

```c
// Testable : dépendance injectée
...
### Solution 2 : Abstraire les dépendances
...
**Principe** : Créer des points d'injection dans le code existant.
...
```c
// Avant : logique et I/O mélangées
int process_user() {
    // Lit depuis fichier
...
// Après : logique extraite et testable
...
## Stratégie progressive d'ajout de tests

### Phase 1 : Tests de fumée (Smoke tests)

**Objectif** : Vérifier que le système démarre et fonctionne basiquement.
...
void test_basic_functionality() {
    // Fonctionnalité de base fonctionne
...
**Effort** : Très faible  
**Valeur** : Détecte les crashes majeurs  
...
**Effort** : Faible  
**Valeur** : Protection contre régressions  

### Phase 3 : Tests de non-régression

**Objectif** : Empêcher le retour de bugs connus.

```c
// test_regression.c
void test_bug_42_overflow() {
    // Bug #42 corrigé le 2025-01-15
...
**Valeur** : Évite de re-introduire les bugs  

### Phase 4 : Tests unitaires des fonctions critiques

**Objectif** : Tester les fonctions importantes isolément.

**Stratégie** : Commencer par les fonctions :
1. Les plus utilisées
2. Les plus modifiées récemment
3. Les plus bugguées historiquement
4. Les plus critiques (sécurité, données)
...
**Valeur** : Haute (protège les parties critiques)  
...
**Effort** : Élevé  
**Valeur** : Teste le système réel  

---

## Patterns de test pour le code legacy

### Pattern 1 : Golden Master (Test de référence)

**Principe** : Capturer la sortie actuelle comme référence, puis vérifier qu'elle ne change pas.

```c
// test_golden_master.c

// 1. Générer le golden master (une fois)
void generate_golden_master() {
    // Exécuter le programme avec input connu
    system("./legacy_program < test_input.txt > golden_output.txt");
    // Commiter golden_output.txt dans Git
}

// 2. Test : comparer à la référence
...
**Utilité** : Idéal pour du code complexe dont on ne comprend pas tout le comportement.

### Pattern 2 : Test Harness (Harnais de test)

**Principe** : Créer une infrastructure pour faciliter les tests.
...
**Principe** : Enregistrer les appels pour vérification ultérieure.
...
**Principe** : Remplacer une dépendance par une version simplifiée.
...
│   │   ├── unity.c
│   │   └── unity.h
│   ├── test_parser.c    # Tests du parser
│   ├── test_validator.c # Tests du validator
│   ├── test_integration.c
│   ├── test_runner.c    # Lance tous les tests
...
void test_parse_empty() {
    // Utilise shared_buffer de test précédent
    // Problème si test_parse_normal n'a pas été exécuté avant !
    int result = parse(shared_buffer);
    free(shared_buffer);
    TEST_ASSERT_EQUAL_INT(-1, result);
}
...
**Objectif** : Suite complète en <10 secondes pour encourager l'exécution fréquente.
...
```c
45: if (input == NULL) return -1;    // ✅ Couvert
46:     errno = EINVAL;              // ❌ Non couvert
```

**Action** : Ajouter un test qui passe `NULL` et vérifie `errno`.

**Objectifs réalistes** :
- 60% : Minimum acceptable
- 80% : Bon
- 90%+ : Excellent (mais attention au coût)
- 100% : Rarement justifié (coût/bénéfice)

---

## Conclusion

Ajouter des tests à du code legacy est un **investissement** qui paie rapidement :

- ✅ **Confiance** : Modifier le code sans peur
- ✅ **Qualité** : Détecter les bugs tôt
- ✅ **Documentation** : Tests = spécifications exécutables
- ✅ **Refactoring** : Filet de sécurité pour améliorer le code
- ✅ **Vélocité** : Développement plus rapide à long terme

**Stratégie recommandée** :

1. **Semaine 1** : Tests de fumée et caractérisation (quick wins)
2. **Semaine 2-3** : Tests de non-régression (bugs connus)
...
N'attendez pas le projet parfait pour commencer à tester. Commencez maintenant, un test à la fois.
...
⏭️ [Modernisation](/32-documentation-maintenance/05.3-modernisation.md)
