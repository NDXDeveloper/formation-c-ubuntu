🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 5.7.1 Installation et configuration

## Introduction

**clang-format** est un outil de formatage automatique de code pour C, C++ et d'autres langages. Il permet de :
- Formater automatiquement votre code selon des règles définies
- Assurer la cohérence du style dans tout votre projet
- Gagner du temps en évitant de formater manuellement
- Éliminer les débats d'équipe sur le style de code

Dans cette section, nous allons voir comment installer et configurer clang-format sur Ubuntu, ainsi que les bases de son utilisation.

---

## Qu'est-ce que clang-format ?

clang-format fait partie du projet LLVM/Clang. C'est un outil en ligne de commande qui :
- Lit votre code source
- Applique des règles de formatage
- Réécrit le code formaté

**Avantages :**
- ✅ Formatage cohérent et automatique
- ✅ Support de nombreux styles prédéfinis (LLVM, Google, Chromium, Mozilla, WebKit, etc.)
- ✅ Hautement configurable
- ✅ Intégration facile avec les éditeurs et IDE
- ✅ Peut être intégré dans le workflow CI/CD

---

## Installation sur Ubuntu

### Méthode 1 : Installation via apt (recommandée pour débuter)

La façon la plus simple d'installer clang-format est d'utiliser le gestionnaire de paquets apt :

```bash
# Mettre à jour la liste des paquets
sudo apt update

# Installer clang-format
sudo apt install clang-format
```

Cette commande installe la version par défaut disponible dans les dépôts Ubuntu.

**Vérifier l'installation :**
```bash
clang-format --version
```

Vous devriez voir quelque chose comme :
```
Ubuntu clang-format version 14.0.0-1ubuntu1
```

### Méthode 2 : Installer une version spécifique

Si vous souhaitez une version plus récente ou spécifique :

```bash
# Installer clang-format version 17
sudo apt install clang-format-17

# Créer un lien symbolique pour l'utiliser par défaut
sudo update-alternatives --install /usr/bin/clang-format clang-format /usr/bin/clang-format-17 100
```

**Versions disponibles :**
```bash
# Lister les versions disponibles
apt-cache search clang-format
```

**Vérifier quelle version est active :**
```bash
clang-format --version
```

### Méthode 3 : Installation depuis les dépôts LLVM officiels

Pour obtenir la toute dernière version :

```bash
# Ajouter le dépôt LLVM
wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | sudo apt-key add -
sudo add-apt-repository "deb http://apt.llvm.org/$(lsb_release -cs)/ llvm-toolchain-$(lsb_release -cs) main"

# Mettre à jour et installer
sudo apt update
sudo apt install clang-format
```

### Vérification de l'installation

Pour vérifier que clang-format est bien installé et accessible :

```bash
# Vérifier la version
clang-format --version

# Vérifier l'emplacement
which clang-format

# Afficher l'aide
clang-format --help
```

---

## Utilisation de base en ligne de commande

### Formater un fichier (aperçu)

Pour voir le résultat du formatage sans modifier le fichier :

```bash
clang-format mon_fichier.c
```

Cette commande affiche le code formaté dans le terminal, mais **ne modifie pas le fichier**.

### Formater un fichier (modification en place)

Pour formater le fichier directement :

```bash
clang-format -i mon_fichier.c
```

L'option `-i` signifie "in-place" (sur place), le fichier sera modifié directement.

**Exemple pratique :**

Créez un fichier `test.c` mal formaté :
```c
#include <stdio.h>
int main(){int x=10;int y=20;if(x>5){printf("Grand\n");}return 0;}
```

Formatez-le :
```bash
clang-format -i test.c
```

Le fichier devient :
```c
#include <stdio.h>
int main() {
  int x = 10;
  int y = 20;
  if (x > 5) {
    printf("Grand\n");
  }
  return 0;
}
```

### Formater plusieurs fichiers

**Formater tous les fichiers .c et .h dans le répertoire courant :**
```bash
clang-format -i *.c *.h
```

**Formater récursivement tous les fichiers dans un projet :**
```bash
find . -name "*.c" -o -name "*.h" | xargs clang-format -i
```

**Alternative avec des sous-répertoires :**
```bash
find . -type f \( -name "*.c" -o -name "*.h" \) -exec clang-format -i {} \;
```

### Vérifier le formatage sans modifier

Pour vérifier si un fichier est correctement formaté :

```bash
clang-format --dry-run --Werror mon_fichier.c
```

- `--dry-run` : Ne modifie pas le fichier
- `--Werror` : Retourne une erreur si le formatage n'est pas correct

Cette commande est utile pour les scripts CI/CD.

---

## Configuration : Le fichier .clang-format

### Qu'est-ce que .clang-format ?

Le fichier `.clang-format` est un fichier de configuration qui définit les règles de formatage pour votre projet. clang-format cherche ce fichier :
1. Dans le répertoire du fichier source
2. Dans les répertoires parents (remonte l'arborescence)
3. Jusqu'à trouver un fichier `.clang-format` ou atteindre la racine

**Avantage :** Placez un seul fichier `.clang-format` à la racine de votre projet et tous les fichiers seront formatés de la même manière.

### Créer un fichier .clang-format de base

**Option 1 : Utiliser un style prédéfini**

```bash
# Créer un .clang-format basé sur le style LLVM
clang-format -style=llvm -dump-config > .clang-format
```

**Option 2 : Créer manuellement**

Créez un fichier `.clang-format` à la racine de votre projet :

```yaml
---
# Configuration clang-format de base
Language: Cpp
BasedOnStyle: LLVM

# Indentation
IndentWidth: 4
TabWidth: 4
UseTab: Never

# Accolades
BreakBeforeBraces: Linux

# Largeur de ligne
ColumnLimit: 100

# Espaces
SpaceAfterCStyleCast: false
SpaceBeforeParens: ControlStatements
SpaceBeforeAssignmentOperators: true

# Alignement
AlignConsecutiveAssignments: false
AlignConsecutiveDeclarations: false

# Autres
AllowShortFunctionsOnASingleLine: None
AllowShortIfStatementsOnASingleLine: Never
AllowShortLoopsOnASingleLine: false
...
```

### Styles prédéfinis disponibles

clang-format propose plusieurs styles prédéfinis :

| Style | Description | Utilisation typique |
|-------|-------------|---------------------|
| **LLVM** | Style du projet LLVM | Projets C/C++ modernes, équilibré |
| **Google** | Style guide de Google | Projets suivant les conventions Google |
| **Chromium** | Style du projet Chromium | Navigateur Chromium |
| **Mozilla** | Style du projet Mozilla | Firefox et projets Mozilla |
| **WebKit** | Style du moteur WebKit | Safari et projets WebKit |
| **Microsoft** | Style Visual Studio | Projets Windows/Visual Studio |
| **GNU** | Style GNU | Projets GNU (GCC, etc.) |

**Générer la configuration d'un style prédéfini :**
```bash
# Style LLVM
clang-format -style=llvm -dump-config > .clang-format

# Style Google
clang-format -style=google -dump-config > .clang-format

# Style GNU
clang-format -style=gnu -dump-config > .clang-format
```

### Exemple : Comparaison des styles

**Code source original :**
```c
#include <stdio.h>
int main(){int x=10;if(x>5){printf("Grand\n");}return 0;}
```

**Style LLVM :**
```c
#include <stdio.h>
int main() {
  int x = 10;
  if (x > 5) {
    printf("Grand\n");
  }
  return 0;
}
```

**Style Google :**
```c
#include <stdio.h>
int main() {
  int x = 10;
  if (x > 5) {
    printf("Grand\n");
  }
  return 0;
}
```

**Style GNU :**
```c
#include <stdio.h>
int
main ()
{
  int x = 10;
  if (x > 5)
    {
      printf ("Grand\n");
    }
  return 0;
}
```

**Style Linux Kernel :**
```c
#include <stdio.h>
int main()
{
        int x = 10;
        if (x > 5) {
                printf("Grand\n");
        }
        return 0;
}
```

---

## Options de configuration importantes

Voici les options les plus couramment utilisées dans `.clang-format` :

### Indentation et espacement

```yaml
# Nombre d'espaces pour l'indentation
IndentWidth: 4

# Largeur d'une tabulation
TabWidth: 4

# Utiliser des espaces au lieu de tabs
UseTab: Never  # Options: Never, Always, ForIndentation, ForContinuationAndIndentation

# Indenter les directives du préprocesseur (#include, #define, etc.)
IndentPPDirectives: None  # Options: None, AfterHash, BeforeHash

# Indenter les étiquettes case dans un switch
IndentCaseLabels: false
```

### Style des accolades

```yaml
# Style d'accolades
BreakBeforeBraces: Linux
# Options:
#   Attach     : int foo() {
#   Linux      : int foo()\n{
#   Mozilla    : int foo()\n{  (sauf fonctions)
#   Stroustrup : int foo()\n{  (else sur même ligne)
#   Allman     : int foo()\n{  (toujours nouvelle ligne)
#   GNU        : int foo()\n{\n  (2 espaces, style GNU)
#   WebKit     : Style WebKit
#   Custom     : Utiliser BraceWrapping pour personnaliser
```

### Longueur des lignes

```yaml
# Nombre maximum de caractères par ligne
ColumnLimit: 100  # 0 = pas de limite

# Pénalité pour dépasser la limite
PenaltyExcessCharacter: 1000000
```

### Espaces

```yaml
# Espaces autour des opérateurs d'affectation
SpaceBeforeAssignmentOperators: true

# Espaces avant les parenthèses
SpaceBeforeParens: ControlStatements
# Options: Never, ControlStatements, Always, NonEmptyParentheses

# Espaces après les cast C-style
SpaceAfterCStyleCast: false

# Espaces dans les conteneurs vides
SpaceInEmptyParentheses: false

# Espaces dans les parenthèses
SpacesInParentheses: false

# Espaces dans les crochets
SpacesInSquareBrackets: false
```

### Alignement

```yaml
# Aligner les affectations consécutives
AlignConsecutiveAssignments: false

# Aligner les déclarations consécutives
AlignConsecutiveDeclarations: false

# Aligner les commentaires de fin de ligne
AlignTrailingComments: true
```

### Fonctions et conditions courtes

```yaml
# Autoriser les fonctions courtes sur une ligne
AllowShortFunctionsOnASingleLine: None
# Options: None, InlineOnly, Empty, Inline, All

# Autoriser les if courts sur une ligne
AllowShortIfStatementsOnASingleLine: Never
# Options: Never, WithoutElse, OnlyFirstIf, AllIfsAndElse

# Autoriser les boucles courtes sur une ligne
AllowShortLoopsOnASingleLine: false
```

### Saut de lignes

```yaml
# Toujours casser avant les accolades de fonction multiligne
AlwaysBreakAfterDefinitionReturnType: None

# Casser après le type de retour de fonction
AlwaysBreakAfterReturnType: None

# Nombre maximum de lignes consécutives vides
MaxEmptyLinesToKeep: 1

# Garder les sauts de ligne dans les paramètres
BinPackParameters: true
```

---

## Configuration recommandée pour débutants

Voici une configuration simple et efficace pour commencer :

```yaml
---
# Configuration clang-format simple pour débutants
Language: Cpp
BasedOnStyle: LLVM

# === INDENTATION ===
IndentWidth: 4                    # 4 espaces par niveau
TabWidth: 4
UseTab: Never                     # Toujours utiliser des espaces

# === ACCOLADES ===
BreakBeforeBraces: Linux          # Style K&R/Linux
# Fonctions : accolade sur nouvelle ligne
# if/while/for : accolade sur même ligne

# === LARGEUR DE LIGNE ===
ColumnLimit: 100                  # Max 100 caractères par ligne

# === ESPACES ===
SpaceAfterCStyleCast: false       # int *p = (int *)ptr; (pas d'espace)
SpaceBeforeParens: ControlStatements  # if (condition) mais fonction()
SpaceBeforeAssignmentOperators: true  # x = 10; (avec espace)
SpacesInParentheses: false        # (x + y) et non ( x + y )

# === ALIGNEMENT ===
AlignConsecutiveAssignments: false
AlignConsecutiveDeclarations: false
AlignTrailingComments: true       # Aligner les commentaires de fin de ligne
PointerAlignment: Right           # int *ptr; (pointeur collé au nom)

# === FONCTIONS ET CONDITIONS COURTES ===
AllowShortFunctionsOnASingleLine: None     # Jamais de fonction sur une ligne
AllowShortIfStatementsOnASingleLine: Never # Jamais de if sur une ligne
AllowShortLoopsOnASingleLine: false        # Jamais de boucle sur une ligne

# === LIGNES VIDES ===
MaxEmptyLinesToKeep: 1            # Maximum 1 ligne vide consécutive
KeepEmptyLinesAtTheStartOfBlocks: false  # Pas de ligne vide au début des blocs

# === INCLUDES ===
SortIncludes: true                # Trier les #include
IncludeBlocks: Regroup           # Regrouper les includes par catégorie

# === DIVERS ===
IndentCaseLabels: false          # Ne pas indenter les case dans un switch
...
```

**Sauvegarder ce fichier comme `.clang-format` à la racine de votre projet.**

---

## Tester votre configuration

### Créer un fichier de test

Créez un fichier `test_format.c` avec du code mal formaté :

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
int main(){
int x=10,y=20,z;
z=x+y;
if(z>25){
printf("Grand\n");
}else{
printf("Petit\n");
}
for(int i=0;i<5;i++){
printf("%d ",i);
}
return 0;
}
```

### Appliquer le formatage

```bash
# Formater avec votre .clang-format
clang-format -i test_format.c
```

### Résultat attendu

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

int main()
{
    int x = 10, y = 20, z;
    z = x + y;

    if (z > 25) {
        printf("Grand\n");
    } else {
        printf("Petit\n");
    }

    for (int i = 0; i < 5; i++) {
        printf("%d ", i);
    }

    return 0;
}
```

---

## Utilisation avancée en ligne de commande

### Formater avec un style sans fichier .clang-format

```bash
# Utiliser directement un style prédéfini
clang-format -style=google -i mon_fichier.c

# Utiliser un style avec quelques modifications
clang-format -style="{BasedOnStyle: llvm, IndentWidth: 4}" -i mon_fichier.c
```

### Formater seulement une partie du fichier

```bash
# Formater les lignes 10 à 20
clang-format -lines=10:20 mon_fichier.c

# Formater plusieurs plages
clang-format -lines=10:20 -lines=30:40 mon_fichier.c
```

### Spécifier le fichier de configuration

```bash
# Utiliser un fichier de configuration spécifique
clang-format -style=file:/chemin/vers/.clang-format -i mon_fichier.c
```

### Afficher les différences

```bash
# Voir ce qui sera changé (diff)
clang-format mon_fichier.c | diff mon_fichier.c -
```

---

## Résolution de problèmes

### Problème : clang-format not found

**Symptôme :**
```bash
bash: clang-format: command not found
```

**Solution :**
```bash
# Vérifier si installé
dpkg -l | grep clang-format

# Réinstaller
sudo apt update
sudo apt install clang-format
```

### Problème : Le formatage ne s'applique pas

**Causes possibles :**
1. Le fichier `.clang-format` n'est pas dans le bon répertoire
2. Le fichier `.clang-format` contient des erreurs de syntaxe

**Solutions :**
```bash
# Vérifier où clang-format cherche la config
clang-format -style=file -dump-config mon_fichier.c

# Valider la syntaxe de votre .clang-format
clang-format -style=file:.clang-format -dump-config > /dev/null
```

### Problème : Le style ne correspond pas à vos attentes

**Solution :** Comparez avec un style prédéfini :

```bash
# Générer la config du style LLVM
clang-format -style=llvm -dump-config > llvm-style.yaml

# Comparer avec votre config
diff .clang-format llvm-style.yaml
```

### Problème : Certains fichiers ne sont pas formatés

**Vérifiez :**
```bash
# Voir les options appliquées
clang-format -style=file -dump-config mon_fichier.c | grep Language
```

Assurez-vous que `Language: Cpp` est bien défini dans votre `.clang-format`.

---

## Organisation d'un projet avec clang-format

### Structure recommandée

```
mon_projet/
├── .clang-format          ← Configuration à la racine
├── src/
│   ├── main.c
│   ├── module1.c
│   └── module2.c
├── include/
│   ├── module1.h
│   └── module2.h
├── tests/
│   └── test_main.c
└── README.md
```

Tous les fichiers utiliseront automatiquement le `.clang-format` de la racine.

### Script de formatage du projet

Créez un script `format.sh` à la racine :

```bash
#!/bin/bash
# Script pour formater tout le projet

echo "Formatage du code en cours..."

# Trouver et formater tous les fichiers .c et .h
find src include tests -type f \( -name "*.c" -o -name "*.h" \) \
    -exec clang-format -i {} \;

echo "Formatage terminé !"
```

Rendez-le exécutable :
```bash
chmod +x format.sh
```

Utilisez-le :
```bash
./format.sh
```

---

## Bonnes pratiques

### 1. Commiter le fichier .clang-format

Toujours inclure `.clang-format` dans votre dépôt Git :

```bash
git add .clang-format
git commit -m "Ajout de la configuration clang-format"
```

**Pourquoi ?** Tous les contributeurs utiliseront le même style.

### 2. Formater avant de commiter

Prenez l'habitude de formater avant chaque commit :

```bash
# Formater les fichiers modifiés
git diff --name-only --cached | grep -E '\.(c|h)$' | xargs clang-format -i

# Puis commiter
git add .
git commit -m "Mon commit"
```

### 3. Documenter le style dans le README

Ajoutez une section dans votre `README.md` :

```markdown
## Style de code

Ce projet utilise clang-format pour le formatage automatique.

### Installation
```bash
sudo apt install clang-format
```

### Utilisation
```bash
# Formater tout le projet
./format.sh

# Formater un fichier
clang-format -i mon_fichier.c
```

### Configuration
Le fichier `.clang-format` à la racine définit le style du projet.
```

### 4. Ne pas formater le code legacy progressivement

Si vous intégrez clang-format dans un projet existant :

**Option 1 :** Formater tout d'un coup (commit dédié)
```bash
find . -name "*.c" -o -name "*.h" | xargs clang-format -i
git commit -m "Formatage: application de clang-format sur tout le projet"
```

**Option 2 :** Formater seulement les nouveaux fichiers ou modifications

### 5. Exceptions : Désactiver temporairement le formatage

Parfois, vous voulez un formatage manuel spécifique :

```c
// clang-format off
int tableau[3][3] = {
    {1, 2, 3},
    {4, 5, 6},
    {7, 8, 9}
};
// clang-format on
```

**⚠️ À utiliser avec parcimonie !**

---

## Résumé

### Installation

```bash
# Installation simple
sudo apt install clang-format

# Vérification
clang-format --version
```

### Utilisation de base

```bash
# Formater un fichier
clang-format -i mon_fichier.c

# Formater plusieurs fichiers
clang-format -i *.c *.h

# Formater tout le projet
find . -name "*.c" -o -name "*.h" | xargs clang-format -i
```

### Configuration

1. Créer `.clang-format` à la racine du projet
2. Partir d'un style prédéfini : `clang-format -style=llvm -dump-config > .clang-format`
3. Personnaliser selon vos besoins
4. Commiter le fichier dans Git

### Points clés

- ✅ clang-format assure un formatage cohérent
- ✅ Le fichier `.clang-format` définit les règles
- ✅ Plusieurs styles prédéfinis disponibles
- ✅ Facile à intégrer dans le workflow
- ✅ S'intègre avec les éditeurs et le CI/CD

---

## Pour aller plus loin

Dans les prochaines sections, vous découvrirez :
- **5.7.2 Styles courants** : Comparaison détaillée des styles prédéfinis
- **5.7.3 Intégration IDE** : Configuration dans VS Code, Vim, etc.
- **5.7.4 Pre-commit hooks Git** : Automatiser le formatage avant les commits
- **5.7.5 Formatage dans le CI/CD** : Vérification automatique dans GitHub Actions

L'utilisation de clang-format est une excellente habitude à prendre dès maintenant. Cela vous fera gagner du temps et garantira un code propre et professionnel !

⏭️ [Styles courants](/05-operateurs-et-controle/07.2-styles-courants.md)
