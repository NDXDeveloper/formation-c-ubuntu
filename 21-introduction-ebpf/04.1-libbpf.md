...
**libbpf** est la bibliothèque C officielle pour développer des applications eBPF modernes. Elle constitue la **fondation** sur laquelle sont construits la plupart des outils eBPF que vous utilisez aujourd'hui : Cilium, Falco, bpftrace, et bien d'autres.

Si vous voulez comprendre comment eBPF fonctionne réellement "sous le capot", ou si vous souhaitez développer vos propres outils eBPF personnalisés, **libbpf est incontournable**.
...
**libbpf** est une bibliothèque C qui simplifie le développement d'applications eBPF en fournissant des fonctions de haut niveau pour :

1. **Charger** des programmes eBPF dans le kernel
2. **Attacher** ces programmes à des points d'ancrage (hooks)
3. **Interagir** avec les BPF maps depuis l'userspace
4. **Gérer** le cycle de vie des programmes eBPF

### Pourquoi libbpf existe

Sans libbpf, développer une application eBPF nécessiterait d'utiliser directement les **syscalls bas niveau** :
...
// ... des dizaines de lignes de code répétitif
```
...
**libbpf abstrait la complexité** tout en restant performant et flexible.

---

## L'évolution de l'écosystème eBPF

### Les trois générations d'outils eBPF

Pour comprendre la place de libbpf, il faut comprendre l'évolution de l'écosystème :

#### **Génération 1 : Syscalls bruts (2014-2016)**

```c
// Écrire directement dans le kernel via syscalls
...
union bpf_attr attr = {
    ...
    // ... des dizaines de champs à remplir
};
...
- ❌ Pas portable entre versions kernel

#### **Génération 2 : BCC (2016-2020)**

BCC (BPF Compiler Collection) a révolutionné l'accessibilité d'eBPF en introduisant un **workflow Python + C** :

```python
# Programme BCC : Python + C inline
from bcc import BPF

# Code eBPF en C (compilé à la volée)
prog = """
int hello(void *ctx) {
...
**Problèmes :**
- ❌ **Dépendance runtime lourde** : Nécessite LLVM/Clang sur chaque machine
- ❌ **Démarrage lent** : Compilation à chaque exécution (1-5 secondes)
- ❌ **Consommation mémoire** : LLVM = 100+ MB de RAM
- ❌ **Pas portable** : Le binaire contient du code source C
- ❌ **Difficile à distribuer** : Impossible de faire un binaire standalone

#### **Génération 3 : libbpf + CO-RE (2020-aujourd'hui)**

libbpf avec **CO-RE** (Compile Once - Run Everywhere) représente l'approche moderne :
...
**Avantages :**
- ✅ **Binaire standalone** : Un seul fichier exécutable
- ✅ **Portable** : Fonctionne sur différentes versions kernel (CO-RE)
- ✅ **Démarrage instantané** : Pas de compilation runtime
- ✅ **Léger** : Pas de dépendance LLVM/Clang en production
- ✅ **Distribution facile** : apt install, docker, etc.
...
## Les concepts clés de libbpf

### 1. **bpf_object** : Conteneur du programme

Un **bpf_object** représente un fichier objet eBPF chargé en mémoire :
...
// Ou itérer sur tous les programmes
...
Un **bpf_link** représente l'attachement d'un programme à un hook :
...
// Ou attacher manuellement à un kprobe
...
**Important :** Tant que le `bpf_link` existe, le programme reste attaché. Sa destruction détache le programme.

### 4. **bpf_map** : Structures de données partagées

Les BPF maps permettent le partage de données entre kernel et userspace :
...
### Le problème de la portabilité kernel

Historiquement, un programme eBPF compilé pour une version du kernel **ne fonctionnait pas** sur une autre version :

```
Programme compilé pour kernel 5.10
                ↓
        Fonctionne ? ❌ sur kernel 5.15
                      ❌ sur kernel 6.1
```
...
// Kernel 5.15 : Champs réorganisés !
struct task_struct {
    char comm[16];
    int pid;  // ← Position changée !
    // ... champs différents
};
```
...
**CO-RE** (Compile Once - Run Everywhere) résout ce problème via trois composants :

#### 1. **BTF (BPF Type Format)**

BTF est un format qui décrit la structure des types du kernel :
...
Le kernel moderne (5.x+) embarque BTF, permettant à libbpf de connaître la structure exacte des types.

#### 2. **Macros BPF_CORE_READ**

Ces macros génèrent du code qui s'adapte automatiquement :
...
3. Ajuste les accès mémoire en conséquence

#### 3. **libbpf relocation**

À l'exécution, libbpf **relocate** les accès :

```
Compilation (une fois) :
  program.bpf.c → program.bpf.o (avec relocation info)
...
Chargement (runtime) :
  ...
  Applique les relocations
         ↓
  Charge le programme modifié dans le kernel
```
...
```c
#include <vmlinux.h>  // Types kernel générés depuis BTF
#include <bpf/bpf_helpers.h>
#include <bpf/bpf_core_read.h>
...
- ✅ Kernel 5.15
- ✅ Kernel 6.1
- ✅ Kernel avec config custom

---

## libbpf vs BCC : Comparaison détaillée

| Aspect | BCC | libbpf + CO-RE |
|--------|-----|----------------|
| **Langage** | Python + C inline | C pur |
| **Compilation** | Runtime (à chaque lancement) | Compile-time (une fois) |
| **Démarrage** | 1-5 secondes | < 50 ms |
| **Dépendances runtime** | LLVM/Clang (100+ MB) | libbpf (< 1 MB) |
| **Consommation RAM** | 100-200 MB | < 5 MB |
| **Distribution** | Source code nécessaire | Binaire standalone |
| **Portabilité kernel** | Limitée | Excellente (CO-RE) |
| **Performance** | Bonne | Excellente |
| **Facilité d'utilisation** | ★★★★★ (très facile) | ★★★☆☆ (courbe d'apprentissage) |
| **Cas d'usage idéal** | Prototypage, scripts | Production, outils durables |

### Quand utiliser BCC

- ✅ **Prototypage rapide** : Tester une idée en 5 minutes
- ✅ **Scripts one-shot** : Debugging ponctuel
- ✅ **Environnement dev** : Où LLVM est déjà installé
- ✅ **Apprentissage** : Commencer avec eBPF facilement

### Quand utiliser libbpf

- ✅ **Applications production** : Outils déployés en prod
- ✅ **Distribution publique** : apt/yum packages, containers
- ✅ **Performance critique** : Démarrage rapide requis
- ✅ **Systèmes embarqués** : Ressources limitées
- ✅ **Outils système** : Intégration dans l'OS
...
# Version kernel minimale recommandée : 5.4+
uname -r
```
...
# Sortie attendue : 1.0.0 ou supérieur
...
**vmlinux.h** contient toutes les définitions de types du kernel :

```bash
# Générer depuis le kernel actuel
...
my-ebpf-tool/
├── src/
│   ├── program.bpf.c        # Code kernel eBPF
│   └── loader.c             # Code userspace
├── include/
│   └── vmlinux.h            # Types kernel
...
// Hook à la sortie de sys_openat
...
int trace_exit_openat(struct trace_event_raw_sys_exit *ctx) {
    ...
    // Récupérer le filename sauvegardé
    const char **filename_ptr = bpf_map_lookup_elem(&tmp_filename, &pid);
    if (!filename_ptr)
        return 0;

    // Réserver de l'espace dans le ringbuffer
...
// Callback pour traiter les événements du ringbuffer
...
int main(int argc, char **argv) {
    ...
    // Attacher tous les programmes automatiquement
...
## Fonctionnalités avancées de libbpf

### 1. **Skeleton : Génération automatique de code**

libbpf peut générer du code C qui simplifie encore plus l'utilisation :
...
int dump_task(struct bpf_iter__task *ctx) {
    ...
    SEQ_PRINTF(ctx->meta->seq, "PID: %d, COMM: %s\n",
               task->pid, task->comm);
...
### 4. **Vérifier la compatibilité BTF**

```bash
# Vérifier si le kernel supporte BTF
...
- **"BPF CO-RE (Compile Once – Run Everywhere)"** par Andrii Nakryiko : https://nakryiko.com/posts/bpf-portability-and-co-re/
...
**libbpf** est la fondation moderne du développement eBPF. En tant qu'ingénieur système ou DevOps, maîtriser libbpf vous permet de :

- ✅ **Développer des outils eBPF production-ready** avec binaires standalone
- ✅ **Créer des applications portables** grâce à CO-RE
- ✅ **Obtenir des performances optimales** (démarrage instantané, overhead minimal)
- ✅ **Distribuer facilement** via packages système ou containers
- ✅ **Comprendre** comment fonctionnent les outils eBPF sous le capot

Bien que la courbe d'apprentissage soit plus raide que BCC, libbpf est l'approche recommandée pour tout projet sérieux. C'est la technologie utilisée par **Cilium, Falco, et tous les outils eBPF modernes**.

Dans la section suivante (21.4.2), nous explorerons **bpftool**, l'outil en ligne de commande essentiel pour inspecter, debugger, et gérer les programmes eBPF.

---

**💡 À retenir :**
- libbpf est la bibliothèque C officielle pour eBPF moderne
- CO-RE permet la portabilité entre versions kernel (Compile Once, Run Everywhere)
- Les applications libbpf sont des binaires standalone (pas de dépendance LLVM/Clang)
- Structure en 2 parties : code kernel (.bpf.c) + loader userspace (.c)
- Plus complexe que BCC mais meilleur pour la production
- Utilisé par tous les outils eBPF professionnels modernes

---

⏭️ [bpftool](/21-introduction-ebpf/04.2-bpftool.md)
