...
- Identifier les compilateurs installés sur votre Ubuntu
- Vérifier leurs versions
- Tester le support des différents standards C (C99, C11, C17, C23)
- Comprendre les options de compilation
- Installer des versions alternatives si nécessaire
...
## 🖥️ Les compilateurs C disponibles sur Ubuntu
...
### Étape 1 : Vérifier si GCC est installé
...
**Interprétation** :
- **Première ligne** : Version de GCC = `13.2.0`
- **Ubuntu 13.2.0-23ubuntu4** : Version packagée pour Ubuntu
- Vous avez GCC installé ! ✅
...
|-------------|---------------------|-----------------|
| GCC 4.x | `gnu90` (basé sur C89/C90) | ~2005-2015 |
| GCC 5-10 | `gnu11` (basé sur C11) | 2015-2020 |
| GCC 11-14 | `gnu17` (basé sur C17) | 2021-2024 |
| GCC 15+ | `gnu23` (basé sur C23) | 2025+ |

> ⚠️ **Note** : `gnu11`, `gnu17` = C11/C17 + extensions GNU. On verra la différence plus bas.
...
**Décodage** :
- `201710L` = C17 (publié en 2017-10)
- `201112L` = C11 (publié en 2011-12)
- `199901L` = C99 (publié en 1999-01)
- `202311L` = C23 (publié en 2023-11)

### Étape 3 : Tester le support de chaque standard

Créons un petit programme test pour vérifier les standards supportés.
...
### Étape 1 : Vérifier si Clang est installé
...
Utilisez le même fichier `test_std.c` :
...
## 📋 Tableau de compatibilité GCC/Clang

### Support des standards C

| Standard | GCC min. | Clang min. | Disponibilité |
|----------|----------|------------|---------------|
| **C90/C89** | Toutes versions | Toutes versions | ✅ Universel |
| **C99** | GCC 3.0+ (2001) | Clang 3.0+ (2011) | ✅ Universel |
| **C11** | GCC 4.6+ (2011) | Clang 3.1+ (2012) | ✅ Universel |
| **C17** | GCC 8.0+ (2018) | Clang 6.0+ (2018) | ✅ Universel |
...
> ⚠️ **Note** : Certaines implémentations de glibc n'incluent pas `<threads.h>`. Si ça échoue, c'est un problème de bibliothèque, pas de compilateur. Utilisez plutôt `pthread.h` (POSIX threads).
...
### Test 4 : Nouveautés C23 (si supporté)
...
## 🛠️ Options de compilation importantes
...
| `-std=c11` | C11 strict |
| `-std=c17` | C17 strict |
| `-std=c23` | C23 strict (si supporté) |
...
- **Avec extensions GNU** (`gnu11`) : Ajoute des fonctionnalités GNU (plus flexible)
...
- Évite les dépendances aux extensions spécifiques

### Vérifier les macros prédéfinies
...
| `__clang__` | Défini si compilateur Clang |
| `__linux__` | Système Linux |
...
- Utiliser des fonctionnalités récentes (C23)
...
- **GCC 11+ ou GCC 13+** (installé par défaut)
- **Standard cible : C11 ou C17**

**Commande de compilation à utiliser** :
...
- `-pedantic` : Mode strict conforme à la norme
...
```
- C99 pour compatibilité maximale
...
```
- C17 (version corrigée de C11)
- `-g` pour symboles de débogage

---

## 🧪 Script de vérification automatique

Créez un script `check_compiler.sh` pour tout vérifier automatiquement :
...
1. **GCC est le compilateur principal** sur Ubuntu (généralement installé par défaut)
2. **Vérifier la version** : `gcc --version`
3. **Vérifier le standard** : `gcc -dM -E - < /dev/null | grep __STDC_VERSION__`
4. **Ubuntu récent = bon support C11/C17** (GCC 11+)
5. **C23 nécessite GCC 14+** (support partiel en 2025)
...
⏭️ [Pourquoi C11 reste la base recommandée en production](/01-ecosysteme-c-linux/03.3-c11-base-production.md)
