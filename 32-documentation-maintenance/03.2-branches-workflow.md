🔝 Retour au [Sommaire](/SOMMAIRE.md)

# 32.3.2 Branches et workflow

## Introduction

Imaginez que vous travaillez sur un projet C en production. Des clients l'utilisent quotidiennement. Soudain, vous voulez ajouter une nouvelle fonctionnalité qui va prendre 2 semaines de développement. Comment faire sans casser le code en production ?

C'est là qu'interviennent les **branches Git**. Une branche est comme une ligne temporelle parallèle où vous pouvez expérimenter librement sans affecter la version stable.

Dans cette section, nous allons apprendre :
- Ce qu'est une branche Git et comment ça fonctionne
- Les workflows Git populaires (Git Flow, GitHub Flow, etc.)
- Comment organiser les branches dans un projet
- Les stratégies de fusion (merge vs rebase)
- Les bonnes pratiques pour le travail en équipe

---

## Qu'est-ce qu'une branche ?

### Concept de base

Une **branche** est un pointeur mobile vers un commit. C'est une ligne de développement indépendante qui vous permet de :

- Travailler sur une fonctionnalité sans toucher au code principal
- Expérimenter sans risque
- Maintenir plusieurs versions en parallèle (production, développement, features)

### Analogie : Le livre dont vous êtes le héros

Imaginez un livre dont vous êtes le héros :

```
Page 1: L'aventure commence
├─> Choix A: Aller dans la forêt (branche feature/forest)
│   ├─> Page 34: Vous trouvez un trésor
│   └─> Page 35: Vous rentrez au village
│
└─> Choix B: Aller à la montagne (branche feature/mountain)
    ├─> Page 67: Vous combattez un dragon
    └─> Page 68: Vous devenez un héros
```

Chaque choix est une "branche" : une histoire parallèle qui peut se dérouler indépendamment. À la fin, vous pouvez choisir quelle histoire fusionner dans l'histoire principale.

### Visualisation Git

```
main branch:
A---B---C---D---E   (production stable)
     \         /
      F---G---H      (feature/nouvelle-fonction)
```

- `A, B, C, D, E` : Commits sur la branche principale
- `F, G, H` : Commits sur une branche de développement
- Le merge final fusionne la feature dans main

---

## Branches par défaut

### La branche `main` (ou `master`)

Quand vous créez un dépôt Git, une branche par défaut est créée :

- Historiquement : `master`
- Moderne : `main` (GitHub et GitLab utilisent maintenant `main`)

C'est votre **branche principale**, généralement :
- Le code en production
- Toujours stable et testée
- Protégée (pas de push direct)

### Vérifier votre branche actuelle

```bash
# Voir toutes les branches
git branch

# Voir la branche actuelle
git branch --show-current

# Voir toutes les branches avec détails
git branch -vv

# Voir toutes les branches (locales et distantes)
git branch -a
```

---

## Créer et utiliser des branches

### Créer une nouvelle branche

```bash
# Créer une branche (mais rester sur la branche actuelle)
git branch feature/export-json

# Créer et basculer sur la nouvelle branche (recommandé)
git checkout -b feature/export-json

# Ou avec la commande moderne (Git 2.23+)
git switch -c feature/export-json
```

### Basculer entre les branches

```bash
# Ancienne méthode
git checkout main
git checkout feature/export-json

# Nouvelle méthode (plus claire)
git switch main
git switch feature/export-json
```

### Pousser une branche vers le dépôt distant

```bash
# Première fois : créer la branche distante
git push -u origin feature/export-json

# Pushs suivants
git push
```

### Supprimer une branche

```bash
# Supprimer une branche locale (après merge)
git branch -d feature/export-json

# Forcer la suppression (même si pas mergée)
git branch -D feature/export-json

# Supprimer une branche distante
git push origin --delete feature/export-json
```

---

## Conventions de nommage des branches

Un bon nom de branche doit être **descriptif** et **structuré**.

### Format recommandé

```
<type>/<description-courte>
```

**Types courants** :
- `feature/` : Nouvelle fonctionnalité
- `bugfix/` ou `fix/` : Correction de bug
- `hotfix/` : Correction urgente en production
- `refactor/` : Refactoring sans changement de comportement
- `docs/` : Documentation
- `test/` : Ajout de tests
- `chore/` : Tâches de maintenance
- `experimental/` : Expérimentations

### Exemples de bons noms

✅ **Bons noms** :
```bash
feature/json-export
feature/user-authentication
bugfix/memory-leak-parser
hotfix/security-vulnerability-cve-2025-123
refactor/split-large-functions
docs/api-reference
test/unit-tests-crypto-module
chore/update-dependencies
experimental/new-algorithm
```

✅ **Avec numéros d'issue** (encore mieux) :
```bash
feature/123-json-export
bugfix/456-memory-leak
hotfix/789-security-patch
```

❌ **Mauvais noms** :
```bash
test              # Trop vague
my-branch         # Pas descriptif
fix               # Quel fix ?
new-stuff         # Quoi comme "stuff" ?
branch-2025-01-15 # Utiliser des dates comme noms
```

### Règles de nommage

- **Minuscules** : `feature/json-export` pas `Feature/JSON-Export`
- **Tirets** : `feature/json-export` pas `feature/json_export`
- **Pas d'espaces** : `feature/json-export` pas `feature/json export`
- **Descriptif mais concis** : Max 40 caractères
- **Référencer l'issue** si applicable : `feature/123-json-export`

---

## Workflows Git populaires

Un **workflow** est une convention sur comment organiser les branches dans un projet. Différents workflows existent pour différents besoins.

### 1. Git Flow (workflow traditionnel)

**Pour qui ?** Projets avec releases planifiées, plusieurs versions en production.

**Branches principales** :
- `main` (ou `master`) : Code en production
- `develop` : Code en développement

**Branches secondaires** :
- `feature/*` : Nouvelles fonctionnalités (depuis develop)
- `release/*` : Préparation d'une release (depuis develop)
- `hotfix/*` : Corrections urgentes (depuis main)

**Schéma** :
```
main:     A---B---C-------D-------E   (production: v1.0, v1.1, v1.2)
               \         /       /
develop:        F---G---H---I---J---K  (développement continu)
                 \     /
feature/X:        L---M              (nouvelle feature)
```

**Workflow typique** :

1. **Développer une feature** :
   ```bash
   git checkout develop
   git checkout -b feature/export-json
   # ... développement ...
   git commit -m "Implement JSON export"
   git checkout develop
   git merge --no-ff feature/export-json
   git branch -d feature/export-json
   ```

2. **Préparer une release** :
   ```bash
   git checkout develop
   git checkout -b release/1.1.0
   # ... tests, bugfixes mineurs, bump version ...
   git checkout main
   git merge --no-ff release/1.1.0
   git tag v1.1.0
   git checkout develop
   git merge --no-ff release/1.1.0
   git branch -d release/1.1.0
   ```

3. **Hotfix en production** :
   ```bash
   git checkout main
   git checkout -b hotfix/security-patch
   # ... correction urgente ...
   git checkout main
   git merge --no-ff hotfix/security-patch
   git tag v1.1.1
   git checkout develop
   git merge --no-ff hotfix/security-patch
   git branch -d hotfix/security-patch
   ```

**Avantages** :
- ✅ Structure claire et prévisible
- ✅ Plusieurs versions en parallèle
- ✅ Séparation développement/production

**Inconvénients** :
- ❌ Complexe pour petits projets
- ❌ Beaucoup de branches à gérer
- ❌ Merge commits nombreux

**Quand l'utiliser ?**
- Logiciels avec versions releases (v1.0, v1.1, v2.0)
- Produits avec plusieurs versions supportées en parallèle
- Équipes moyennes à grandes (5+ développeurs)

### 2. GitHub Flow (workflow simplifié)

**Pour qui ?** Projets web, déploiement continu, petites équipes.

**Branches** :
- `main` : Code en production (toujours déployable)
- `feature/*` : Toutes les autres branches (features, bugfixes, etc.)

**Principe** : Une seule branche principale, tout le reste est des branches de travail éphémères.

**Schéma** :
```
main: A---B---C---D---E---F---G   (toujours déployable)
           \     /   \     /
feature/X:  H---I     J---K       (branches courtes)
```

**Workflow typique** :

1. **Créer une branche depuis main** :
   ```bash
   git checkout main
   git pull origin main
   git checkout -b feature/json-export
   ```

2. **Développer et pusher régulièrement** :
   ```bash
   # Développement
   git add .
   git commit -m "Add JSON export function"
   git push -u origin feature/json-export

   # Continuer à pusher
   git commit -m "Add tests for JSON export"
   git push
   ```

3. **Ouvrir une Pull Request** :
   - Sur GitHub, ouvrir une PR de `feature/json-export` vers `main`
   - Demander une code review
   - Discuter et ajuster si nécessaire

4. **Merger dans main** :
   ```bash
   # Via l'interface GitHub (recommandé)
   # Ou en ligne de commande:
   git checkout main
   git merge feature/json-export
   git push origin main
   git branch -d feature/json-export
   ```

5. **Déployer** :
   - Le merge dans `main` déclenche automatiquement le déploiement (CI/CD)

**Avantages** :
- ✅ Simple et facile à comprendre
- ✅ Parfait pour déploiement continu
- ✅ Peu de branches à gérer

**Inconvénients** :
- ❌ Pas adapté aux releases planifiées
- ❌ Difficile de maintenir plusieurs versions

**Quand l'utiliser ?**
- Applications web déployées continuellement
- Projets open-source sur GitHub
- Petites à moyennes équipes (2-10 développeurs)
- Pas besoin de releases versionnées

### 3. GitLab Flow

**Pour qui ?** Compromis entre Git Flow et GitHub Flow.

**Variante 1 : Production branch**
```
main:       A---B---C---D---E      (développement)
                 \     \     \
production:       F-----G-----H    (production stable)
```

**Variante 2 : Environment branches**
```
main:        A---B---C---D---E     (développement)
              \     \     \
staging:       F-----G-----H       (environnement de test)
                \     \
production:      I-----J           (production)
```

**Principe** :
- `main` est la branche de développement (comme `develop` dans Git Flow)
- Des branches d'environnement (`staging`, `production`) représentent les environnements
- Le code est "promu" de `main` vers `staging` vers `production`

**Workflow** :

1. Développer sur des branches feature depuis `main`
2. Merger dans `main` après review
3. Cherry-pick ou merge vers `staging` pour tests
4. Cherry-pick ou merge vers `production` après validation

**Avantages** :
- ✅ Flexible selon les besoins
- ✅ Support des environnements multiples
- ✅ Moins complexe que Git Flow

**Inconvénients** :
- ❌ Peut devenir confus sans discipline
- ❌ Cherry-picking peut être source d'erreurs

**Quand l'utiliser ?**
- Projets avec plusieurs environnements (dev, staging, prod)
- Besoin de contrôle sur ce qui est déployé où
- Équipes moyennes utilisant GitLab CI/CD

### 4. Trunk-Based Development

**Pour qui ?** Équipes expérimentées avec forte culture CI/CD.

**Principe** : Tout le monde travaille sur `main` (ou sur des branches de très courte durée, <1 jour).

**Schéma** :
```
main: A---B---C---D---E---F---G---H   (tout le monde commit ici)
```

Ou avec des branches ultra-courtes :
```
main:       A---B---D---E---G---H
             \     /   /   /
feature/X:    C     (merge rapide)
feature/Y:        F     (merge rapide)
```

**Règles strictes** :
- Branches de feature < 24 heures
- Tests automatisés obligatoires
- Feature flags pour cacher le code incomplet
- Intégration continue très mature

**Avantages** :
- ✅ Intégration continue vraie
- ✅ Moins de conflits de merge
- ✅ Feedback rapide

**Inconvénients** :
- ❌ Nécessite discipline extrême
- ❌ Tests automatisés indispensables
- ❌ Pas pour débutants

**Quand l'utiliser ?**
- Équipes senior avec expertise CI/CD
- Déploiement plusieurs fois par jour
- Culture DevOps mature

---

## Choisir le bon workflow pour votre projet C

### Projet C typique selon le contexte

| Type de projet | Workflow recommandé | Raison |
|----------------|---------------------|--------|
| Bibliothèque C open-source | GitHub Flow | Simple, PR-friendly, communauté |
| Logiciel embarqué | Git Flow | Releases versionnées, stabilité cruciale |
| Outil CLI système | GitHub Flow | Déploiement simple, itérations rapides |
| Daemon/Service Linux | GitLab Flow | Multiples environnements (dev/staging/prod) |
| Kernel module | Git Flow | Versions multiples, stabilité maximale |
| Projet étudiant/perso | GitHub Flow | Simplicité, apprentissage |

### Recommandations pour débutants

Si vous débutez avec Git, commencez par **GitHub Flow** :

```bash
# Toujours partir de main à jour
git checkout main
git pull

# Créer une branche pour chaque tâche
git checkout -b feature/ma-feature

# Développer, commiter, pusher
git add .
git commit -m "..."
git push -u origin feature/ma-feature

# Ouvrir une PR sur GitHub
# Merger après review
# Supprimer la branche
```

C'est simple, intuitif, et vous apprend les bases avant de passer à des workflows plus complexes.

---

## Stratégies de fusion : Merge vs Rebase

Quand vous voulez intégrer les changements d'une branche dans une autre, vous avez deux options principales.

### 1. Merge (fusion)

**Commande** :
```bash
git checkout main
git merge feature/export-json
```

**Ce qui se passe** :
```
Avant merge:
main:     A---B---C
               \
feature:        D---E

Après merge:
main:     A---B---C---F    (F = commit de merge)
               \     /
feature:        D---E
```

Git crée un **commit de merge** qui a deux parents : le dernier commit de `main` et le dernier de `feature`.

**Avantages** :
- ✅ Préserve l'historique complet
- ✅ Montre clairement quand une feature a été intégrée
- ✅ Sûr (non-destructif)

**Inconvénients** :
- ❌ Historique peut devenir complexe avec beaucoup de branches
- ❌ Commits de merge "polluent" l'historique

**Quand l'utiliser** :
- Pour intégrer des branches de feature dans main
- Quand vous voulez préserver l'historique des branches
- Sur des branches partagées (main, develop)

### 2. Rebase (réapplication)

**Commande** :
```bash
git checkout feature/export-json
git rebase main
```

**Ce qui se passe** :
```
Avant rebase:
main:     A---B---C
               \
feature:        D---E

Après rebase:
main:     A---B---C
                   \
feature:            D'---E'  (D et E sont "rejoués" après C)
```

Git "rejoue" les commits de `feature` comme s'ils avaient été faits après le dernier commit de `main`.

**Avantages** :
- ✅ Historique linéaire et propre
- ✅ Pas de commits de merge
- ✅ Facile à lire avec `git log`

**Inconvénients** :
- ❌ Réécrit l'historique (change les hashes des commits)
- ❌ Dangereux sur des branches partagées
- ❌ Peut créer des conflits complexes

**Quand l'utiliser** :
- Sur votre branche locale avant de merger
- Pour nettoyer l'historique avant une PR
- **JAMAIS** sur une branche publique partagée

### 3. Squash merge

**Commande** :
```bash
git checkout main
git merge --squash feature/export-json
git commit -m "Add JSON export feature"
```

**Ce qui se passe** :
```
Avant squash:
main:     A---B---C
               \
feature:        D---E---F

Après squash:
main:     A---B---C---G  (G contient tous les changements de D+E+F)
```

Tous les commits de la branche feature sont "écrasés" en un seul commit sur main.

**Avantages** :
- ✅ Historique très propre sur main
- ✅ Un commit = une feature complète
- ✅ Parfait pour les PRs avec beaucoup de commits WIP

**Inconvénients** :
- ❌ Perd l'historique détaillé de la feature
- ❌ Difficile de revenir sur des changements spécifiques

**Quand l'utiliser** :
- Lors du merge de PRs avec beaucoup de petits commits
- Quand la branche feature contient des commits "WIP" ou "fix typo"
- Pour garder l'historique de main minimal

### Comparaison visuelle

| Stratégie | Historique | Lisibilité | Sécurité | Utilisation |
|-----------|------------|------------|----------|-------------|
| **Merge** | Préservé | Moyenne | Haute | Par défaut |
| **Rebase** | Linéaire | Haute | Moyenne | Branches locales |
| **Squash** | Simplifié | Très haute | Haute | PRs avec commits sales |

### Recommandations pratiques

Pour un projet C typique avec GitHub Flow :

```bash
# 1. Sur votre branche feature, rebasez régulièrement
git checkout feature/export-json
git fetch origin
git rebase origin/main  # Garde votre branche à jour

# 2. Avant de créer la PR, nettoyez l'historique
git rebase -i origin/main  # Squash commits WIP, fix typos

# 3. Lors du merge de la PR, utilisez squash si beaucoup de commits
# Via l'interface GitHub: "Squash and merge"

# Ou en ligne de commande:
git checkout main
git merge --squash feature/export-json
git commit -m "feat: Add JSON export functionality"
```

---

## Gestion des conflits

### Qu'est-ce qu'un conflit ?

Un conflit survient quand Git ne peut pas fusionner automatiquement deux changements car ils modifient les mêmes lignes.

**Exemple** :
```c
// Version sur main (par Alice)
int calculate_sum(int a, int b) {
    return a + b + 1;  // Alice a ajouté +1
}

// Version sur feature (par Bob)
int calculate_sum(int a, int b) {
    return a + b - 1;  // Bob a ajouté -1
}
```

Git ne sait pas quelle version choisir → **conflit**.

### Résoudre un conflit

1. **Git marque le conflit** :
   ```bash
   git merge feature/bob-changes
   # Auto-merging math.c
   # CONFLICT (content): Merge conflict in math.c
   # Automatic merge failed; fix conflicts and then commit the result.
   ```

2. **Ouvrir le fichier en conflit** :
   ```c
   int calculate_sum(int a, int b) {
   <<<<<<< HEAD  (votre version sur main)
       return a + b + 1;
   =======
       return a + b - 1;
   >>>>>>> feature/bob-changes  (version de Bob)
   }
   ```

3. **Résoudre manuellement** :
   ```c
   // Après discussion avec Bob, on décide de garder +1
   int calculate_sum(int a, int b) {
       return a + b + 1;
   }
   ```

4. **Marquer comme résolu et commiter** :
   ```bash
   git add math.c
   git commit -m "Merge feature/bob-changes - keep +1 offset"
   ```

### Outils pour résoudre les conflits

**Ligne de commande** :
```bash
# Voir les fichiers en conflit
git status

# Annuler le merge si trop complexe
git merge --abort

# Voir les différences
git diff
```

**Outils graphiques** :
```bash
# Lancer un outil de merge visuel
git mergetool

# Outils populaires:
# - meld
# - kdiff3
# - vimdiff
# - VS Code (intégré)
```

**VS Code** :
VS Code détecte automatiquement les conflits et affiche des boutons :
- "Accept Current Change" (garder votre version)
- "Accept Incoming Change" (garder leur version)
- "Accept Both Changes" (garder les deux)
- Éditer manuellement

### Prévenir les conflits

✅ **Bonnes pratiques** :
- Pusher et puller fréquemment
- Communiquer avec l'équipe sur qui travaille sur quoi
- Garder les branches de feature petites et courtes
- Utiliser des modules/fichiers séparés quand possible
- Rebase régulièrement sur main

---

## Protection des branches

Pour éviter les erreurs, protégez vos branches importantes (main, develop).

### Sur GitHub

**Settings** → **Branches** → **Branch protection rules**

Options recommandées :
- ✅ **Require pull request before merging** : Pas de push direct
- ✅ **Require approvals** : Au moins 1 review
- ✅ **Require status checks to pass** : CI doit passer
- ✅ **Require branches to be up to date** : Forcer rebase/merge avant
- ✅ **Include administrators** : Même règles pour les admins

### Sur GitLab

**Settings** → **Repository** → **Protected branches**

Options :
- **Allowed to merge** : Qui peut merger (Maintainers, Developers, etc.)
- **Allowed to push** : Qui peut push (généralement "No one" pour main)
- **Require approval** : Nombre de reviews nécessaires

### Configuration locale

Empêcher les push accidentels :

```bash
# Dans le dépôt
git config branch.main.pushRemote no_push
```

Toute tentative de `git push` sur main échouera avec une erreur.

---

## Bonnes pratiques de gestion des branches

### 1. Durée de vie des branches

✅ **Branches courtes** :
- Feature : < 3-5 jours
- Bugfix : < 1 jour
- Hotfix : Quelques heures

❌ **Branches longues** :
- Augmentent les risques de conflits
- Retardent l'intégration
- Rendent la review difficile

**Solution** : Découpez les grandes features en sous-tâches plus petites.

### 2. Une branche = Une tâche

Chaque branche doit avoir un objectif unique et clair.

❌ **Mauvais** :
```bash
git checkout -b my-changes  # Trop vague
# ... ajoute feature X ...
# ... corrige bug Y ...
# ... refactor Z ...
```

✅ **Bon** :
```bash
git checkout -b feature/123-json-export   # Feature X
# ... développe seulement JSON export ...

git checkout -b bugfix/456-memory-leak    # Bug Y séparé
# ... corrige seulement memory leak ...
```

### 3. Synchroniser régulièrement

Gardez votre branche à jour avec main :

```bash
# Option 1: Merge (crée un commit de merge)
git checkout feature/my-feature
git merge origin/main

# Option 2: Rebase (historique linéaire)
git checkout feature/my-feature
git rebase origin/main
```

**Fréquence recommandée** : Au moins une fois par jour, ou avant d'ouvrir une PR.

### 4. Nettoyer les branches obsolètes

```bash
# Lister les branches mergées
git branch --merged main

# Supprimer toutes les branches mergées (sauf main)
git branch --merged main | grep -v "main" | xargs git branch -d

# Supprimer les branches distantes orphelines
git remote prune origin

# Voir les branches qui n'ont plus de remote
git branch -vv | grep ': gone]'
```

### 5. Convention de nommage stricte

Définissez et respectez une convention d'équipe :

```bash
# Format: <type>/<issue>-<description>
feature/123-add-logging
bugfix/456-fix-parser-crash
hotfix/789-security-cve-2025
refactor/321-clean-memory-module
docs/234-update-readme
test/567-add-unit-tests
```

### 6. Pull Requests / Merge Requests

Utilisez toujours des PRs, même en solo :

**Pourquoi ?**
- Force une dernière relecture du code
- Déclenche les tests CI/CD
- Documente les changements
- Permet les discussions

**Template de PR** (`.github/pull_request_template.md`) :
```markdown
## Description
Brève description du changement.

## Type de changement
- [ ] Bugfix
- [ ] Feature
- [ ] Refactoring
- [ ] Documentation

## Checklist
- [ ] Code compile sans warnings
- [ ] Tests ajoutés et passent
- [ ] Documentation mise à jour
- [ ] Pas de nouveaux warnings Valgrind
- [ ] Code formaté avec clang-format

## Tests
Comment tester ce changement ?

## Screenshots (si applicable)

## Issues liées
Fixes #123
See also #456
```

---

## Workflows en équipe

### Rôles typiques

Dans une équipe structurée :

- **Mainteneur** : Peut merger dans main, créer des releases
- **Développeur** : Peut créer des branches, ouvrir des PRs
- **Contributeur externe** : Doit fork, puis PR depuis son fork

### Workflow Pull Request typique

1. **Développeur** : Crée une branche
   ```bash
   git checkout -b feature/123-json-export
   # ... développement ...
   git push -u origin feature/123-json-export
   ```

2. **Développeur** : Ouvre une PR sur GitHub/GitLab
   - Titre clair
   - Description détaillée
   - Référence l'issue (#123)
   - Assigne un reviewer

3. **Reviewer** : Review le code
   - Commente sur les lignes problématiques
   - Demande des clarifications
   - Approuve ou demande des changements

4. **Développeur** : Corrige selon feedback
   ```bash
   # Faire les corrections
   git add .
   git commit -m "Address review comments"
   git push
   ```

5. **Mainteneur** : Merge la PR
   - Via l'interface (squash/merge/rebase selon la stratégie)
   - Ou en ligne de commande

6. **Tout le monde** : Met à jour
   ```bash
   git checkout main
   git pull origin main
   ```

### Résolution de conflits en équipe

Si votre branche entre en conflit avec main :

```bash
# Mettre à jour votre branche
git checkout feature/my-feature
git fetch origin
git rebase origin/main  # ou git merge origin/main

# Résoudre les conflits
# ... éditer les fichiers ...
git add .
git rebase --continue  # si rebase
# ou git commit si merge

# Forcer le push (seulement si rebase)
git push --force-with-lease origin feature/my-feature
```

**⚠️ Important** : `--force-with-lease` est plus sûr que `--force` car il vérifie que personne d'autre n'a pushé entre temps.

---

## Cas pratiques pour projets C

### Cas 1 : Développer une nouvelle fonctionnalité

**Contexte** : Ajouter support SQLite dans votre application.

```bash
# 1. Partir de main à jour
git checkout main
git pull origin main

# 2. Créer une branche
git checkout -b feature/sqlite-support

# 3. Développer par étapes
# Commit 1: Ajouter dépendance SQLite
vim CMakeLists.txt  # Ajouter find_package(SQLite3)
git add CMakeLists.txt
git commit -m "Add SQLite3 dependency to CMake"

# Commit 2: Créer module de base
vim src/database.h src/database.c
git add src/database.*
git commit -m "Create database module with init/close functions"

# Commit 3: Implémenter CRUD
vim src/database.c
git add src/database.c
git commit -m "Implement CRUD operations for users table"

# Commit 4: Tests
vim tests/test_database.c
git add tests/test_database.c
git commit -m "Add unit tests for database module"

# 4. Pusher régulièrement
git push -u origin feature/sqlite-support

# 5. Ouvrir une PR sur GitHub

# 6. Après review et merge, nettoyer
git checkout main
git pull
git branch -d feature/sqlite-support
```

### Cas 2 : Correction de bug urgente (hotfix)

**Contexte** : Bug critique en production, segfault détecté.

```bash
# 1. Partir de main (la production)
git checkout main
git pull origin main

# 2. Créer une branche hotfix
git checkout -b hotfix/segfault-in-parser

# 3. Reproduire et corriger le bug
vim src/parser.c
# Ajouter NULL check
git add src/parser.c
git commit -m "Fix segfault in parser when input is NULL

Check for NULL input before dereferencing in parse_line().
This fixes a crash reported in production.

Fixes: #789"

# 4. Pusher immédiatement
git push -u origin hotfix/segfault-in-parser

# 5. Ouvrir une PR marquée "urgent"

# 6. Après review express et merge
git checkout main
git pull origin main
# Taguer la version
git tag v1.2.1
git push origin v1.2.1

# 7. Backport vers develop si Git Flow
git checkout develop
git merge main
git push origin develop
```

### Cas 3 : Refactoring important

**Contexte** : Refactoriser le module de configuration pour le rendre modulaire.

```bash
# 1. Créer une branche
git checkout -b refactor/modular-config

# 2. Refactorer par étapes SANS changer le comportement
# Étape 1: Extraire fonctions
vim src/config.c
git add src/config.c
git commit -m "refactor: Extract validation logic to validate_config()"

# Étape 2: Renommer pour clarté
git add src/config.c
git commit -m "refactor: Rename ambiguous variable names in config.c"

# Étape 3: Réorganiser structures
vim src/config.h src/config.c
git add src/config.*
git commit -m "refactor: Split config_t into config_parser_t and config_data_t"

# 3. S'assurer que les tests passent à CHAQUE commit
make test  # Après chaque commit

# 4. Ouvrir une PR "Refactor" (nécessite review attentive)
```

### Cas 4 : Contribution open-source

**Contexte** : Contribuer à un projet C open-source sur GitHub.

```bash
# 1. Fork le projet sur GitHub (bouton "Fork")

# 2. Cloner VOTRE fork
git clone https://github.com/VOUS/projet-opensource.git
cd projet-opensource

# 3. Ajouter l'upstream (le dépôt original)
git remote add upstream https://github.com/ORIGINAL/projet-opensource.git

# 4. Créer une branche pour votre contribution
git checkout -b feature/add-ipv6-support

# 5. Développer
# ... commits ...

# 6. Garder votre branche à jour avec upstream
git fetch upstream
git rebase upstream/main

# 7. Pusher vers VOTRE fork
git push origin feature/add-ipv6-support

# 8. Ouvrir une PR depuis votre fork vers le dépôt original
# Via l'interface GitHub

# 9. Répondre aux commentaires et mettre à jour
# ... corrections ...
git push origin feature/add-ipv6-support  # Update la PR automatiquement

# 10. Après merge, synchroniser votre fork
git checkout main
git fetch upstream
git merge upstream/main
git push origin main
```

---

## Outils et visualisation

### Visualiser les branches

**Ligne de commande** :
```bash
# Log avec graphe
git log --oneline --graph --all --decorate

# Avec plus de détails
git log --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit --all
```

**Alias pratique** :
```bash
# Ajouter dans ~/.gitconfig
[alias]
    lg = log --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit --all

# Utilisation:
git lg
```

**Outils graphiques** :
- **gitk** : Outil Tk/Tcl simple (`gitk --all`)
- **tig** : Interface ncurses (`tig`)
- **GitKraken** : Client GUI moderne
- **GitHub Desktop** : Simple et intuitif
- **VS Code** : Avec extension GitLens

### Vérifier l'état de vos branches

```bash
# Branches locales
git branch -vv

# Voir quelle branche track quelle remote
git branch -vv

# Branches distantes
git branch -r

# Toutes les branches
git branch -a

# Branches avec dernier commit
git branch -v
```

---

## Checklist : Avant de merger

Avant de merger une branche dans main, vérifiez :

- [ ] **Tous les commits sont atomiques** (voir section 32.3.1)
- [ ] **Les messages de commit sont clairs**
- [ ] **Le code compile sans warnings** (`make` ou `cmake --build`)
- [ ] **Tous les tests passent** (`make test`)
- [ ] **Pas de fuites mémoire** (Valgrind clean)
- [ ] **Code formaté** (clang-format)
- [ ] **Documentation mise à jour** (README, CHANGELOG, Doxygen)
- [ ] **PR approuvée** par au moins un reviewer
- [ ] **CI/CD passe** (GitHub Actions, GitLab CI)
- [ ] **Branche à jour avec main** (rebasé récemment)
- [ ] **Pas de fichiers temporaires** (.o, executables, logs)

---

## Erreurs courantes et solutions

### Erreur 1 : Push forcé sur une branche partagée

❌ **Ne JAMAIS faire** :
```bash
git push --force origin main
```

Cela réécrit l'historique et casse le dépôt pour tous les autres !

✅ **Solution** : N'utilisez `--force` que sur vos branches personnelles.

### Erreur 2 : Merge d'une branche non testée

❌ **Mauvais** :
```bash
git checkout main
git merge feature/new-stuff  # Sans avoir testé !
```

✅ **Bon** :
```bash
git checkout feature/new-stuff
make clean && make && make test  # Tester d'abord
git checkout main
git merge feature/new-stuff
```

### Erreur 3 : Oublier de mettre à jour avant de créer une branche

❌ **Mauvais** :
```bash
# main est vieille de 3 jours
git checkout -b feature/new  # Part d'une version obsolète
```

✅ **Bon** :
```bash
git checkout main
git pull origin main  # Mettre à jour
git checkout -b feature/new  # Part de la dernière version
```

### Erreur 4 : Branches abandonnées

Problème : Des dizaines de branches non mergées qui traînent.

✅ **Solution** : Nettoyer régulièrement
```bash
# Voir les branches mergées
git branch --merged main

# Supprimer les branches locales mergées
git branch --merged main | grep -v "main" | xargs git branch -d

# Supprimer les branches distantes mergées (via GitHub/GitLab)
```

---

## Conclusion

La gestion des branches est une compétence fondamentale pour tout développeur travaillant en équipe ou maintenant un projet sur le long terme.

**Points clés à retenir** :

1. **Choisissez le bon workflow** pour votre projet (GitHub Flow pour débuter)
2. **Une branche = une tâche** : Branches courtes et focalisées
3. **Noms descriptifs** : `feature/123-json-export`, pas `ma-branche`
4. **Commits atomiques** : Voir section 32.3.1
5. **Pull Requests** : Toujours passer par des PRs pour merger dans main
6. **Protection de branches** : Protégez main contre les push directs
7. **Synchronisation régulière** : Rebase/merge main dans votre branche quotidiennement
8. **Nettoyage** : Supprimez les branches mergées

Avec ces pratiques, vous maintiendrez un historique Git propre, faciliterez la collaboration, et simplifierez le debugging.

---

## Ressources complémentaires

- **Git Flow** : https://nvie.com/posts/a-successful-git-branching-model/
- **GitHub Flow** : https://guides.github.com/introduction/flow/
- **GitLab Flow** : https://docs.gitlab.com/ee/topics/gitlab_flow.html
- **Atlassian Git Tutorials** : https://www.atlassian.com/git/tutorials/comparing-workflows
- **Pro Git Book - Branching** : https://git-scm.com/book/en/v2/Git-Branching-Branches-in-a-Nutshell

**Prochaine section** : 32.3.3 Gestion des binaires) - Nous verrons comment gérer les fichiers binaires dans Git et quelles sont les bonnes pratiques pour les projets C.

⏭️ [Gestion des binaires](/32-documentation-maintenance/03.3-gestion-binaires.md)
